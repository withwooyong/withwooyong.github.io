...
function normalize(text, file = "<메모리>") {
  ...
  //
  // ⚠️ 잃는 것: 「이 페이지가 참조하는 청크 구성이 바뀌었다」를 못 보게 된다. 청크 파일
  //    자체는 walk() 가 `_next` 를 건너뛰어 애초에 이 검사의 대상이 아니다.
  //
  // 🔴 여기에 「참조가 바뀔 만한 변경은 렌더된 본문도 함께 바꾸므로 검사는 그쪽으로 잡는다」고
  //    적혀 있었는데 **2026-09-16 에 반증됐다.** 도식 컴포넌트 하나(`diagram-image.tsx`)를
  //    더하자 webpack 이 공용 청크 **번호**를 다시 매겨 `chunks/9232-…` 가 `chunks/1121-…`
  //    이 됐고, `product-lead-wiki` 5개가 GC-6 에 걸렸다 — **렌더된 본문은 한 글자도
  //    바뀌지 않았고** 파일 길이도 111,864 B 로 바이트까지 같았다 (번호가 둘 다 네 자리다).
  //
  //    그래도 번호는 지우지 않는다. 지우면 청크 구성 변화를 통째로 못 보게 되는데,
  //    그것은 「모르고 지나가는 것」이지 「걸려서 한 번 확인하는 것」보다 나쁘다.
  //    ⇒ 공용 컴포넌트를 더하면 이 검사가 걸리는 것이 **정상**이다. 내용이 정말 그대로인지
  //      배포본과 대조해 확인한 뒤 `--update` 로 갱신하라. 확인 없이 갱신하면 습관이 되고,
  //      그 순간 정작 지켜야 할 `product-lead*` 7개가 같은 명령으로 함께 덮인다.
  //
