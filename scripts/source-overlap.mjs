...
function toParagraphs(markdown) {
  const body = stripTableRules(markdown.replace(/^---\n[\s\S]*?\n---\n/, ""));
  const prose = [];
  const diagrams = [];
...
function scan(publishedPath, sourcePath, min) {
  ...
  const sourceLabels = new Set(
    toParagraphs(source)
      .diagrams.flatMap((d) => [...d.matchAll(/"([^"]+)"/g)].map((m) => m[1].replace(/<br\/?>/g, " ").trim()))
  );
  const labelHits = [];
  for (const chunk of paragraphs.diagrams) {
    for (const m of chunk.matchAll(/"([^"]+)"/g)) {
      const label = m[1].replace(/<br\/?>/g, " ").trim();
      if (sourceLabels.has(label)) labelHits.push(label);
    }
  }
  console.log(`③ 도식 라벨 통째 일치 (처방 ②) — ${labelHits.length}건 · 원본 라벨 ${sourceLabels.size}개`);
