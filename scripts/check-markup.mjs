...
//
// ② 🔴 GFM 취소선이 여는 `**` 를 삼킨다. 한 줄에 물결(`~`)이 둘 이상이고 닫는 `**` 가
//    줄 끝에 오면, 취소선 파싱이 물결 사이를 먼저 가져가면서 강조의 짝이 깨진다.
//    범위 표기(`①~③`)를 즐겨 쓰는 이 리포에서 실재했다.
//
//      ✗ 가~나 계보, 다 적용. **라~마 안 보인다.**
//
//    ⇒ **①의 「구두점 + 별표 + 문자」 정규식으로는 이 유형이 걸리지 않는다.** 닫는 별표
//    뒤가 줄 끝이라 패턴에 맞지 않는다. 이것이 근사를 쓰지 않는 두 번째 이유다.
//
...
function selfTest() {
  ...
  console.log(`\n마크업 검사기 자기 검사: ${pass}/${CASES.length}`);
