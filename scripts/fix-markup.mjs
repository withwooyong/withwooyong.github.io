...
const PARTICLES = [
  ...
  // 서술격 조사와 그 활용형. 인용이나 명제를 강조로 감싸고 「…이다.」·「…였다.」로 받는 것이
  // 이 리포의 상용구라 실제로 많다 — 실측 12곳 가운데 11곳이 여기였다.
  "이었다", "이지만", "인데", "이며", "이라", "이고", "였고", "였다", "이다", "인", "다",
  // 인용격 조사. 「고구마」·「라면」처럼 이것으로 시작하는 낱말은 뒤가 한글이라 아래 방어가
  // 걸러 내므로, 한 글자짜리를 넣어도 낱말을 삼키지 않는다.
  "고", "라",
...
const CASES = [
  ...
  { name: "⑯ 서술격 조사의 과거형 「였다」·「이었다」를 옮긴다",
    md: "관통한 명제가 **“그것”**였다.", fixes: 1, expect: "관통한 명제가 **“그것”였다**." },
  { name: "⑰ 서술격 조사의 연결형 「이고」·「였고」도 같다",
    md: "돌리는 것이 **빌드(Build)**이고, 다음이 온다", fixes: 1 },
  { name: "⑱ 인용격 조사 「라」·「고」를 옮긴다 — 인용을 강조로 감싸는 상용구다",
    md: "원 자료가 **「벤치마크」**라 표기했다", fixes: 1 },
  { name: "⑲ 🔴 「고」·「라」로 시작하는 낱말은 삼키지 않는다 — 「고구마」·「라면」",
    md: "**「가」**고구마를 본다", fixes: 0, skipped: 1 },
];
