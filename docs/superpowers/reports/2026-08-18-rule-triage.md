...
**매 세션 읽히던 105개 중 35개(사장 14 + 자동 21)가 사람 눈에서 빠진다.** 33%다.

> **2026-08-18 리뷰 반영 (2차).** 초판은 자동 18 · 사람 67이었다. 리뷰가 **자동 가능한 3건**
> (금지선 26 · `CR-1.2` · `CR-2.3`)을 사람 목록에서 찾아내 **「자동 — 미구현」으로** 옮겼고,
> `CR-4.1a`는 **「하한 자동 / 상한 사람」으로** 쪼갰다(§5-1). `CR-2.3`은 **분류만이 아니라
> 사유가 틀렸다** — 아래 §8 참조.
>
> ⚠️ **「자동 — 미구현」은 검사기가 아직 없다는 뜻이다.** 그때까지는 사람이 본다 —
> 체크리스트에 남겨 두고 「검사기 신설 시 여기서 뺀다」로 표시했다.
...
| 26 | 「첫 90일에 할 일」 — 그 배치의 명명 지시 | **발행본 9편이 지금도 이 문구를 쓰고 있다.** 원본 소진과 무관하게 서식 충돌을 막는 규칙이다 | **자동 — 미구현** |
...
| **26** | 「첫 90일에 할 일」을 편 제목·H2로 쓰지 마라 | **자동 — 미구현** | ⚠️ **2026-08-18 재판정.** 사장이 아니다(발행본 9편이 지금도 쓴다). 그리고 사람도 아니다 — **§2-1이 이미 검사기를 실행했다**: `grep -rn "^#\{1,3\} .*첫 90일" content/blog` → 0건. **그 한 줄이 곧 검사기다** |
...
### 5-1. ⚠️ 같은 오류를 나도 냈다 — `CR-4.1a`

**초판에서 `CR-4.1a`를 「자동」으로 올리고 본문의 수치 두 개(15~25 KB · 절 25 KB)를 걷었다.
그런데 위임한 검사기가 상한을 명시적으로 거부한다:**

```
tests/blog/content/schema.test.ts:15
/** 분량 하한 … 상한은 검사하지 않는다 — 금지선 13. */
const MIN_BYTES = 13_300;
```

⇒ **규칙이 실행 불가능해졌다.** 수치는 문서에서 사라졌는데 검사기에는 도착하지 않았다.
**이것이 정확히 금지선 55다** — 「`source`를 걷자 `CR-1.10`의 뒷받침이 사라졌다.
규칙은 유효한데 근거가 없어졌다」. 걷기 전에 **받을 자리가 실재하는지** 재지 않았다.

**원인은 두 상한을 한 덩어리로 본 것이다:**

| | 금지선 13 | `CR-4.1a` |
| --- | --- | --- |
| 대상 | 이미 쓴 **발행본의 크기** | **분할할 때 어디서 자르나** |
| 요구 | 「넘어도 된다. 크기를 이유로 내용을 빼지 마라」 | 「절 하나가 25 KB 넘으면 H3 경계로 내려가라」 |
| 시점 | 발행 후 판정 | 분할 설계 시 |

**13은 「줄이지 마라」이고 `CR-4.1a`는 「어디서 자르나」다.** 검사기가 하한만 보는 것이
옳다 — 상한을 HARD로 걸면 13을 어긴다.

⇒ **`CR-4.1a`가 상한 수치를 되찾았고, 분류를 「하한 자동 / 상한 사람」으로 좁혔다.**
금지선 45를 좁힌 것과 같은 조작이다.

> **압축에는 방향이 있다.** 「검사기가 정본」은 검사기가 **그것을** 볼 때만 참이다.
> 걷기 전에 받을 자리를 실측하라 — 파일이 있다는 것과 그 파일이 이 규칙을 진다는 것은 다르다.
...
| CR-1.2 | 원본 메타 인용문을 본문에서 제거 | **자동 — 미구현** | 리터럴 패턴. 본문 앞부분 grep. 실측 미발행 원본 **141개**가 이 패턴을 갖고 발행본 **0개** |
...
| CR-2.3 | 답변마다 표·코드·도식 1개 이상 | **자동 — 미구현** | ⚠️ **사유 정정.** 「Q&A 편을 기계가 식별하지 못한다」고 적었으나 **틀렸다** — `*qna*` 슬러그가 발행본 13편을 **예외 0건**으로 잡는다. `^## Q. ` 로 쪼갠 블록마다 표·코드펜스·mermaid를 본다(질문 251개) |
...
| CR-4.1a | 목표 분량 15~25 KB · 절 25 KB 초과 시 H3로 | **하한 자동 / 상한 사람** | ⚠️ **판정 정정.** 하한 13.3 KB만 `tests/blog/content/schema.test.ts` 가 본다. 그 파일 `:15`가 **「상한은 검사하지 않는다 — 금지선 13」이라** 명시하므로 **상한 25 KB는 검사기가 없다.** 아래 §5-1 참조 |
...
| 설계서 §11 (취소선 유지) | 사장 | 14 |
| 검사기 — **이미 있다** | 자동 · 구현됨 | 16 |
| 검사기 — **아직 없다** (그때까지 사람이 본다) | 자동 · 미구현 | 5 |
| `PUBLISHING-CHECKLIST.md` | 사람 — 발행 판단 | 57 |
| `CLAUDE.md` 코드 제약 | 사람 — `GC-*` | 7 |
| `CLAUDE.md` 도구 함정 | 환경 | 6 |

**자동 21건의 구현 현황** — 「자동으로 분류했다」와 「검사기가 있다」는 다르다:

| | 구현됨 | 미구현 |
| --- | --- | --- |
| 금지선 (6) | 10 · 11 · 52 · 56 · 58 | **26** |
| `GC-*` (6) | 1 · 2 · 5 · 11 · 12 | **6** (`check-baseline.mjs`) |
| `CR-*` (9) | 1.4 · 3.5 · 4.1a 하한 · 4.2 · 5.1 · 5.2 | **1.2 · 1.7 · 2.3** |

⚠️ **`CR-4.1a`는 하한만 자동으로 셌다. 상한은 사람이고 체크리스트가 본다**(§5-1).
미구현 5건은 체크리스트에 남겨 두고 **「검사기 신설 시 여기서 뺀다」로** 표시했다 —
분류만 바꾸고 목록에서 지우면 그동안 아무도 안 본다.
