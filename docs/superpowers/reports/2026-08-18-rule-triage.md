...
| 11 | 분량 하한 미달이면 병합. 억지로 늘리지 마라 | **자동** | `CR-4.1a`와 동일 · 설계서 §3-3 SOFT |
...
| CR-4.1a | 목표 분량 15~25 KB | **자동** | 신설 분량 하한 SOFT 검사 (금지선 11과 동일) |
...
| 3 | 신설 검사기 경로가 아직 확정이 아니다 | 설계서는 `tests/blog/content/schema.test.ts`를 예고했으나 커밋 `63bd4ff`은 `tests/blog/frontmatter.test.ts`에 넣었다. 문서는 **실재하는 경로만** 가리키고 미구현분은 「신설 예정」으로 표기했다 |
