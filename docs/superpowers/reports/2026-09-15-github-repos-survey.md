...
> **처리 (2026-09-16).** 23곳의 설명을 고쳤습니다. 문안과 대조 결과는 이 절 끝의
> 「설명 반영 결과」에 있습니다. 아래 표의 「설명 상태」 열은 **판정 시점의 값이므로 그대로
> 둡니다.**
...
「설명 상태」가 빈약(20) 또는 어긋남(3)인 23곳의 설명을 `gh repo edit -d` 로 고쳤습니다.
**topics 는 건드리지 않았습니다.** 문안은 43개 표의 「성격 요약」 열에서 출발했고, 어긋남 셋은
저장소를 다시 열어 현재 상태를 확인한 뒤 썼습니다. 이번에도 **명령의 성공 보고(23 / 23)가 아니라
API 를 다시 읽은 값으로 판정했습니다.**

문안은 다음 원칙으로 썼습니다.

| 원칙 | 이유 | 적용한 곳 |
| --- | --- | --- |
| 무엇을 하는 저장소인지를 먼저 쓰고, 기술 구성은 뒤에 붙입니다 | 목록 화면은 설명 앞부분만 보여 주므로, 앞에서 용도가 읽혀야 합니다 | 23곳 모두 |
| 옛 설명에 들어 있던 정보는 버리지 않습니다 | 빈약한 설명에도 소유자만 아는 사실이 섞여 있습니다 | `test` 의 「AI 서비스 개발」, `yanadoo-api` · `yanadoo-webview` 의 「3.0 재구축」 |
| 짝을 이루는 저장소는 서로의 이름을 적습니다 | 한쪽만 열어도 다른 쪽을 찾아갈 수 있습니다 | `trans-manager` ↔ `trans-coder`, `spring-cloud-config-repository` → `spring-cloud-config-server`, `yanadoo-webview` → `yanadoo-api` |
| 곧 바뀔 목록과 수는 적지 않습니다 | `pm-plugin` 이 어긋난 원인이 바로 명령 목록의 열거였습니다 | `pm-plugin` 은 명령 이름 대신 역할의 범위를, `ted-startup` 은 에이전트 수 대신 파이프라인의 범위를 적었습니다 |
| 저장소에서 확인되지 않는 상태는 적지 않습니다 | 로컬에서만 한 일은 저장소가 증언하지 못합니다 | `org-chart` 의 매일 자동 실행 |
| private 저장소는 성격까지만 씁니다 | 이 문서는 공개 저장소에 있습니다 | 시스템 이름 · 수치 · 내부 결정 기록을 옮기지 않았고, 아래 표에서도 옛 설명의 원문 대신 모양만 적었습니다 |

| 리포지토리 | 공개 | 옛 설명의 모양 | 새 설명 |
| --- | --- | --- | --- |
| `elasticsearch-webcrawler` | public | 이름 되풀이 | 포털에서 방송 메타데이터를 수집해 Elasticsearch 에 적재하고, 한글 형태소 분석을 적용해 검색이 되는지 확인한 PoC (2016). |
| `trans-manager` | public | 이름 되풀이 | FFmpeg 영상 변환 작업을 ActiveMQ 와 Redis 로 트랜스코더에 나눠 주는 Spring Boot 관리 서버. 작업을 받아 실제로 변환하는 쪽은 trans-coder 입니다 (2016). |
| `trans-coder` | public | 이름 되풀이 | trans-manager 에서 작업을 받아 FFmpeg 로 영상을 변환하는 트랜스코더 클라이언트. 두 저장소가 한 쌍으로 동작합니다 (2016). |
| `DataStructureAndAlgorithmsMadeEasyInJava` | public | 낱말 몇 개 | Data Structures and Algorithms Made Easy in Java 책의 예제를 따라 구현한 자료구조 · 알고리즘 학습 저장소 (2016). |
| `php` | public | 낱말 몇 개 | PHP 를 하루 동안 익히며 작성한 실습 스파이크 (2018). |
| `test` | private | 낱말 몇 개 | AI 서비스 개발용으로 만들었으나 README 와 첫 커밋만 남은 자리 표시 저장소 (2020). |
| `deepmeta-api` | private | 이름 되풀이 | Kafka · RabbitMQ · MyBatis · Oracle 을 갖춘 Spring Boot API 서버의 초기 뼈대 (2020). |
| `yanadoo-corp` | private | 낱말 몇 개 | 인가 서버 · Keycloak SSO · 게이트웨이 예제를 모아 둔 사내 개발 참고용 저장소 (2022). |
| `demo-payment` | private | 낱말 몇 개 | JPA · Querydsl · MyBatis 로 결제 API 를 구성해 본 Spring Boot 개발 데모 (2022). |
| `demo-order` | private | 커밋 메시지 조각 | 결제 서비스 로직을 고치던 중간 상태를 남긴 Spring Boot · JPA · Querydsl 작업 저장소 (2022). |
| `payment-v2` | private | 커밋 메시지 조각 | 결제 시스템 2차 판의 엔티티와 리포지토리를 MSSQL 기준으로 구성해 컴파일까지 확인한 Spring Boot 작업 저장소 (2022). |
| `spring-cloud-config-repository` | public | 이름 되풀이 | 개발 · 스테이징 · 운영 환경별 설정 YAML 셋을 담은 Spring Cloud Config 저장소. spring-cloud-config-server 가 이곳을 git.uri 로 읽습니다 (2022). |
| `yanadoo-api` | private | 두 저장소가 같은 설명 | 서비스 3.0 재구축을 위해 Spring Boot API 서버를 커밋 하나로 통째 올린 스냅샷. JPA · Querydsl · MyBatis · MSSQL 로 구성됩니다 (2023). |
| `yanadoo-webview` | private | 두 저장소가 같은 설명 | 서비스 3.0 재구축에서 앱 웹뷰 화면을 Thymeleaf 로 내려 주는 Spring Boot 서버의 스냅샷. 같은 재구축의 API 서버는 yanadoo-api 입니다 (2023). |
| `yafit-cycle` | private | 커밋 메시지 조각 | 운동 서비스의 웹 화면을 Spring Boot 와 Thymeleaf 로 내려 주는 재직 중 실무 산출물 (2023). |
| `demo-jwt` | private | 이름 되풀이 | Spring Security 에 JWT 인증과 Redis 를 붙여 본 단일 기능 스파이크 (2024). |
| `grit-api` | private | 커밋 메시지 조각 | 기업 교육 서비스의 화면별 API 를 옮겨 만든 Spring Boot · JPA · MSSQL 백엔드. 재직 중 실무 산출물입니다 (2024). |
| `scheduler` | private | 커밋 메시지 조각 | 사내 스케줄러를 다시 개발한 Spring Boot 서버. ShedLock 으로 여러 인스턴스에서의 중복 실행을 막습니다 (2024 ~ 2025). |
| `accounting-k` | private | 낱말 몇 개 | 회계 시스템을 Kotlin 으로 옮기며 헥사고널 구조를 시험했다가 계층형 구조로 되돌린 Spring Boot 저장소 (2026). |
| `accounting` | private | 낱말 몇 개 | Spring Boot 와 React 로 만든 회계 · 정산 관리 시스템. 백엔드는 Spring Batch 와 MSSQL 을 함께 씁니다 (2026). |
| `ted-startup` | private | 설명과 내용이 갈라짐 | Claude Code 멀티에이전트로 기획부터 배포까지 진행하는 SDLC 파이프라인과, 그 파이프라인으로 만드는 주식 데이터 서비스를 함께 담은 저장소. 서비스는 FastAPI 와 Next.js 로 시세 수집 · 데이터 품질 · 시그널 · 백테스트를 다룹니다 (2026). |
| `org-chart` | private | 설명과 내용이 갈라짐 | 사내 그룹웨어 조직도를 Playwright 로 내려받아 전날 스냅샷과 비교하고, 부서 트리를 검색할 수 있는 HTML 페이지로 만드는 Python 도구 (2026). |
| `pm-plugin` | private | 설명과 내용이 갈라짐 | PM 팀이 team-wiki 저장소에 근거와 산출물 문서를 쌓도록 돕는 Claude Code 플러그인. 근거를 올리는 일부터 병합 뒤 위키를 다시 쓰는 일까지 명령으로 제공합니다 (2026). |

**23곳을 모두 「설명만 읽고 무엇을 하는 저장소인지 알 수 있다」는 「충분」 기준에 맞춰
고쳤습니다.** 「옛 설명의 모양」 열의 분포(이름 되풀이 6 · 커밋 메시지 조각 5 · 낱말 몇 개 7 ·
같은 설명 2 · 갈라짐 3)는 위 「무엇이 문제인가」 표와 같습니다. 새 설명 열은 문안 파일에서
그대로 옮겼고, 그 파일이 아래 대조의 기준입니다.

##### 어긋남 셋을 다시 열어 확인한 것

| 저장소 | 다시 연 것 | 현재 상태 | 문안에 반영한 방식 |
| --- | --- | --- | --- |
| `ted-startup` | 최상위 트리 · 소스 하위 디렉터리 · 최근 커밋 100건의 접두사 · README | README 와 에이전트 정의 디렉터리는 여전히 멀티에이전트 플랫폼을 가리키고, 최근 커밋은 데이터 품질 · 백테스트 · 시세 수집 · 시그널을 다룹니다. **둘 중 하나가 낡은 것이 아니라 한 저장소에 둘이 함께 있습니다** | 둘을 한 설명에 모두 적었습니다. 옛 설명은 앞의 절반만 말하고 있었습니다 |
| `org-chart` | README · 인수인계 문서 · 커밋 이력 | 옛 설명이 말하던 Slack 전송은 폐기되었고 결과물은 HTML 페이지로 바뀌었습니다. **매일 자동 실행을 등록하는 단계는 마지막 푸시(2026-04-28) 시점에 대기로 남아 있습니다** | 자동 실행 여부를 적지 않았습니다. 로컬에서 등록했더라도 저장소로는 확인할 수 없습니다 |
| `pm-plugin` | README 의 명령 표 · 최근 커밋 | 명령이 다섯인데 옛 설명은 넷을 나열했고, 빠진 것은 병합 명령입니다 | 명령 이름을 나열하지 않고 역할의 범위로 적었습니다. 명령이 늘어도 이 설명은 낡지 않습니다 |

**세 곳 모두 판정은 맞았습니다.** 다만 `org-chart` 에서는 43개 표의 「성격 요약」(「매일 받아
변경분을 정리하는 사내 자동화」)이 저장소가 보여 주는 상태보다 한 단계 앞서 있었습니다. 같은 행의
topics `automation` · `launchd` 는 저장소의 결정 기록이 정한 실행 방식이므로 틀렸다고 보지 않았고,
topics 는 이번 범위가 아니어서 그대로 두었습니다.

##### 대조

| 대조 | 방법 | 결과 |
| --- | --- | --- |
| 전체 수 | §5-4 의 두 명령을 작업 전후에 실행 | 설명 누락 0 → 0 · topics 누락 0 → 0 |
| 23곳의 문안 | 저장소마다 `gh api repos/withwooyong/<이름>` 으로 설명을 다시 읽어 문안 파일과 문자열로 비교 | 짝 23 · 불일치 0 |
| 23곳이 실제로 바뀌었나 | 작업 전 스냅샷과 비교 | 짝 23 · 그대로인 곳 0 |
| 대상 밖 설명 보존 | 나머지 112곳의 설명을 작업 전후로 비교 | 짝 112 · 달라진 곳 0 |
| topics 보존 | 직접 생성 135곳의 topics 를 작업 전후로 비교 | 짝 135 · 달라진 곳 0 |
| 공개 범위 보존 | 135곳의 public · private 를 작업 전후로 비교 | 달라진 곳 0 |

🔴 **첫 행의 두 수는 이번 작업을 보지 못합니다.** §5-4 의 명령은 설명이 **비어 있는지**만 세므로
작업 전에도 0 이었고 작업 후에도 0 입니다. 이 행이 증명하는 것은 「고치다가 비운 곳이 없다」
하나뿐이며, 설명이 실제로 바뀌었다는 근거는 둘째 · 셋째 행입니다.

🔴 **보존 대조의 0 은 짝을 세어 확인했습니다.** 전후 목록을 이름으로 짝지은 뒤 값이 다른 곳을
세는 방식이라, 짝을 못 찾아도 0 이 나옵니다. 그래서 짝의 수(135 · 112 · 23)를 함께 출력했고,
작업 후 스냅샷의 사본에서 topics 하나와 대상 밖 설명 하나를 일부러 바꾸면 두 결과가 각각
1 이 되는 것까지 확인했습니다.
...
| §4-5 설명 반영 후 | 0 | 0 | 빈약 · 어긋남 23곳의 설명 교체. **이 두 수로는 보이지 않습니다** |
...
마지막 줄은 두 수가 움직이지 않았는데도 기록합니다. **설명을 채우는 일은 이 명령으로 셀 수
있지만, 이미 있는 설명을 고치는 일은 셀 수 없습니다.** 그 작업의 증거는 §4-5 「설명 반영 결과」의
저장소별 되읽기 대조에 있습니다.
...
| ~~빈약하거나 어긋난 설명 고치기~~ | ~~23~~ | **완료.** §4-5 「설명 반영 결과」에서 고치고 저장소별로 되읽어 대조했습니다. topics 는 건드리지 않았습니다 (2026-09-16) |
