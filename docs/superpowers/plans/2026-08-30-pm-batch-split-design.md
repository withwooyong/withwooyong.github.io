...
| ③ | **시리즈 A 편1(지도편)을 먼저 쓰고 실측으로 고정 비용을 확정** | 미착수 — §10-1 |
...
**⇒ 이 표는 편1 착수 전에 아래 명령으로 채운다. 지금은 비워 둔다.**

```bash
# 후보 글에 낱말이 실재하는지 확인하고 행 번호를 얻는다
cd content/blog && grep -n "프로덕트\|기획\|PRD\|이해관계자" \
  agentic-coding/*.md ai-transformation/*.md | head -40
```

| 걸 글 | 행 번호 | 낱말 | 가리킬 곳 | 확인 |
| --- | ---: | --- | --- | :---: |
| (편1 착수 전 채운다) | | | | ☐ |
...
| ---: | --- | --- |
| 1 | 설계서 | 이 문서 |
| 2~10 | A 편1~9 | 편마다 발행 + 검증 |
| 11~18 | B 편1~8 | 편마다 발행 + 검증 |
| 19 | 시리즈 간 링크 | §6-2 4건 |
| 20 | 카테고리 간 링크 | §6-3 |
...
| # | 가설 | 판정 |
| :---: | --- | :---: |
| **H-1** | 고정 비용의 단조 감소(C-8)는 **시리즈 경계에서 리셋된다** | ☐ |
