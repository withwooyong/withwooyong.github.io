...
| ④ | 🔴 **시리즈 A 편2를 쓰고 인용 비율을 실측해 편3 이후를 재환산** | 미착수 — §3-6 |
...
| # | 가설 | 판정 |
| :---: | --- | :---: |
| **H-1** | 고정 비용의 단조 감소(C-8)는 **시리즈 경계에서 리셋된다** | 🔶 **A 편1 실측: 리셋보다 강하다** — 3,750 으로 앞 배치 편1(3,338)보다 높다. B 편1 에서 확정 (§10-1-b) |
| **H-2** | 시리즈별 k 분리(A 1.85 · B 2.05)가 단일 k 보다 정확하다 | ☐ |
| **H-3** | 인용은 **세 용도로 갈라지고**, 배정에 남은 ①·②의 배율은 산문 배율에 가깝다 | ☐ |
...
| 항목 | 설계값 | 실측 | 차 |
| --- | ---: | ---: | ---: |
| 발행 B | 21,000 | **22,678** | +1,678 (+8.0%) |
| 원본 §0 | 8,646 | 8,646 | — |
| 겉보기 배율 | 2.429 | **2.623** | +0.194 |
| 고정 비용 (도입 + 정리) | (상수 2,455) | **3,750** (2,842 + 908) | +1,295 |
| 용어 개수 | 표에서 추출 | **36개 전량 승계** | — |
| 용어절 B | — | **5,266** | — |
| 도식 | (편당 평균 671) | **2개 · 849 B** | +178 |

⚠️ 발행 B 는 형제편 링크 8개를 **뺀** 값이다. 아래 10-1-c 가 이유를 적는다. 링크를 전부
붙이면 약 +384 B 로 **23,062** 가 된다.

#### 10-1-a. 🔴 C-3(지도편 용어당 B 는 단조 상승한다)이 **반증됐다**

앞 배치가 두 표본으로 지지 판정을 내린 상수다(`208.7 → 215.3`). 이번 값은 **146.3** 이라
크게 어긋나므로, 표본 하나로 뒤집기 전에 **발행된 지도편 전량을 한 번에 쟀다.**

| 용어 수 | 용어절 B | 용어당 B | 편 |
| ---: | ---: | ---: | --- |
| 7 | 600 | 85.7 | `search-system-overview` |
| 12 | 1,158 | 96.5 | `redis-cache-design` |
| 19 | 1,767 | 93.0 | `rag-knowledge-map` |
| 23 | 4,783 | 208.0 | `notification-center-architecture-map` |
| 23 | 4,934 | 214.5 | `auth-service-launch-map` (앞 배치 편1) |
| 24 | 4,955 | 206.5 | `batch-processing-fundamentals` |
| 29 | 4,454 | 153.6 | `realtime-transport-and-websocket` |
| 33 | 3,391 | 102.8 | `rdbms-and-nosql-fundamentals` |
| 34 | 5,217 | 153.4 | `cicd-pipeline-fundamentals` |
| 36 | 4,749 | 131.9 | `partitioning-fundamentals` |
| 36 | **5,266** | **146.3** | **이번 편1** |

**용어당 B 는 독립 변수가 아니라 파생값이다.** 용어절 바이트는 용어가 23개를 넘으면
개수와 거의 무관하게 **3,391 ~ 5,266** 사이에 머문다(8편 평균 4,719). 개수가 늘면 풀이가
짧아져 총량이 그 구간에 수렴하기 때문이다. 앞 배치가 본 「단조 상승」은 **초기 지도편의 용어가
7·12·19개로 적었던 것**을 발행 순서의 효과로 읽은 것이다 — 실제 변수는 순서가 아니라 개수다.

⇒ **예산은 「용어당 B × 개수」가 아니라 용어절을 5,000 B 안팎의 상수로 잡는다.**
B 시리즈 지도편(용어 3표 합산·중복 제거)에도 이 상수를 쓴다. 개수가 몇으로 나오든
용어절이 5,000 을 크게 넘으면 풀이가 길어진 것이지 용어가 많은 것이 아니다.
...
H-1 은 「시리즈 경계에서 리셋된다」였는데, 실측은 리셋보다 강하다 — **앞 배치 편1보다 412 B
높다.** 도입이 2,842 인 것이 원인이고, 이 배치의 도입이 길어진 이유는 §4-2 가 요구한
...
검출된 7건 가운데 5건(용어 풀이)은 재작성했고, 남은 2건은 정당으로 판정했다.

| 남은 겹침 | 판정 근거 |
| --- | --- |
| 원 자료 인용문 45자 | `>` 블록으로 인용을 명시했고 앞 문장이 출처를 밝힌다 |
| 「용어명 + 원어」 20~25자 | 용어표의 첫 두 열은 정의상 원본과 같다. 실질 겹침은 그 뒤 한 낱말뿐 |
