...
| 항목 | 설계 | 실측 | 차 |
| --- | ---: | ---: | ---: |
| 원본 B (면접 블록 제외) | 8,101 | **8,126** | +25 |
| 발행 B | 19,117 | **19,008** | 🔴 **−109 (−0.6%)** |
| 밴드 | 17,985 ~ 20,250 | — | **안** (상단까지 **1,242**) |
| 고정 비용 | (상수 2,455) | **3,346** (1,235 + 1,053 + 1,058) | 🔴 **+891** |
| 역산 k — 상수 2,455 기준 | 2.05 | **2.030** | −0.020 |
| 역산 k — 실측 고정 비용 기준 | 2.05 | **1.921** | −0.129 |
| 표 | (≈1.05 배) | 4,255 (원본 1,811 · 🔴 **2.350**) | +1.30 |
...
§10-12 는 편3의 0.004 를 근거로 「**상수 2,455 가 첫 편 이후로는 정확하다**」고 적었다. **편4가
반례다.** 세 표본이 3,013 · 2,479 · 3,346 으로 흩어져 있어 어느 값으로도 수렴하지 않는다.

⇒ **예산 산식은 그대로 둔다**(상수 2,455 · k 2.05). 세 편의 상수 기준 k 가 2.163 · 2.188 ·
2.030 으로 모두 밴드 1.91~2.19 안이므로 산식은 여전히 쓸 만하다. **취소하는 것은 산식이
아니라 「고정 비용이 상수에 수렴한다」는 문장이다.**

🔴 **표본 하나로 수렴을 선언한 것이 잘못이었다.** 두 k 가 붙는 것은 고정 비용이 우연히 상수와
가까울 때 자동으로 일어나는 일이지 경향의 증거가 아니다.
...
두었으므로 지금의 여유 1,242 에서 55 를 뺀 **1,187** 이 실질 여유다.
