...
> ## ⏸️ 이 단계는 2026-08-26 에 **보류됐다. 아래 T9~T13 을 그대로 실행하지 마라.**
>
> 사용자가 현재 메인 페이지의 히어로(`hero-hello` 손글씨)를 유지하기로 했고, 지식 아틀라스와
> 검색을 먼저 만들기로 했다. 그쪽이 **[`2026-08-26-atlas-and-search.md`](./2026-08-26-atlas-and-search.md)** 다.
>
> **T9 는 실제로 구현했다가 반려됐다.** 리뷰에서 나온 4건이 전부 구현이 아니라 **계획 자체의 결함**이었다 —
> 액센트 면적 **15.68%**(GC-9 상한의 3.1배) · LCP 가 서드파티 폰트 **216KB** 에 묶임 ·
> reduce 에서 h1 이 선행사를 잃음 · 정적 구간 **66.7vh**. 수치와 근거는 새 계획서의
> §「이월된 것」에 옮겨 뒀다. **히어로를 다시 설계할 때 그 절부터 읽어라.**
>
> T10~T13(메인 5섹션 · `/work` · `/about` · 스텁)은 손대지 않았고 그대로 유효하다.
> 다만 새 계획서의 **GC-11 이 `pages/index.tsx` 수정을 금지**하므로, 그쪽이 끝난 뒤에 재개한다.
