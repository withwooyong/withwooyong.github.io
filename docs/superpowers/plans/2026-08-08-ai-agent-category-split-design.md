...
**작성일**: 2026-08-08
**상태**: **실행 완료 (2026-08-08)** — 설계 51편 = 발행 51편, 도식 158 = 158
**근거**: [요구사항 §13·§14](../specs/2026-08-07-tech-blog-requirements.md), 조사 에이전트 6기 실측
**실행에서 확정된 규칙**: [요구사항 §15](../specs/2026-08-07-tech-blog-requirements.md)

> ## 실행 결과
>
> | 배치 | 대상 | 편수 | 도식 | 커밋 |
> | ---: | --- | ---: | ---: | --- |
> | B1 | `langgraph/01·02·03` | 7 | 22 | `6792ac6` |
> | B2 | `langgraph/04·05·06` | 10 | 27 | `236fac8` |
> | B3 | `langgraph/07·08·09·10` | 9 | 25 | `840e280` |
> | B4 | `rag/07·08` | 8 | 36 | `ed468e0` |
> | B5 | `rag/09·10` | 12 | 45 | `68f98a2`·`715a89f` |
> | B6 | Q&A | 4 | 0 | `45d3235` |
> | B7 | 지식 지도 | 1 | 3 | `102d4c8` |
> | **계** | | **51** | **158** | + 검증 `73f5b60`·`b654a5a`·`629ea9c` |
>
> **검증**: `npm test` 36 · `tsc` 0 · `build` 0 · sitemap 83 → **142 URL** · 내부 링크 깨짐 0 · **들어오는 링크 0인 글 0** · 금칙어·화행 잔재·플레이스홀더 0
>
> **독립 검증이 찾은 결함 6건** — 전부 반영. 지어낸 문항 **0건**(Q&A 94문항 전수 원본 대조), 표 수치 셀 단위 일치. 실제 결함은 §15-3의 세 유형이었다 — 화행 잔재, 세어 쓴 수가 틀림, 링크 주장 불일치.
...
### ⚠️ 의도적 도식 감소 2건 — `langgraph/11`의 도식 4개

`langgraph/11`은 H2의 84%가 압축본이라 **Q&A 4편으로만 나간다**(§5-1). 그 안의 mermaid 4개는 전부 앞 문서 도식의 재현이다(예: `11:340` Supervisor 라우팅 도식 = `06 §3-2`).

| 처리 | 이유 |
| --- | --- |
| **Q&A 편에 싣지 않는다** | 같은 도식이 카테고리 안에서 두 번 나온다. §4-2 정본 고정 규칙과 같은 원리 |
| 해당 주제 편으로 **링크** | Q&A 답변은 3~5줄 + 원문 링크가 원칙이다 |

CR-2.3("답변마다 표·코드·도식 중 하나를 최소 1개")은 **표로 충족**한다.

### 최종 기대값

```
원본 163
  − 1   rag/08 §6-4 계층형 팀 도식 (3중 중복)
  − 4   langgraph/11 도식 (Q&A 편은 링크로 대체)
  ─────
  158
```
...
| 배치 | 원본 | 발행 | 누계 |
| --- | ---: | ---: | ---: |
| B1 `langgraph/01·02·03` | 22 | **22** | 22 |
| B2 `langgraph/04·05·06` | 27 | **27** | 49 |
| B3 `langgraph/07·08·09·10` | 25 | 25 | 74 |
| B4 `rag/07·08` | 37 | **36** | 110 |
| B5 `rag/09·10` | 45 | 45 | 155 |
| B6 Q&A (`langgraph/11`) | 4 | **0** | 155 |
| B7 `langgraph/00` | 3 | 3 | **158** |

**B1·B2는 실측 확인 완료**(각각 22·27, 누계 49).
...
## 부록 B. 3단계에서 일괄 수정할 것 — 면접 화법 잔재

발행 34편(B1~B4)을 전수 스캔한 결과다. **금칙어 스캔이 못 잡는 유형**이라 별도로 기록한다 — `인용할 때는 "…" 수준으로 말하는 것이 안전하다`에는 금칙어가 한 개도 없다. 문제는 단어가 아니라 **문장이 독자에게 발화를 지시한다**는 것이다.

### 원인 — 원본이 조건과 화법을 한 문장에 붙여 놓았다

| 원본 문장 | 살려야 할 것 | 걷어야 할 것 |
| --- | --- | --- |
| `면접에서 인용할 때는 "2025년 4월 기준 4천 건대" 수준으로 말하는 편이 안전하다` | 시점·범위 조건 | 발화 지시 |
| `이것은 X사의 실적이며 작성자 본인의 실적이 아니다` | 출처가 제3자라는 사실 | 실적 경계 선언 |
...
| 파일 | 행 | 원문 | 치환 방향 |
| --- | ---: | --- | --- |
| `build-or-buy-agent-stack.md` | 21 | `…정리자의 수행 실적이 아니다` | `…모두 원 자료의 것이다`로 절단 |
| 〃 | 41 | `인용할 때 둘을 섞으면 안 된다` | `두 근거는 성격이 달라 함께 묶이지 않는다` |
| 〃 | 55 | `인용할 때 이 편향을 함께 말해야 한다` | `이 편향이 표 전체에 깔려 있다` |
| 〃 | 100 | `인용할 때는 "…" 수준으로 말하는 것이 안전하다` | **문장 삭제.** 98행 `정확한 숫자를 단언하면 안 된다`가 이미 같은 규칙이다 |
| `mcp-a2a-landscape.md` | 100 | `범위로 말하는 편이 정확하다 … 반박당한다` | `이 지표는 "2025년 4월 기준 4천 건대"라는 범위로만 확정된다` |
| 〃 | 102 | `현재형으로 말하면 안 된다` | `현재형으로 쓸 수 없다` |
| 〃 | 184 | `이 표를 인용할 때는 어느 쪽 근거인지 반드시 명시해야 한다` | `두 값은 근거가 달라 합산되지 않는다` |
| `mcp-rag-as-service.md` | 52 | `인용할 때 시점을 붙이지 않으면 … 현재로 말하거나` | `시점이 없으면 오래된 값이 현재값으로 읽힌다` |
| `groundedness-cost-limits.md` | 87 | `반박당하지 않는다 … 근거를 요구받고 … 의심받는다` | `좁힌 주장은 근거 범위 안에서 성립한다` |

### 경계 4건 — 판정

| 파일 | 행 | 판정 |
| --- | ---: | --- |
| `groundedness-cost-limits.md` | 50 | **수정.** H3 제목 `함께 말해야 할 다섯 가지 한계` → `함께 따라붙는 다섯 가지 한계` |
| `mcp-a2a-landscape.md` | 254 | 수정(경미). `말하면` → `쓰면` |
| `dynamic-rag-code-agent.md` | 21 | 수정(경미). `섞어 말하면` → `섞어 다루면` |
| `mcp-a2a-landscape.md` | 19 | **유지.** 이 글의 서술 규칙 선언이지 발화 지시가 아니다 |

### 오탐 18건 — 다시 히트해도 고치지 마라

| 유형 | 예 |
| --- | --- |
| 구현 권고 | `이렇게 구현하는 편이 안전하다` |
| 주어가 모델 | `LLM이 답한다` |
| 현장 묘사 | `6개월 뒤 "왜 안 똑똑해지냐"는 질문을 받는다` |
| 비유·수사 | `표준어 하나만 말하면 되는 구조` |
| 사고실험 | `물을 대상이 없다` |
| **이미 전환된 것** | `mcp-a2a-landscape.md:300` — 원본의 "면접 답변 소재"를 **"아키텍처 리뷰 판정 질문"으로 이미 바꾼 것**이다. 되돌리면 안 된다 |

### 검사 정규식

```
인용할 때|라고 말|수준으로 말|것이 안전|말하는 순간|실적이 아니|직접 한 것이 아니|듣는 사람|물어보면|반박당|의심받|감점
```
