...
| 가설 | 예측한 값 | 실측 | 판정 |
| --- | ---: | ---: | :---: |
| **C-1** 본편 비율은 발행 순서로 커진다 | 05(2.06) **이상** | **2.101** | **지지** |
| **C-2** Q&A 항목당 B는 원본 B로 커진다 | **≥ 496** (02 최대 초과) | **503.3** | **지지** |
| **C-3** 지도편 용어 비율은 단조 상승한다 | **≥ 1.81** | 0.96 | **검정 불가** |
| **C-4** 비율은 원본 **성분**에 갈린다 — 표는 낮고 불릿/산문은 높다 | 표 **≈1.4** · 비표 **≈2.3** | 표 **1.14** · 비표 **2.32** | **방향 지지 · 크기 미달** |

**C-1은 아슬아슬하게 지지된다.** 2.101 대 2.06으로 여유가 2%뿐이고, 그마저 마감 순증 310 B가
없었다면 2.097이다 — 판정은 바뀌지 않으나 **표본 6개로 세운 추세의 해상도가 그 정도**라는 뜻이다.
다음 배치가 2.05를 내면 추세는 깨지지 않고 **평평해진다.** 값이 아니라 밴드로 물려주라는
§3-2의 지시가 여전히 옳다.

**C-2가 두 번째로 지지됐다 — 이제 상수가 아니라 방법이다.** 앞 배치는 「Q&A는 비율이 아니라
항목 수」를 한 표본에서 발견했고, 이번은 **다른 원본에서 재현**했다. 편7을 비율로 잡았다면
목표는 28,000 ÷ 15,916 = **1.76**으로 5배치 실측 최소(1.41)와 최대(2.18) 사이 아무 데나
찍는 값이었다. 항목 수로 잡아 26,300~29,200 밴드를 얻었고 **29,192로 여유 8 B에 착지했다.**

#### 🔴 C-3의 미달은 억제만으로 설명되지 않는다 — **두 갈래다** `[마감 실측]`

종전 문단은 미달 전부를 용어 억제로 돌렸다. **발행본을 절 단위로 재 보니 그렇지 않다.**

| | 원본 §0 | 발행 「용어 정리」 | 코퍼스 5편 |
| --- | ---: | ---: | ---: |
| 바이트 | 5,443 | **5,233** | — |
| 용어 수 | 51 | **34** (승계 26 + ★신규 8) | 21~32 |
| **용어당 B** | 107 | **154** | 56 → 114 → 149 → 186 → **207** |

⇒ **개수도 밀도도 아래로 빗나갔다.**

| 갈래 | 실측 | 비율 기여 |
| --- | --- | ---: |
| ① 개수 억제 (51 → 34) | 지시대로 이행 | 1.81 에 닿으려면 **48개**가 필요했다 |
| ② 밀도 미달 (207 → 154, **−26%**) | 지시에 없었다 | 34개가 207 B였다면 비율 **1.29** |

억제를 보정해도 1.29이므로 **예측 1.81은 어느 쪽으로도 회복되지 않는다.**
그래도 「기각」이 아니라 「검정 불가」인 이유는 그대로다 — 개입(억제)과 관측이 섞였고,
②의 원인이 「51개를 추리느라 정의를 짧게 썼다」인지 「6,500 B 예산이 밀도를 눌렀다」인지
구분할 자료가 없다. **원본 쪽 밀도(107)는 코퍼스 101~116 안에 그대로 있다** —
부푸는 것은 원본이 아니라 집필 밀도라는 §3-1-b의 관찰은 유지된다.

다음 배치가 C-3을 다시 검정하려면 **억제 없이 전량 승계한 지도편**이 필요하다.
그리고 그때는 비율만이 아니라 **용어당 B를 함께 적어라** — 비율 하나로는 이 두 갈래가 안 갈린다.

**C-4는 이번 배치가 만든 가설이다**(§3-1-f). 근거가 **한 글 안의 두 점**뿐이라 이번 배치의
예산에는 반영하지 않았고, 마감에서도 절 단위 재측정은 하지 않았다 — **미검정으로 남긴다.**
다음 배치는 본편 6편에서 절별로 재라. `compose.mjs`가 원본 성분을 주고, 발행본은 H2로 잘라
`wc -c` 하면 된다. **맞으면 §2-4의 결론(「원본 성분으로 비율을 정하지 않는다」)이 절 단위에서는 뒤집힌다.**
...
원본 `06-CI-CD-GitHub-Actions-Jenkins.md` **83,252 B** (§14 실명 경력 등 두 조각 2,645 B 제외 → 배정 **80,607 B**, §1 각주)
→ 발행 7편 **169,345 B**. 겉보기 배치 비율 **2.101**.

바이트는 전부 **저장 기준(LF)** 이다. `core.autocrlf=true`라 디스크의 `wc -c`는 줄 수만큼 크게 나온다 —
`git cat-file -p HEAD:<경로>` 로 재라(§10-4 함정 ③).

| 편 | slug | 원본 B | 목표 B | 발행 B | 비율 | 밴드 판정 |
| :---: | --- | ---: | ---: | ---: | ---: | --- |
| 1 | `cicd-pipeline-fundamentals` | 7,175 | 20,561 | **20,867** | 2.91 | 고정비용 방식 (밴드 없음) |
| 2 | `version-control-as-cicd-premise` | 11,424 | 22,600 | **22,655** | 1.98 | 20,100 ~ 24,500 · 여유 1,845 |
| 3 | `branching-strategy-and-containers` | 10,262 | 21,474 | **21,505** | 2.10 | 17,600 ~ 21,500 · 🔴 **+5** |
| 4 | `github-actions-pipeline` | 11,418 | 21,985 | **22,040** | 1.93 | 18,000 ~ 22,000 · 🔴 **+40** |
| 5 | `jenkins-pipeline-and-agents` | 11,658 | 25,046 | **25,046** | 2.15 | 20,700 ~ 25,300 · 여유 254 |
| 6 | `zero-downtime-and-quality-gates` | 12,754 | 25,500 | **28,040** | 2.20 | 22,950 ~ 28,050 · 여유 **10** |
| 7 | `questions-that-come-back` | 15,916 | 28,000 | **29,192** | 1.83 | 26,300 ~ 29,200 · 여유 **8** |
| | **합계** | **80,607** | | **169,345** | **2.101** | |

밴드 상한은 검사기가 강제하지 않는다(`tests/blog/content/schema.test.ts` — 「상한은 검사하지 않는다」).
초과 2건은 게이트 실패가 아니라 **계획이 못 센 순증**이고, §10-2가 그 두 경로다.

**편1 성분 분해** (§10-0이 요구한 항목):

| 성분 | 예산 (§3-1-b) | **실측** | 차 |
| --- | ---: | ---: | ---: |
| frontmatter | 700 | **718** | +3% |
| 도입 | 1,500 | **1,917** | +28% |
| 본문 (H2 2개) | 3,500 | **8,423** | **+141%** |
| 용어 정리 | 6,500 | **5,233** | −19% |
| 시리즈 구성 + 정리 | 2,800 | **4,571** | +63% |
| **파일 전체** | **≈15,000** | **20,867** | **+39%** |

본문 +141%가 §3-1-f가 잡은 것이다 — 예산표에 **줄이 하나 빠져 있었다.** 그 정정으로
편1 목표가 14,300에서 8차례에 걸쳐 20,561까지 올라갔고, 그 재환산이 편2~편7 전체에 전파됐다.

**발행 시각** — 커밋 8건 전부 2026-08-19:
...
| 2 | `38aee11` | 6 | `6ee868b` |
| 3 | `92fe3c0` | 7 | `e2f5565` |
| 4 | `84dd06c` | 인바운드 링크 | `9074877` |

#### 카테고리 위상 — 나가는 길은 뚫렸고 **들어오는 길은 그대로다** `[§7-4-b가 §10에 남기라고 한 실측]`
...
| **나가는** 카테고리 간 링크 | **0** | **4** | 3 |
| **들어오는** | 1 | **1** | — |
| 편당 나가는 | 0.00 | **0.13** | 0.10 |

나가는 4건은 편1이 3건(`ai-transformation/sdlc-human-gates` · `ai-agent/agent-vs-workflow` ·
`search-engineering/elasticsearch-operations`), 편6이 1건(`agentic-coding/agentic-coding-qna-governance`)이다.
목표 3을 넘겼고 편당 밀도도 `search-engineering`(0.17) 바로 아래까지 올라왔다.

🔴 **그러나 들어오는 길은 1건 그대로고, 편수만 25 → 32로 늘었다.** 유일한 진입로는 여전히
`ai-agent/ai-agent-qna-operations.md` 하나다. 이 카테고리는 이제 **바깥으로는 말하는데
아무도 찾아오지 않는** 모양이다 — 배치 전(양방향 최약)과 종류가 다른 결함이다.

⚠️ **검사기는 이것을 잡지 않는다.** `links.test.ts`의 카테고리 고립 검사는 「한 방향이라도 **0**인가」를
본다. 비율 임계를 쓰지 않은 것은 의도된 설계다(배치마다 분모가 달라져 새 글마다 깨진다).
⇒ **1은 0이 아니므로 초록이다. 고립의 「정도」는 게이트가 없다.**
다음 배치는 `backend-engineering`으로 **들어오는** 링크를 만들어라 — 캐시·배치·실시간 주제는
`rag`·`ai-agent`·`high-traffic` 어느 쪽에서든 자연스럽게 걸린다.

재현 스크립트(§7-4-b가 자리만 잡아 뒀던 것):
...
const files=[];
for(const c of cats) for(const f of fs.readdirSync(path.join(root,c)).filter(x=>x.endsWith(".md")))
  files.push({cat:c, body:fs.readFileSync(path.join(root,c,f),"utf8")});
const outn={},inn={}; cats.forEach(c=>{outn[c]=0;inn[c]=0;});
for(const f of files){
  const seen=new Set();
  for(const m of f.body.matchAll(/\]\(\/blog\/([a-z-]+)\/([a-z0-9-]+)/g)){
    if(!cats.includes(m[1])||m[1]===f.cat) continue;
    const k=m[1]+"/"+m[2]; if(seen.has(k)) continue; seen.add(k);
    outn[f.cat]++; inn[m[1]]++;
  }
}
for(const c of cats) console.log(c.padEnd(22), "나감", outn[c], "· 들어옴", inn[c]);
'
```

⚠️ 같은 편이 같은 글을 두 번 링크하면 1건으로 센다(`seen`). 그것을 빼면 편1의 목차 표가
한 카테고리를 여러 번 가리킬 때 수치가 부푼다.

---

### 10-2. 🔴 순증이 예산 **밖에서** 두 번 들어왔다

이번 배치 최대의 발견이다. 예산은 「집필 목표 B」에만 걸려 있는데, 실제 바이트는 그 뒤로
**두 경로에서 더 들어온다.** 둘 다 계획서 어디에도 줄이 없었다.

```mermaid
flowchart TD
    P["§3-2 예산<br/>목표 B · ±10% 밴드"] --> W["집필 초안"]
    W --> R1["리뷰 축① 충실도<br/>순증 +a"]
    W --> R2["리뷰 축② 시리즈 정합<br/>순증 +b"]
    R1 --> M{"컨트롤러 반영"}
    R2 --> M
    M --> PUB["발행"]
    PUB --> CLOSE["배치 마감<br/>인바운드 링크 +55/편"]
    CLOSE --> FINAL["최종 바이트"]

    R1 -. "누출 ① 각 리뷰어는 자기 축만 센다" .-> M
    CLOSE -. "누출 ② 마지막 편이 나온 뒤 앞 편들이 다시 자란다" .-> FINAL

    style P fill:#e8f4ea
    style FINAL fill:#fde8e8
    style M fill:#fff4e0
    style CLOSE fill:#fff4e0
```

점선 둘이 예산이 못 보는 자리다. 실선만 따라가면 「목표 안에 들어왔다」가 되고,
점선을 더하면 편3·편4가 밴드를 넘는다.

#### ① 두 축 리뷰의 순증은 합산된다 — 편6이 상한을 587 B 넘겼다

리뷰를 위험 축별로 2명 이상 띄우는 것은 맹점을 깨기 위해서다. 대가가 이것이다 —
**각 리뷰어는 자기 축의 순증만 센다.** 편6에서 두 축을 다 반영했더니 순증 **1,334 B**가
나 상한을 587 B 넘겼고, 되풀이 여섯 자리를 찾아 깎아서야 들어왔다. 충실도 리뷰어가
「순증 500~600 B」라 추정했는데 그것은 자기 축만 센 값이었다.
...
**효과가 즉시 났다** — 편7은 밴드 상한에서 15 B를 남기고 시작해 **8 B로 착지**했다.

⚠️ **그래도 13 B가 어긋났다.** 예측 −10 B, 실제 +3 B. 차이는 리뷰어가 처방 표 **밖**에 적은
「~하면 함께 고쳐야 한다」는 **조건부 처방**이었다. 표에 없으니 합계에도 없다.
⇒ 다음 배치는 브리프에 한 줄 더 넣어라 — **「조건부 처방도 표 안에 넣어라. 조건이 성립할 때의 ±B를 적어라」.**

#### ② 마감 단계 인바운드 링크는 어느 편의 예산에도 없다 — 편4가 상한을 40 B 넘겼다

§7-10-k는 편1~편6 → 편7 링크를 **배치 마감의 별도 커밋**으로 미뤘다. 편7이 없는데 링크를
걸 수 없으니 판단 자체는 옳다. 문제는 **그 +55 B/편이 예산에 없었다**는 것이다.

| 편 | 마감 순증 | 내역 | 결과 |
| :---: | ---: | --- | --- |
| 1 | **+114** | 목차 표 1행 + 예고 문단 1자리 | 밴드 없음 |
| 2 | +55 | 「편7이」 → 링크 | 여유 1,845 → 흡수 |
| 3 | +31 | §9-4 시점 표기 (링크 아님) | 상한 **+5 초과** |
| 4 | +55 | 「선택은 편7이 받는다」 → 링크 | 상한 **+40 초과** — 마감 전 여유는 15였다 |
| 6 | +55 | 「다음 편은」 → 링크 | 여유 65 → **10** |
| | **+310** | | |

전부 **이미 있는 낱말에 마크업만 씌운 것**이라 산문은 한 자도 늘지 않았다. 그런데도 40 B가 넘는다.
⇒ **밴드는 편 안에서 닫히지 않는다.** 마지막 편이 발행되면 앞 편들이 다시 자란다.
**다음 배치는 본편 예산에 인바운드 링크 몫 60 B/편을 미리 잡아라.** 지도편은 목차 표가 있어 120 B다.

편3의 +31은 성격이 다르다 — `openjdk:21-jdk`에 「2026년 7월 기준」을 붙였다.
**시점 없는 버전 값은 시간이 지나면 틀려지고, 밴드는 원본을 나누기 위한 계획 장치일 뿐이다.**
발행된 편에서 군더더기를 찾아 5 B를 만드는 대신 넘기는 쪽을 골랐다. 같은 상황이 오면 같게 판단하라.

---

### 10-3. 겹친 자리는 배타가 아니라 **각자 반쪽**일 수 있다

§7-9-m ③이 세운 규칙은 「두 리뷰어가 같은 줄에 다른 문안을 내면 하나를 고른다」였다.
편7 `88`에서 깨졌다(상세는 §7-10-l).

| | 지적한 것 | 없으면 잃는 것 |
| --- | --- | --- |
| 충실도 축 | **사실** — Jenkins 원격 트리거의 403은 방화벽이 아니라 CSRF 보호다 | 독자가 방화벽을 뒤진다 |
| 시리즈 축 | **귀속** — 편5가 이름만 적고 넘긴 자리다 | 편5와의 이음매가 끊긴다 |

어느 하나를 고르면 나머지 절반이 사라진다. ⇒ **요구를 표로 갈라 판정하고, 갈라지면 셋째 문안을 세운다.**
통합이 +10 B로 따로 적용(+20)의 **절반**이었다 — **겹침은 손실이 아니라 압축 기회다.**

같은 편에서 **처방이 이음매를 만든다**는 것도 새로 봤다. 편6에서 배운 「처방이 되풀이를 만든다」의
이웃 현상이다 — 표 `86`이 3걸음인데 산문 `114`가 「네 걸음」이 됐다. 둘 다 원본에 근거가 있어
사실 오류는 아니나 **한 편 안에서 수가 둘**이다. 「걸음 나눔」(+3 B)으로 세는 것 자체를 없앴다.
⇒ **처방을 반영한 뒤에는 그 절만이 아니라 그 편 전체에서 수·이름·순서가 갈리지 않는지 훑어라.**

---

### 10-4. 검사기 넷을 세웠다 — 전부 자기 증명이 먼저다

| 검사기 | 잡는 것 | 자기 증명 | 마감에서 실제로 잡은 것 |
| --- | --- | ---: | --- |
| `check-counts` | README 3자리 + CHANGELOG 편수가 실제와 어긋남 | 14/14 | 🔴 **4건 즉시 검출** |
| 앵커 실존 (`links.test.ts`) | 절이 사라져 닿지 않는 `#앵커` | 합성 9건 | 0 (파괴 실험으로 증명) |
| 카테고리 고립 (`links.test.ts`) | 편이 아니라 **카테고리**가 떨어지는 것 | — | 0 |
| `compose.mjs` 도식·코드 분리 | 「코드%」가 실은 「도식%」였던 앞 배치 결함 | — | — |

`check-counts`가 바로 값을 했다. **편수는 틀려도 빌드가 깨지지 않아 조용히 낡는 자리**다.
앵커 검사기는 합성 자기시험만으로는 부족해 **발행본의 실제 앵커를 깨뜨려** 실패
(「닿지 않는 앵커 1건 / 검사 28건」)를 확인한 뒤 되돌렸다 — 합성 fixture는 실제 문서 구조를
닮지 않아서, 통과해도 실물에서 0건을 낼 수 있다.

#### 이번 배치에서 새로 당한 도구 함정

| # | 함정 | 어떻게 드러났나 | 대응 |
| :---: | --- | --- | --- |
| ① | `rehype-slug`는 **H1~H6 전부**에 id를 붙인다 | `buildToc`(H2·H3)으로 앵커를 검사하면 H4 링크가 거짓 양성 | `headingIds`를 따로 내보내고 `scanHeadings`를 공유 |
| ② | `github-slugger`는 **상태를 든다** (중복 제목에 `-1`) | 깊이로 거른 뒤 슬러그하면 번호가 페이지와 어긋난다 | 문서 순서대로 한 번에 돌린다 (자기시험 ⑤가 고정) |
| ③ | `core.autocrlf=true` + `git checkout --` | 되돌린 파일이 CRLF로 재체크아웃돼 `wc -c`가 줄 수만큼 커진다 | `git cat-file -p HEAD:<경로>` 로 잰다 |
| ④ | 히어독이 백슬래시를 **한 겹 벗긴다** | 정규식의 이스케이프가 실제 CR/LF 문자가 돼 치환이 조용히 실패 | 정규식이 든 파일은 `Write`/`Edit` 도구로 |
| ⑤ | 긴 히어독은 **셸을 죽인다** | 집필자·리뷰어·컨트롤러가 각각 `unexpected EOF`를 맞았다 | 처음부터 나눠 쓰거나 `Write` 도구로 |
| ⑥ | `dup-scan` 은 **자문 도구**라 발견 유무와 무관하게 exit 0 | verifier 브리프에 「발견 0」을 기준으로 적어 어긋남이 보고됐다 | 종료 코드가 아니라 **건수를 사람이 판정**한다고 브리프에 적어라 |
...
| # | | 근거 |
| :---: | --- | --- |
| 1 | 본편 예산에 **인바운드 링크 60 B/편**(지도편 120)을 미리 넣어라 | §10-2 ② |
| 2 | 리뷰 브리프에 **「조건부 처방도 표 안에, ±B와 함께」** 를 더해라 | §10-2 ① |
| 3 | 처방 반영 뒤 **그 편 전체**에서 수·이름·순서를 훑어라 | §10-3 |
| 4 | 두 리뷰어가 같은 줄에 서면 **배타로 보지 말고 표로 갈라라** | §10-3 |
| 5 | Q&A편은 **항목 수 × 항목당 B**. 비율을 갖다 대지 마라 | §10-0 C-2 (2회 재현) |
| 6 | 본편 비율은 **값이 아니라 밴드로** 물려라. 2.10은 여유 2%다 | §10-0 C-1 |
| 7 | 지도편은 비율만이 아니라 **용어당 B를 함께 적어라** | §10-0 C-3 |
| 8 | C-4(절 단위 성분)는 **미검정**이다. 본편 6편에서 절별로 재라 | §10-0 C-4 |
| 9 | 검사기의 **자기 증명을 먼저 돌려라.** 증명 없는 0건은 거짓 0과 구분되지 않는다 | §10-4 |
| 10 | 🔴 `backend-engineering`으로 **들어오는** 링크를 만들어라. 32편에 1건이고 검사기는 안 잡는다 | §10-1 |
