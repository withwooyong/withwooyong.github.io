...
**T5 를 시작하기 전에 반드시 읽을 것:** §「실측 기록」의 「이 표에서 읽어야 할 것 — T5 의 입력」과 그 아래 「T5 로 넘긴 것」. 조사 처리와 404 인덱싱이 팔레트 설계를 바꾼다.

### 지금 초록이 아닌 것 — 전부 의도된 것이다

| 검사 | 상태 | 언제 초록이 되나 |
| --- | --- | --- |
| `npm run e2e` | **2 failed** (16 passed · 16 skipped) | `/atlas/` 셸 센티넬 2건(desktop·mobile). **T11** 에서 셸이 붙으면 초록이 되고 skip 16건이 동시에 깨어난다 |
| `shell.spec.ts` 의 asPath 검사 | 위 16건에 포함돼 skip | **T12** 에서 `/atlas/` 를 NAV 에 올려야 한다. T11~T12 사이 한 구간에서는 **빨간 것이 정상**이며, 그 빨강은 「셸은 붙였는데 NAV 에 안 올렸다」는 뜻이다 |
