...
| `extends` 엣지 (본문 링크) | 추정 미상 | **T2에서 실측** |
| `sequence` 엣지 (series) | 미상 | **T2에서 실측** |

⚠️ **스펙의 「엣지 1,000 안팎」을 믿지 마라.** 사전 조사에서 본문 내부 링크가 `](/blog/` 기준 **1,100건**(154개 파일)으로 나왔다. 스펙은 이 값을 세지 않고 추정했다. 고유 `(from, to)` 쌍이 몇 개인지는 **T2에서 실제로 세고**, 그 수치로 렌더러 임계(§7.5의 Dot ≤300 · Canvas ≤2,000)를 판단한다.
