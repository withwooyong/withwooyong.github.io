...
| GC-11 | 발행 산출물 금칙어 | **목록의 정본은 `scripts/check-forbidden.mjs` 다. 문서에 복사하지 마라.** 2026-08-18 실측: 목록이 9개 문서로 갈라져 있었고 그중 어디에도 한글 표기형(「패스트캠퍼스」·「테디노트」)이 없어 검사가 **거짓 0건**을 냈다. 그 0건이 CHANGELOG에 사실로 기록됐다. ⇒ 발행 전 `npm run check-forbidden` 이 **HARD 0건**이어야 한다. 돌리기 전에 `npm run check-forbidden:verify` 로 **검사기가 실제로 잡는지 먼저 증명하라** — 증명 없는 0건은 거짓 음성과 구분되지 않는다 |
