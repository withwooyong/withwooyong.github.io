...
2026-08-18 전수 분류 결과를 **판정** 열에 적었다(`docs/superpowers/reports/2026-08-18-rule-triage.md`).
「자동」은 **검사기가 정본**이다 — 판정 기준을 이 표에 다시 적지 마라(금지선 54).
「사람」 7건은 **코드 제약**이라 발행 체크리스트가 아니라 `CLAUDE.md` 소관이다.

| # | 제약 | 판정 | 정확한 값 |
| --- | --- | :---: | --- |
| GC-1 | 정적 export만으로 동작 | **자동** | `npm run build` 가 강제한다. API 라우트·ISR·서버 액션·`next/image` 로더를 넣으면 빌드가 깨진다 |
| GC-2 | 모든 경로는 슬래시로 끝난다 | **자동** | 빌드 + 신설 링크 검사가 정본. 내부 링크는 `/blog/search-engineering/es-architecture/` 형태 |
| GC-3 | App Router 금지 | 사람 | ⚠️ **빌드가 막지 않는다.** `app/` 디렉터리 컨벤션을 도입하지 않는다. Pages Router 유지 |
| GC-4 | 경로 별칭 | 사람 | ⚠️ **상대경로도 빌드를 통과한다.** `@/*` → 저장소 루트. `@/lib/...`, `@/components/...` |
| GC-5 | `fs` 사용 위치 | **자동** | `getStaticProps`/`getStaticPaths`에서 호출되는 모듈에서만. 클라이언트 번들에 들어가면 빌드가 깨진다 |
| GC-6 | 기존 페이지 무영향 | **자동** (신설 예정) | `scripts/check-baseline.mjs` (CI 전용)가 정본이 된다. 그때까지는 수동. `pages/index.tsx`, `pages/en/`, `pages/product-lead*` 산출물 |
| GC-7 | 한글 줄바꿈 | 사람 | 본문 텍스트 요소에 `break-keep` 적용 |
| GC-8 | 다크모드 | 사람 | `<html class="dark">` 토글 방식. 모든 신규 컴포넌트에 `dark:` 변형 제공 |
| GC-9 | 커밋 메시지 | 사람 | 한글로 작성 |
| GC-10 | 푸시 금지 | 사람 | 사용자가 명시적으로 요청하기 전까지 `git push`를 실행하지 않는다 |
| GC-11 | 발행 산출물 금칙어 | **자동** | **목록의 정본은 `scripts/check-forbidden.mjs` 다. 문서에 복사하지 마라.** 발행 전 `npm run check-forbidden` 이 **HARD 0건**이어야 한다. 돌리기 전에 `npm run check-forbidden:verify` 로 **검사기가 실제로 잡는지 먼저 증명하라** — 증명 없는 0건은 거짓 음성과 구분되지 않는다(금지선 53) |
| GC-12 | **타입 검사는 별도 검증이다** | **자동** | 코드를 만지는 모든 태스크는 `npx tsc --noEmit`을 실행하고 종료 코드를 보고한다. **Vitest는 esbuild로 타입을 지울 뿐 검사하지 않는다** — 테스트 전원 통과 상태에서 `tsc`가 오류를 잡는 일이 실제로 발생했다 |
| GC-13 | `tsconfig.json` 수정 금지 | 사람 | ⚠️ **기계가 막지 않는다.** `target: "es5"` 등을 바꾸면 전 프로젝트 컴파일 산출물이 달라져 GC-6에 저촉된다. 타입 오류는 호출부를 고쳐 해결한다 |

> **GC-11의 유래** — 2026-08-18 실측: 금칙어 목록이 **9개 문서로 갈라져** 있었고 그중 어디에도
> 한글 표기형(「패스트캠퍼스」·「테디노트」)이 없어 검사가 **거짓 0건**을 냈다. 그 0건이 CHANGELOG에
> 사실로 기록됐다. 이 사고가 **「목록의 정본은 코드다」**(금지선 54)의 근거이며,
> 2026-08-18 규칙 전수 분류의 출발점이다.
