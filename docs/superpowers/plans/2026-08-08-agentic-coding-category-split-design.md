...
> ⚠️ **정정 (2026-08-13, B9 실측)** — `[실측]` 절별로 세면 **최소 69곳**이다(§1 도식 10 · §2 표 10 · §3 18~20 · §4 31 · §6 1).
> **52는 §1 도식과 §2 표를 세지 않은 값**으로 보인다. 그 누락이 지도편의 범위를 **§3·§4 옮기기**로 좁혔는데,
> 실제로는 **도식과 문서별 요약표도 전부 재작성 대상**이었다. **상세는 §9 B9 완료 블록.**
...
> ## ✅ 완료 (2026-08-13, B9 실측) — **카테고리가 닫혔다. 31편으로 §10 검산값과 일치**
>
> | 항목 | 값 |
> | --- | --- |
> | 발행 | **1편** — `topic-map-reading-paths` (25,311 B · H2 6 · 표 13) |
> | series | **없음**(독립편) — §10 「독립편 3 = 편1·편26·지식 지도」 충족 |
> | 커밋 | **`89beda4`** |
> | `agentic-coding` | 30 → **31편** · 전체 **113편** · sitemap **174 URL** |
>
> ### ⚠️ 정정 1 — 「참조 52곳」은 검산되지 않는다 (§3 L206 · §9 이 표)
>
> `[실측]` 원본 `00`의 포인터를 절별로 세면 **§1 도식 10 + §2 표 10 + §3 18~20 + §4 31 + §6 1 = 최소 69곳**이다.
> **52에 가까운 것은 §3+§4+§6(50~52)뿐** — 즉 **§1 도식과 §2 표를 세지 않은 값**으로 보인다.
>
> **숫자보다 중요한 것은 그 숫자가 범위를 좁혔다는 점이다.** 「52곳」은 지도편을 **§3·§4를 옮기는 일**로
> 규정하는데, 실제로는 **§1 도식과 §2 문서별 요약표도 전부 다시 만들어야 했다.**
>
> ### ⚠️ 정정 2 — B9는 번역이 아니라 **재작성**이다. §4-0의 「~15KB」는 원본 기준으로 불가능하다
>
> `[실측]` 원본 `00`은 129줄·6.7KB이고 §5·§6은 발행 제외(§7-5)라 **가용분이 약 5KB**다.
> 남은 §0~§4에서도 그대로 옮길 수 있는 셀이 사실상 없었다:
>
> | 원본 자리 | 왜 못 쓰나 |
> | --- | --- |
> | §2 「원본 범위 `Part N`」 열 | 금칙어 `Part [0-9]` |
> | §2 「분량 570줄」 열 | **원본 파일의 줄 수.** 발행본은 30편으로 재편돼 대응하지 않는다 |
> | §2 「면접 우선순위」 열 · §3 「면접 직전」 행 · §1 노드 `10` | 금칙어 |
> | §1 노드 `06` · §2 `06` 행 · §3 자동화 행 · §4 2칸 | **`06` 참조 5자리** (금지선 8) |
> | §0 용어표 | 사용자 결정 — 기발행 30편이 이미 각자 용어표를 갖는다(축 E 위험) |
>
> **최종 25,311 B는 원본 팽창이 아니라 발행본 30편에서 새로 만든 결과다.** 배율 지표는 이 배치에 적용되지 않는다.
>
> ### ⚠️ 정정 3 — **`00 §4`는 깊이 3단 표를 이미 갖고 있다.** §5-5가 이것을 명시하지 않았다
>
> `[실측]` 원본 `00` L86: *"같은 주제가 여러 문서에 걸쳐 있다. **깊이 순으로 정리한다.**"* · L88 표 헤더: `| 주제 | 1차 (가장 깊음) | 2차 | 3차 |`
>
> §5-5는 「`00 §4`를 기준표로 쓰되 3차 열을 믿지 마라」까지만 적었고, **깊이 3단이라는 구조 자체가 원본 것**이라는 점은 적지 않았다.
> 그 결과 **1차 반영이 「깊이 순서는 이 글의 판정이다」라고 쓸 뻔했다** — 반영자가 원본을 다시 열어 잡았다.
>
> **최종 귀속**: 「주제 열다섯 중 **열하나와 깊이 3단은 원 자료의 교차 참조표에서 받았고**, 나머지 넷을 세우고 서른 편을 배정한 것이 이 글의 판정이다」
>
> ### ⚠️ 정정 4 — 「frontmatter 11필드 전부 필수」는 사실이 아니다. **`series: ""`는 빌드를 죽인다**
>
> `[실측]` `lib/blog/frontmatter.ts:77` — `series`·`seriesOrder`·`updated`·`source`는 **선택 필드**이고,
> 값이 있으면 `str()`이 **빈 문자열을 거부**한다. 독립편은 **필드 자체를 생략**해야 한다(기발행 편1·편26과 동일).
>
> **B1~B8은 전부 `series`가 있는 편이라 부딪히지 않았고 B9가 처음 걸렸다.** 브리프가 이 오류를 그대로 실어 1차 빌드가 실패했다.
>
> ### 실행 결과
>
> | 단계 | 산출 |
> | --- | --- |
> | 변환 | 24,162 B |
> | 5축 독립 검증 | **32건** (A 8 · B 6 · C 8 · D **0** · E 7 · 컨트롤러 3) → 반영 18 · 부분 2 · **오탐 2** · 미반영 8 |
> | 1차 반영 | **+1,275 B** · 새 문장 **17자리** · 반박 0 |
> | 재검증 2기 | 1기 **해소 19 · 미해소 0** / 2기 **신규 9건** (전칭 71개 추출 → 결함 6, **8.5%**) |
> | 2차 반영 | **−144 B** · 새 문장 **0자리** · **구조 완전 불변** |
> | 최종 독립 검증 | 통독 신규 4건 → **2건 반영 · 2건 미반영** · 축자 복제 산문·표 셀 **0건** |
>
> **세 축(A·B·E)이 서로 모른 채 주제 10에 독립적으로 도달했다.** 축을 나눈 설계가 작동한 가장 강한 신호다.
...
> ⚠️ **2026-08-18 전수 분류 — 이 표의 9개(2~9·12)는 사장됐다.**
> 배치 스코프 지시였고 그 배치가 끝났다. **취소선으로 남긴다 — 다른 문서가 번호로 참조한다**(금지선 55).
> 살아 있는 규칙의 이관처는 아래 표의 「근거」 열에 적었다.
> 판정 근거는 `docs/superpowers/reports/2026-08-18-rule-triage.md`,
> 사람 판단 항목은 `docs/superpowers/PUBLISHING-CHECKLIST.md`.

| # | 금지선 | 근거 |
| --- | --- | --- |
| 1 | **"중복이다"라는 인상으로 도식·표를 지우지 마라. 항목 수를 세라. 세어 보고 이 설계서를 어겨도 된다** | 조사 6기 전원이 전제를 뒤집었고, 검증 2기가 설계서 오류 29건을 잡았다 · **2026-08-18 사람** — 체크리스트 §5 |
| ~~2~~ | ~~**원본 내부 중복 1건은 제거한다** — `08 §3-4` tools 열의 `"기본"` 25행(**실측 ~150 B**)은 바로 위 3행 요약표가 이미 말한다(편20)~~ | **2026-08-18 사장** — 커밋 `06166e9`(B6)가 `aiagent/08`을 3편으로 발행했다. 규칙 본문이 스스로 「B6에서 실행 완료」라 적었다 |
| ~~3~~ | ~~`200줄`·`85%`는 발행본과 **다른 객체**를 가리킨다~~ | **2026-08-18 사장** — `aiagent/` 11파일 전부 소진. 두 수치에 대한 1회성 판정이었다 |
| ~~4~~ | ~~**`강의`를 다른 주체로 갈아 끼우지 마라.** `Anthropic 공식 문서` 등으로 바꾸면 원본이 하지 않은 검증 불가능한 주장이 생긴다~~ | **2026-08-18 사장** — `aiagent/` 소진. **원리는 `CR-1.10`이 영구 승계한다** |
| ~~5~~ | ~~**㉢ 귀속 구분 3건은 부정·정정 관계를 그대로 보존하라**~~ | **2026-08-18 사장** — `aiagent/` 소진. §6-4 ㉢의 특정 3건이었다 |
| ~~6~~ | ~~**면접·인용 절을 걷을 때 §6-5의 11건을 먼저 지정된 편으로 옮겨라.** 그중 3건은 출처 대장이다~~ | **2026-08-18 사장** — 커밋 `76a50bf`(B8)가 `aiagent/10-면접-커닝페이퍼`를 Q&A 4편으로 발행했다 |
| ~~7~~ | ~~`07 §7-4`의 4개 패턴은 **이름과 예방책만.** 절대 수치는 §7-2와 함께 버린다~~ | **2026-08-18 사장** — 커밋 `2fc5c9f`(B5)가 `aiagent/07`을 4편으로 발행했다 |
| ~~8~~ | ~~**`06`과 `08 §4~§8·§10`은 이번 범위가 아니다.** 손대지 마라~~ | **2026-08-18 사장** — 커밋 `9c0b9fd`(T1)가 `aiagent/06`을 3편으로, `6a56f4a`(T2)가 `08 §4~§8·§10`을 편9로 발행했다 |
| ~~9~~ | ~~`01`은 §6-1·§9·§10만 쓴다. **§1~§8을 되살리지 마라**~~ | **2026-08-18 사장** — 커밋 `388519e`(B1)가 `aiagent/01`·`02`를 5편으로 발행했다 |
| 10 | **통제 어휘 밖의 태그를 쓰지 마라** | **2026-08-18 자동** — `tests/blog/frontmatter.test.ts` 가 정본. **목록을 문서에 복사하지 마라**(금지선 54) |
| 11 | 발행 실측이 분량 하한을 밑돌면 **인접 편과 병합**하고 보고하라. 억지로 늘리지 마라 | **2026-08-18 자동(SOFT)** — `CR-4.1a`와 같은 규칙. 임계값은 `tests/blog/content/schema.test.ts` 가 정본 |
| ~~12~~ | ~~`02 §12-2`는 **1행이 편4, 5행이 편5**다. 「앞 3행/뒤 3행」이 아니다~~ | **2026-08-18 사장** — 커밋 `388519e`(B1)가 `aiagent/02`를 발행했다 |
...
### 11-0. 살아남은 규칙의 이관처 (2026-08-18)

**아래 9개는 영구 원칙이다. 이 표가 아니라 발행 체크리스트를 보라** — 규칙이 두 자리에 살면 갈라진다(금지선 54).

| # | 이관처 |
| ---: | --- |
| 1 | `PUBLISHING-CHECKLIST.md` §5 도식 근거 (`CR-1.6`·`CR-6.1`·`CR-6.3`과 한 항목으로 합침) |
| 13 | §7 톤·분량 — 「상한은 합격선이 아니다」 |
| 14 · 15 | §1 귀속 |
| 16 · 17 | §2 전수 배정 — **검사기로 만들지 않기로 했다**(설계서 §3-4) |
| 18 · 19 | §1 귀속 (좌표·근거) |
| 20 | §8 프로세스 |
