...
> ⚠️ **2026-08-18 전수 분류 — 이 표의 5개(22~25·27)는 사장됐다.**
...
>
> **26은 사장이 아니다** — 원본은 소진됐으나 **발행본 9편이 지금도 그 문구를 쓰고 있어** 서식 충돌이 남는다.
> 설계서 §3-1의 표본 추정(22~27 전부 배치전용)을 실측으로 정정한 건이다.

| # | 금지선 | 근거 |
| ---: | --- | --- |
| **21** | **실명·1인칭을 지운 자리에 새 주장을 세우지 마라.** 「나의 현재 위치」 열을 지우면 **열을 지우는 것으로 끝난다.** 빈 칸을 메우려 일반화하면 원문에 없는 주장이 생긴다 | §6-1 ② |
| ~~**22**~~ | ~~**`04_영역별` 7파일을 같은 규칙으로 처리하지 마라.** 「입사 후 실천」 절은 **5파일에만 있다**~~ | **2026-08-18 사장** — 커밋 `37e413b`(T4)가 `aiteam/03`·`04_영역별`을 편2·편3으로 발행했다. 원문의 「7파일」은 실측과 일치했다 |
| ~~**23**~~ | ~~**30에이전트 세트 C를 낼 때 A·B와 다르다는 것을 명시하라.** 선례는 `thirty-agent-catalog.md` L150~166~~ | **2026-08-18 사장** — 커밋 `6a56f4a`(T2)가 편9를 발행했다 |
| ~~**24**~~ | ~~**30종 명부를 다시 싣지 마라.** `thirty-agent-catalog.md`로 링크한다. 편9의 고유값은 **N-1 위임 흐름 도식과 N-3 프롬프트 기법**이다~~ | **2026-08-18 사장** — 커밋 `6a56f4a`(T2)가 편9(`agent-definition-by-department.md`)를 발행했다. **중복 방지 일반형은 `CR-6.1`·`dup-scan.mjs`가 승계한다** |
| ~~**25**~~ | ~~**`06 §2-7`의 H3 제목과 리드를 그대로 옮기지 마라.** 원본이 **4등급 표를 「3등급」이라 부른다.** 제목·리드·mermaid 노드 수·표 행 수가 **전부 4로 일치**하는지 확인하라~~ | **2026-08-18 사장** — 커밋 `695cdad`(T3)가 `aiteam/05`·`06`을 편4로 발행했다. ⚠️ 여기의 `06`은 `aiteam/06`이고 금지선 8의 `06`은 `aiagent/06`이다 — **같은 번호가 다른 파일이다** |
| **26** | **「첫 90일에 할 일」을 편 제목·H2로 쓰지 마라.** ⚠️ **2026-08-18 실측 정정** — 「발행본 7편의 마무리 **절** 서식」이 아니라 **발행본 9편의 마무리 표 열 제목**이다(H2로 쓰인 곳 0건) | §7-2 · **2026-08-18 사람** — 원본은 소진됐으나 발행본 서식 충돌이 남아 유효하다. 체크리스트 §7 |
| ~~**27**~~ | ~~**`aiteam/06`의 성숙도 3단계·90일 로드맵은 자체 정의다.** 외부 표준처럼 쓰지 말고 **「이 글의 정리다」 귀속을 붙여라**~~ | **2026-08-18 사장** — 커밋 `695cdad`(T3) 발행분 `enablement-infra-and-maturity.md`가 「이 글의 정리다」를 `:87`·`:103`·`:138`·`:346`·`:359`에 실제로 붙였다. **원리는 금지선 14가 승계한다** |
...
| # | 이관처 |
| ---: | --- |
| 21 | `PUBLISHING-CHECKLIST.md` §4 익명화 |
| 26 | §7 톤·분량 (**사장 아님** — 위 정정 참조) |
| 28 · 30 | §5 도식 근거 |
| 29 | §2 전수 배정 — **검사기로 만들지 않기로 했다**(설계서 §3-4) |
| 31 | §3 삭제 파급 |
| 32 | §1 귀속 |
| 33 | §8 프로세스 (금지선 53의 선행형) |
| 34 | §4 익명화 |
