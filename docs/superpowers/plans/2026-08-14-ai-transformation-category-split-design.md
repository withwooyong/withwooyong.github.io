...
**진행**: T1(편6·7·8) `9c0b9fd` · T2(편9) `6a56f4a` · **T3(편1·편4) `695cdad`** → **현재 119편 · sitemap 181 URL · 6/11 발행**
...
| 팽창률 | ⚠️ **1.20은 반증됐다. 실측 1.63~2.71** — T1 편6 1.63 · 편7 1.80 · 편8 1.93 · T2 편9 2.50 · **T3 편1 1.72 · 편4 2.71**. (`agentic-coding` 승계값 1.20은 그 카테고리에서도 실측 1.76이었다)<br/>⚠️ **T3에서 규칙성이 드러났다 — 쪼개는 편은 낮고(1.72) 묶는 편은 높다(2.71).** 편4는 13.5 KB 불릿·표 뭉치에 도식 4개와 층위 설명이 붙어 세 배 가까이 늘었다 |
...
| **T4** | 편2·편3 (`04_영역별`) | 묶기 작업의 본체 · **▶ 다음.** ⚠️ **금지선 22가 여기서 걸린다 — 「입사 후 실천」 절이 7파일 중 5개에만 있다** |
...
| 항목 | 기대값 |
| --- | ---: |
| 발행 편수 | **113 + 11 = 124편** — ⚠️ **T3까지 실측 119편**(6/11 발행, 5편 남음) |
| sitemap URL | ~~174 + 11 + 신규 태그 5 = **190**~~ → ⚠️ **실측 181.** **신규 태그가 하나도 열리지 않았다** — 편1~편4·편6~편9의 태그 24개가 전부 기존 사용분이라 **URL 증가분이 편수 증가분과 정확히 같다.** 남은 5편도 같으면 최종 **186**이다 |
| `npm test` | 36개 통과 |
| `npx tsc --noEmit` | 0 |
| `npm run build` | 0 |
...
### ⚠️ 오탐 트랩 — **7종**이다 (T1 3 · T2 1 · **T3 2** · 어휘 1)
...
>
> ⚠️⚠️ **배제 문자류를 계속 늘리면 언젠가 진짜를 놓친다.** T3 시점에 `[^주전공과문]`까지 왔다.
> **다음 트랩이 나오면 정규식에 더하지 말고 「알려진 오탐」 목록으로 처리하라.**
...
| 2 | **`> 🖊️ TODO(허우용):` 블록은 별도로 세라** — T3 범위에 2건(`01 L51`·`02 L13`), 전체 원본에 9건. **실명·회사명·면접 맥락이 한 덩어리로 들어 있다** |
