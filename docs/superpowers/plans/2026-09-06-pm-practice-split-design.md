...
| 편3 | ✅ [`mydata-and-market-structure`](../../../content/blog/product-management/mydata-and-market-structure.md) — **164행**(문제를 찾는 채널 여섯 갈래) · **166행**(AARRR 퍼널과 북극성 지표) | 그쪽이 채널로 나눈 것을 편3 이 방법의 층위로 푼다. 규칙 10 대로 낱말 실재를 grep 으로 확인하고 행 번호를 적었다 |
| 편6 | ✅ [`product-org-and-strategy`](../../../content/blog/product-management/product-org-and-strategy.md) — **20행**(네 개의 직함을 두 축으로 가른다) · **68행**(조직을 전문성으로 나눌 때와 목표로 나눌 때) | 🔴 **낱말이 달랐다.** 그쪽은 「직책」·「직급」·「조직문화」를 **0건** 쓰고 「**직함**」을 쓴다(2·14·18·20·22·24·31·33·149행). 그쪽이 「회사가 무엇이라 부르는가」를 물었다면 편6 은 「그 이름이 무엇의 이름인가」를 묻는다 |
| 편7 | ✅ [`planning-and-product-ops`](../../../content/blog/product-management/planning-and-product-ops.md) — **71행**(요구사항이 들어와 종료로 나가기까지) · **86 ~ 96행**(아홉 단계 표) · **98행**(분석을 두 번 하도록 못 박은 이유) | 🔴 **초판이 지목한 자리가 없었다.** 도메인 8편에서 「글로벌」이 두 편에 각 1회뿐이라 접합면이 성립하지 않았다. 대신 같은 절차 목록을 그쪽은 **순서**로, 편7 은 **관문**으로 다루는 대비를 접합면으로 삼았다 (§10-7-d) |
| 편8 | ✅ [`tactics-and-goal-setting`](../../../content/blog/product-management/tactics-and-goal-setting.md) — **18행**(발표를 걷어내면 논리만 남는다) · **20행**(서술형 문서를 먼저 읽고 회의를 시작한다) · **35행**(과업을 건네는 회의와 문제를 건네는 회의) | 🔴 **칸이 비어 있었고 찾으니 자리가 있었다.** 도메인 8편에서 「커뮤니케이션」과 「소통」이 **0건**이지만, 같은 주제를 다른 낱말로 다룬 자리가 있었다. 그쪽은 **한 번의 승인 자리**에서, 편8 은 **매주 반복되는 자리**에서 형식을 바꾼다 (§10-8-e) |

🔴 **접합부를 확정하기 전에 낱말이 실재하는지 `grep` 으로 확인하고 행 번호를 적어라** — 규칙 10
이다. 앞 배치는 §7-2 에서 이것을 빠뜨려 **원본에 0건인 낱말을 근거로 자리를 지목했다.**
위 표의 「기존 시리즈」 칸이 편 이름이 아니었던 것은 그 확인을 하지 않았기 때문이다. **편3 · 편6 · 편7 · 편8 을 채워 목표 4건을 넘겼고 편2 만 남았다** — 편2 는 이미 발행됐으므로 다음 배치가 아니라 그 편을 손볼 때 함께 본다.
🔴 **편7 에서 드러난 것은 더 강하다 — 칸이 편 이름이 아니면 채우는 일이 아니라 찾는 일이다** (§10-7-d).

🔴 **편6 에서 이 경고가 실제로 발동했다.** 초판이 적어 둔 접합 근거(「조직 구조와 제품 전략의
접점」)는 맞았으나 **지목한 낱말이 그 편에 0건**이었다 — 그쪽은 「직함」을 쓰고 「직책」을 쓰지
않는다. ⇒ **낱말 실재 확인은 접합 근거가 그럴듯한지와 별개다.** 근거가 옳아도 낱말이 없으면
독자가 건너간 곳에서 그 낱말을 찾지 못한다. 155행짜리 파일에서 `grep` 이 0을 낸 것이
검사기의 고장이 아님을 절 제목 목록으로 대조한 뒤에 판정했다.
...
| 원본 | 자리 | 왜 뒤집히나 |
| --- | --- | --- |
| `12` §6 | ✅ **262행**(머리 행) · **264 ~ 267행**(네 축) — 국내 은행 / 인터넷전문은행 / 핀테크 스타트업 **3분 비교** | 세 열을 합쳐 「국내 조직문화는 이렇다」로 쓰면, 표가 말하는 **차이**가 사라지고 정반대의 서술이 된다. 편6 실측 — 「갈등 없음」(은행)과 「갈등 매우 높음」(스타트업)이 서로를 상쇄해 **세 열 어디에도 없는 조직**이 만들어진다 |
| `11` §5 | 가상 프로젝트의 **단계별 판단** | 단계를 합치면 「데이터를 보고 정했다」만 남고 **어느 단계에서 무엇을 보았는지**가 사라진다 |
| `10` §3 | ✅ **105행**(절 제목) · **111 ~ 113행**(세 국면의 순서) · **127행**(관문) · **129행**(일곱 단계) — 「하면 된다」가 아닌 **「되면 한다」** | 검토와 추진을 한 층으로 합치면 원본이 뒤집으려던 순서가 원래대로 돌아간다. 편7 실측 — 세 국면(세울 때 · 굴릴 때 · 절차 자체) 가운데 하나만 떼어 읽으면 「일단 만들고 본다」나 「계획을 다 세운 뒤 착수한다」가 되어 **국면을 나눈 것 자체인 주장이 사라진다** |
...
##### 🔴 실물을 열어 보니 도식 하나가 세로로 늘어져 있었다 — 검사기는 통과시켰다

`check-mermaid` 는 문법만 보므로 **화면에서의 비례를 판정하지 않는다.** 빌드한 산출물을
브라우저로 열어 SVG 세 개의 크기를 재니 하나만 방향이 반대였다.

| 도식 | 고치기 전 | 고친 뒤 | 노드 |
| --- | ---: | ---: | ---: |
| ① 4단계 순환 (LR) | 730 × 124 (비 5.91) | 그대로 | 4 |
| ② 지표 사슬 | **214 × 874 (비 0.25)** | **770 × 308 (비 2.50)** | 8 |
| ③ 사례 연구 5단계 (LR) | 863 × 81 (비 10.61) | 그대로 | 5 |

②는 `flowchart TD` 에 subgraph 둘을 세로로 쌓아 8개 노드가 한 줄로 늘어섰고, 콘텐츠 폭
630px 가운데 **214px 만 쓰면서 세로로 874px** 을 차지했다. `flowchart TB` 로 바꾸고 각
subgraph 안에 `direction LR` 을 넣어 두 층이 가로로 놓이게 했다.

⚠️ **subgraph 사이를 잇는 화살표는 노드가 아니라 subgraph 끼리 연결해야 한다** (`CU --> BZ`).
subgraph 내부 노드를 밖과 이으면(`C4 --> B1`) mermaid 가 `direction` 을 무시한다.

⇒ **다음 편에 물릴 것**: `flowchart TD`/`TB` 에 subgraph 를 둘 이상 쌓을 때는 **반드시 실물을
열어 가로세로비를 재라.** 판정 기준은 「비가 1 미만이면 다시 그린다」이며, 이 리포의 정상
도식들은 2.5 ~ 10.6 이다.
...
### 10-3. 편3 실측 — 🔴 **고친 상수를 지지하는 첫 대조군이다**

발행본은 [`why-data-and-metric-dictionary`](../../../content/blog/product-management/why-data-and-metric-dictionary.md) 이고 최종 **17,850 B** 다 (목표 17,162 · 밴드 16,203 ~ 18,120).

#### 10-3-a. 초과의 출처가 편2와 반대였다

초고가 **20,459 B** 로 목표를 **19.2%** 넘겼다. 규칙 16 대로 절 목록이 아니라 절별 배율을
먼저 쟀다.
...
🔴 **편2 는 초과의 59.4%가 배율 밖이어서 상수를 고쳤고, 편3 은 67.1%가 배율 안이다.**
⇒ **상수 3,746 을 다시 건드리지 않았다.** 고친 상수가 다른 원본에서도 대체로 맞았다는
뜻이며, §10-2-b 가 대조군 없이 세운 값에 **첫 대조군이 붙었다.** 다만 고정 비용이 여전히
1,144 B 초과이므로 상수가 확정된 것은 아니다 — **편4 가 두 번째 대조군이다.**

#### 10-3-b. 배율이 가장 높은 절이 배정 밖에서 왔다

§3 대응부의 2.667 안에 「이름이 같아서 틀리는 두 쌍」이라는 절 하나가 통째로 있었다.
그 내용은 원본 §3 의 **경력 메모 블록**(114행)이 강조한 것이고 배정 안에는 정의만 있다.
⇒ 절을 없애고 앞 절 뒤의 한 문단으로 접었다 (−358 B).

⚠️ **경력 메모를 배정에서 뺐다고 그 내용이 발행본에 들어오지 않는 것이 아니다.** 블록을
옮기지 않았어도 **그 블록이 무엇을 중요하다고 말했는지**가 절 구성에 남는다. 배정 밖 주장이
새어 들어오는 경로가 인용이 아니라 **강조의 배분**이라는 것이 이번에 드러났다.

#### 10-3-c. 「복제를 지우면 는다」가 반증됐다
...
§8-2 3번은 앞 배치 실측 **+232 B** 를 근거로 「고쳐 쓴 문장은 길어진다」고 적었는데
이번은 **−11 B** 였다. ⇒ **그 규칙은 방향이 아니라 「다시 재라」로만 읽는다.**

#### 10-3-d. 예고 둘이 각각 맞고 틀렸다

| 예고 | 어디서 | 결과 |
| --- | --- | --- |
| 「강의」 SOFT 가 발행본에 남을 것 | §2-4-b | ✅ 초고에 **5건** 나왔다. 다만 편1·편2 가 「강의」를 **한 번도 쓰지 않았다** — 시리즈 관례를 실측하고 5건을 전부 일반 명제로 다시 썼다 |
| 「면접」 SOFT 가 편3 에서 더 나올 것 | §10-2 끝 | ❌ **0건이다.** 정성 수집 5법을 옮기며 `IDI/FGI` 를 「면담」으로 적었기 때문이다 |

🔴 **뒤엣것이 규칙 15 의 예다 — 금지된 것은 낱말이 아니라 자리다.** 같은 방법론을 가리키는
말이 여럿이므로, 금칙어를 예고할 때는 **어느 낱말을 쓸지가 아직 정해지지 않았다**는 것을
함께 적어야 한다.

#### 10-3-e. 원본 대조에서 남긴 2건

`source-overlap` ①이 초고에서 4건이었고 산문 2건을 고쳐 **2건**으로 줄였다. 남은 둘은
**표의 수치 행**(전환율 52·16·32 와 활성 사용자 100·110·115)이며 사실이라 다르게 쓸 수 없다.

⚠️ **②가 0건인 것은 겹침이 없어서가 아니다** (규칙 17). ②는 `**` 를 지우므로 걸린 문자열이
20자 임계값 밑으로 내려간다 — 실측으로 「전환율이 **40%로 두 배**가 되어 월」 25자가 정규화
뒤 14자가 된다. **강조가 든 축자 복사는 ①에서만 보인다.**

🔴 **원본 대조를 피해 고친 문장이 마크업 검사를 깨뜨렸다.** `전환율이 **40%로 두 배**가` 를
`전환율이 **두 배인 40%**가` 로 바꿨더니 닫는 별표 앞이 `%` 가 되어 **강조가 렌더되지 않는
자리**로 바뀌었다(위반 2회). 조사를 강조 안에 넣어 닫았다.
⇒ **한 검사를 통과시키려 고친 문장은 다른 검사를 다시 돌려야 한다.** 이 배치에서 겹치는
쌍은 `source-overlap` ①과 `check-markup` 이다 — 둘 다 **강조 기호 주변**을 보기 때문이다.

⚠️ **이 문단을 쓰다가 같은 위반을 한 번 더 냈다.** 위반 사례를 인용하면 그 인용이 위반이
된다. 훅이 커밋을 막아 드러났고, 예시를 **인라인 코드로 감싸** 닫았다 — 검사기의 케이스 ⑫가
그것을 면제한다.

#### 10-3-f. 편4 예산 — 재환산하지 않는다

상수와 k 를 그대로 두므로 §3-2 의 값이 유효하다. **편4 는 목표 18,674 B · 밴드 17,606 ~
19,741** 이다. 편4 는 원본 `11` §5·§6 이며 **실습 사례 전 과정**이라 편3 이 정의만 다루고
넘긴 자리를 받는다 — 지표 정의를 다시 설명하지 않고 편3 을 링크한다(§4-1).

### 10-4. 편4 실측 — ✅ **고친 상수를 지지하는 두 번째 대조군이다**

#### 10-4-a. 초고가 밴드 안에서 나왔다 — 편3 과 달리 배율을 강제로 재지 않아도 되었다
...
| 초고 B | 20,459 | **19,147** |
| 목표 대비 | **+19.2%** | **+2.5%** |
| 규칙 16 의 10% 문턱 | 넘었다 — 절별 배율을 쟀다 | **넘지 않았다** |

편3 이 문턱을 넘어 절별 배율을 재야 했던 것과 달리 편4 는 밴드(17,606 ~ 19,741) 안에서
나왔다. 그래도 §10-3-a 가 편4 를 **고정 비용 상수의 두 번째 대조군**으로 지목했으므로 역산했다.

> **역산 k = (발행본 B − 고정 비용 3,746 − 링크 몫) ÷ 배정 원본 B**
...
**두 편의 k 가 0.017 차이로 붙었고 둘 다 밴드(1.81 ~ 2.09) 안이다.** 편2 에서 2,455 → 3,746
으로 고친 고정 비용이 연속 두 편에서 유지되므로 **편5 예산은 재환산하지 않는다.** §3-2 의
**목표 20,004 B · 밴드 18,840 ~ 21,167** 이 그대로 유효하다.

⚠️ 배정 원본을 사본으로 떠서 다시 재니 **7,622 B** 로 §2-2 의 7,624 와 2 B 달랐다. 절 경계의
빈 줄 처리 차이이며 k 를 소수 셋째 자리까지 바꾸지 않는다. **다만 이 2 B 를 확인하지 않고
넘어가면 다음 편에서 같은 차가 다른 원인으로 읽힌다.**

#### 10-4-b. 🔴 고립 검사가 링크 누락 **두 곳**을 잡았다 — §6-1 이 절차를 적지 않았다

`tests/blog` 가 편4 를 **고립 1편**으로 판정했다. 편4 자신은 지도편 · 편2 · 편3 을 가리켰지만
**편4 를 가리키는 글이 하나도 없었다.**

| 자리 | 무엇이 비어 있었나 | 왜 비었나 |
| --- | --- | --- |
| 지도편의 편 목록 표 | 편4 행이 링크 없는 평문이었다 (편2·편3 만 링크였다) | 지도편을 쓴 세션에는 편4 가 아직 없어 링크를 걸 수 없었다 |
| 편3 의 마지막 예고 문장 | 「다음 편은 그 한 바퀴를 끝까지 따라간다」에 링크가 없었다 | 같은 이유다 |

🔴 **§6-1 은 「편1 이 일곱 편을 가리킨다」는 상태만 적었고, 그 상태를 만드는 절차를 적지
않았다.** 예고를 쓴 세션은 대상이 없어 링크를 걸 수 없고, 발행하는 세션은 자기 파일만 보므로
이 자리를 놓친다. **두 세션 사이에 아무도 책임지지 않는 구간이 생긴다.**

⇒ **편5~편8 은 발행할 때 자기 파일 말고 두 곳을 함께 고쳐라.**

1. `pm-practice-map.md` 의 편 목록 표에서 **자기 행을 링크로** 바꾼다.
2. **앞 편의 마지막 예고 문장**에 자기 링크를 건다.

✅ 이 구간을 실제로 막은 것은 `tests/blog/content` 의 **고립 검사**다. 다섯 스크립트 검사기는
전부 초록이었고 훅도 통과했을 것이다 — **깨진 것이 새 파일이 아니라 「가리키는 쪽」이라
스캔 대상에는 아무 문제가 없었기 때문이다.**

#### 10-4-c. 복제를 지우자 분량이 **158 B** 늘었다

`dup-scan` 이 초고에서 편4 관련 **3종 4건**을 잡았고 셋 다 편3 의 문장을 이어받다 생겼다.

| 걸린 자리 | 판정 |
| --- | --- |
| 도입부 「그런데 실제 일은 지표를 정의하는 데서 시작하지 않는다」 | 편3 의 마지막 문단을 그대로 받았다. 고쳐 썼다 |
| 도입부 「적어 넘기고, 결과를 다시 읽는 과정이 한 바퀴로 이어」 | 같은 자리다 |
| 「이 편이 쓴 용어」 절의 도입 문장 | 🔴 **관례인 줄 알았으나 아니었다.** 편2 는 같은 절을 다른 문장으로 열고 있어, 이 문장을 쓰는 편은 편3 과 편4 **둘뿐**이었다. 베낀 것이므로 고쳐 썼다 |

고친 뒤 편4 는 **0건**이 되었고 분량은 19,147 → **19,305 B**(+158 B)가 되었다. `CLAUDE.md` 가
실측 +232 B 로 적어 둔 자리이고 이번은 그보다 작다. **밴드 상단까지 남은 여유는 436 B 였다.**

⇒ **「시리즈의 정형 문구」라고 생각한 것이 실제로 정형인지 세어 보라.** 앞 편 하나만 보고
따라 쓰면 그 편과의 복제가 되며, 편이 셋 이상 쌓이기 전에는 관례와 복제가 구분되지 않는다.

#### 10-4-d. 원본 대조 — ③을 0으로 닫고 ①은 11건을 남겼다

| 항목 | 초고 | 최종 | 판정 |
| --- | ---: | ---: | --- |
| ① 공백 보존 | 12건 | **11건** | 임팩트 시뮬레이션 · ICE 점수 · 퍼널 전환율 같은 **사례의 수치 행**이라 §10-3-e 의 판정대로 남긴다 |
| ② 공백 제거 | 6건 | **5건** | 같다 |
| ③ 도식 라벨 통째 일치 | **5건** | **0건** | 원본 흐름도의 단계 이름을 그대로 옮긴 것이었다. **본문이 실제로 쓰는 표현**으로 바꿔 닫았다 |

③을 닫은 방법이 부수 효과를 냈다. 「팀 세팅 · 할 일 수집 · 이해관계자 설득」을 「목표 세우기 ·
할 일 모으기 · 설득과 합의」로 바꾸자 **도식의 라벨과 본문의 서술이 같은 낱말을 쓰게 되었다.**
원본 라벨을 그대로 두면 도식과 본문이 다른 어휘로 같은 것을 가리키는 상태가 된다.

⚠️ **편4 는 원본 §5 자체가 수치 표의 연속이라 ①의 겹침이 구조적으로 많다.** 편3 이 2건을
남긴 것과 11건을 직접 비교하지 마라 — 원본의 성분이 다르다.

---

### 10-5. 편5 실측 — ✅ **세 편이 같은 상수를 지지한다. 더 이상 대조군을 쌓지 않는다**

#### 10-5-a. 역산 k 가 세 편 연속으로 밴드 안이다

| 값 | 편3 | 편4 | **편5** |
| --- | ---: | ---: | ---: |
| 목표 B | 17,162 | 18,674 | **20,004** |
| 초고 B | 20,459 | 19,147 | **21,021** |
| 목표 대비 | +19.2% | +2.5% | **+5.1%** |
| 규칙 16 의 10% 문턱 | 넘었다 | 넘지 않았다 | **넘지 않았다** |
| 최종 B | 17,849 | 19,305 | **21,046** |
| **역산 k** | 2.050 | 2.033 | **2.076** |

세 편의 k 가 **0.043 폭 안에** 모였다. 편2 에서 2,455 → 3,746 으로 고친 고정 비용이 세 편
연속으로 유지되므로 **편6~편8 의 예산을 재환산하지 않는다.** §3-2 의 값이 그대로 유효하다.

⇒ **§3-4 의 재환산 지점은 둘 다 닫혔다.** 남은 세 편에서 k 를 다시 계산할 이유는, 초고가
규칙 16 의 10% 문턱을 넘는 경우뿐이다.

#### 10-5-b. 🔴 밴드 상단의 여유가 **121 B** 였다 — 편4 의 4분의 1이다

| 편 | 최종 B | 밴드 상단 | 여유 | 복제 수정이 늘린 폭 |
| :---: | ---: | ---: | ---: | ---: |
| 4 | 19,305 | 19,741 | 436 B | +158 B |
| **5** | **21,046** | **21,167** | **121 B** | **+25 B** |

`CLAUDE.md` 가 기록한 실측은 **+232 B** 다. 이번 초고가 그 폭으로 늘었다면 밴드를 **86 B**
넘겼을 것이고, 넘긴 사실은 `compose` 를 다시 돌리기 전까지 드러나지 않는다.

⇒ **여유가 세 자리로 좁으면 복제 수정의 방향을 정할 때 길이를 함께 보라.** 이번에는 여섯 자리
가운데 넷을 같은 길이 이하로 바꿔 +25 B 에 그쳤다.

#### 10-5-c. 🔴 `source-overlap` ①의 건수는 편끼리 비교할 값이 아니다

초고에서 ①(공백 보존)이 **6건 · 최장 24자**를 냈다. 편4 의 11건과 자릿수가 비슷하지만
**남길 수 있는지의 판정은 정반대**였다.

| 편 | 걸린 것의 성격 | 판정 | 왜 |
| :---: | --- | --- | --- |
| 4 | 임팩트 시뮬레이션 · ICE 점수 · 퍼널 전환율의 **수치 행** | 남겼다 | 값을 바꾸면 사례가 성립하지 않는다 |
| **5** | 노력 항목의 **서술 문장** 넷 · 표의 판정 문장 하나 · 절 제목 하나 | **전부 고쳐 썼다** | 바꿔 쓸 수 있는데 바꾸지 않은 것이다 |

절 제목이 걸린 것이 이번의 새로운 자리다. 「성과가 좋아도 신뢰가 없으면 운으로 보인다」는
원본 표의 판정 문장을 그대로 옮긴 것이었고, **제목이라서 눈에 띄지 않았다** — 본문을 읽을 때는
자기가 쓴 문장으로 읽힌다. 「신뢰가 없으면 같은 결과도 운으로 읽힌다」로 바꿔 닫았다.

⇒ **①의 건수를 앞 편과 비교해 「이 정도면 정상」이라고 판단하지 마라.** 성격이 다르면 같은
건수가 정반대의 처분을 받는다. 편4 가 11건을 남긴 것은 그 편의 원본이 수치 표의 연속이었기
때문이지 11건이 허용치여서가 아니다.

#### 10-5-d. ✅ §10-4-b 의 절차를 처음으로 따랐다 — 고립 검사에 걸리지 않았다

편4 가 뚫린 구간이다. 이번에는 발행 커밋 안에서 두 곳을 함께 채웠다.

| 자리 | 무엇을 했나 |
| --- | --- |
| `pm-practice-map.md` 의 편 목록 표 | 편5 행을 링크로 바꿨다 |
| 편4 의 마지막 예고 문장 | 「다음 편은 그 묶음, 곧 팀 자체를 본다」에 편5 링크를 걸었다 |

⚠️ **초고에는 세 번째 자리가 하나 더 있었다.** 도입부에서 편6 을 가리키려다 링크 주소를
지도편으로 걸어 두었는데, 검사기 다섯 가운데 어느 것도 이것을 위반으로 보지 않는다 — 있는
문서를 가리키는 멀쩡한 링크이기 때문이다. **편6 이 아직 없으므로 링크를 빼고 평문으로 두었다.**
⇒ **「없는 편을 가리키고 싶을 때 지도편으로 대신 걸어 두는」 습관을 만들지 마라.** 그 링크는
검사를 통과하면서 독자를 다른 곳으로 보낸다.

### 10-6. 편6 실측 — 🔴 **규칙 16 을 편3 이후 처음으로 다시 발동했다**

#### 10-6-a. 초고가 +24.0% 였고 원인이 두 갈래였다

| 값 | 편4 | 편5 | **편6** |
| --- | ---: | ---: | ---: |
| 목표 B | 18,674 | 20,004 | **20,009** |
| 초고 B | 19,147 | 21,021 | **24,813** |
| 목표 대비 | +2.5% | +5.1% | **+24.0%** |
| 규칙 16 의 10% 문턱 | 넘지 않았다 | 넘지 않았다 | **넘었다** |
| 최종 B | 19,305 | 21,046 | **20,909** |
| **역산 k** | 2.033 | 2.076 | **2.059** |

규칙 16 대로 절 목록이 아니라 **절별 배율**을 먼저 쟀고, 그 표가 초과를 두 갈래로 갈랐다.

| 갈래 | 크기 | 무엇이었나 |
| --- | ---: | --- |
| ① 배정 절의 배율 초과 | +1,887 B | 여덟 절 가운데 **다섯이 밴드(1.81 ~ 2.09) 밖**이었다 |
| ② 고정 3절 초과 | +2,087 B | 원본이 없는 도입 · 용어 · 정리가 예산상 고정분 3,808 을 넘겼다 |

**배율이 튄 절은 전부 원본이 짧은 절이었다.**

| 절 | 원본 B | 초고 B | 배율 |
| --- | ---: | ---: | ---: |
| §4-2 (팀 빌딩 관점의 세 개념) | 372 | 1,600 | **4.301** |
| §5-3 (시작 전 두 가지) | 639 | 2,171 | **3.397** |
| §6 (3분 비교) | 1,624 | 4,201 | 2.587 |
| §4-3 (PL · PM · PO) | 841 | 2,148 | 2.554 |
| §5-1 · §5-2 · §5-4 | — | — | 1.878 ~ 1.994 ✅ |

⇒ **원본이 짧을수록 배율이 튄다.** 옮길 것이 적은 자리는 설명으로 채우게 되기 때문이다.
§4-2 의 원본은 3행짜리 표 하나뿐인데 거기에 「계급을 걷어 내면 정렬이 드러난다」는 논의를
붙였고, 그 논의는 §6 에서 더 강한 형태로 다시 나왔다 — **중복이었으므로 §6 에 남기고 §4-2 에서
지웠다.**

#### 10-6-b. ✅ 대조군이 ②를 원인 목록에서 뺐다

②는 이번만의 일로 보였다. 그런데 **편5 를 같은 방식으로 재니** 그쪽도 고정 3절이 5,323 B 로
상수를 1,577 B 넘기고 있었고, 그 상태로 k = 2.076 을 냈다.
...
⇒ **고정 비용 상수 3,746 은 그 세 절의 실제 바이트가 아니라 배율에 흡수되는 몫이다.**
초과의 실체는 배정 절이었다. **대조군을 세우지 않았다면 손댈 필요가 없던 도입과 정리를
깎았을 것이고, 그래도 초과는 남았을 것이다.**

⚠️ 배정 원본을 다시 실측하니 **8,304 B** 로 설계서 §3-2 의 8,308 과 4 B 차였다(0.05%).
경계를 「인용이 끊기는 지점」으로 잡는 방식이 §2-3 과 같으므로 블록 뒤 빈 줄 처리의 차이로
읽고 목표 B 는 그대로 썼다.

#### 10-6-c. `source-overlap` ①은 6건을 남겼고 판정은 편5 와 달랐다

초고에서 ①이 **10건 · 최장 46자**였다. §10-5-c 대로 건수가 아니라 성격으로 판정했다.

| 성격 | 건수 | 판정 |
| --- | ---: | --- |
| 서술 문장 | 5 | **전부 고쳐 썼다** — 바꿔 쓸 수 있는데 바꾸지 않은 것이다 |
| 항목 이름 · 용어 정의 · 세 유형의 이름 | 6 | **남겼다** |

남긴 6건은 표의 세로줄이 끼어 길이가 부풀려진 것이다. 「밍글링」과 「프로덕트 팀 리드」의
풀이는 **지도편 용어표와 맞춰야 하므로 바꾸면 오히려 어긋나고**, 세 유형의 이름은 고유명사에
준한다. ③(도식 라벨)은 원본에 도식이 없어 발행본 라벨 10개가 전부 새로 그린 것이라 0건이다.

✅ **복제를 고친 뒤 분량이 5 B 줄었다.** `CLAUDE.md` 의 실측은 +232 B, 편4 는 +158 B,
편5 는 +25 B 였다. §10-5-b 대로 고칠 문장의 길이를 함께 보고 같은 길이 이하로 바꾼 결과다.

#### 10-6-d. `dup-scan` 이 편5 의 문장을 56자 잡았다

「앞 편들과의 관계를 한 줄로 두면, 편2가 무엇을 위에 둘지를…」이 걸렸다. §10-4-c 가 겪은
자리와 같은 종류이며, 이 문장을 쓰는 편이 **편5 · 편6 둘뿐**이라 관례가 아니라 복제였다.
고쳐 쓴 뒤 양쪽 다 0건이 되었다.

⇒ **앞 편에서 좋았던 문장은 그 편의 것이지 시리즈의 것이 아니다.** 편2 ~ 편5 의
「이 편이 쓴 용어」 도입 문장 넷이 모두 다른 낱말을 쓰는 것이 시리즈의 실제 관례다.

#### 10-6-e. 🔴 설계서 편집 중에 파일을 0 B 로 잘랐다

`io.open(p, "w")` 는 **여는 순간 파일을 비우고**, 이어지는 `write()` 가 인코딩 오류로 죽으면
빈 파일만 남는다. 이모지를 서로게이트 쌍(`🔴`)으로 적은 것이 원인이었고
1,276행 설계서가 **0 B** 가 되었다. 커밋된 파일이라 `git checkout HEAD --` 로 복구했다.

⇒ **읽고 쓰는 치환은 임시 파일에 쓴 뒤 옮기거나 `Edit` 도구로 하라.** 그리고 복구 뒤
워크트리의 CR 이 **0 → 1,276** 으로 바뀌었다 — `autocrlf` 가 체크아웃에서 붙인 것이며
`git diff` 는 비어 있다. **정본은 워크트리가 아니라 blob 이다**(`CLAUDE.md` 의 그 항목).

#### 10-6-f. 편7 예산 — 재환산하지 않는다

네 편의 k(2.050 · 2.033 · 2.076 · **2.059**)가 여전히 0.043 폭 안이다. 초고가 문턱을 넘은
것은 배율이 아니라 **짧은 절을 설명으로 채운 집필 습관**에서 왔으므로 상수를 손대지 않는다.
편7 은 §3-2 의 **22,953 B**(밴드 21,578 ~ 24,328)를 그대로 쓴다.

⇒ **편7 · 편8 은 초고를 쓸 때 원본이 짧은 절을 먼저 표시해 두어라.** 이번 초과의 절반이
그 자리에서 나왔다.

---

### 10-7. 편7 실측 — 🔴 **규칙 16 을 두 편 연속 발동했고 원인의 비중이 편6 과 달랐다**

#### 10-7-a. 초과의 81%가 배율이었다 — 편6 은 대조군이 고정 3절을 뺐지만 이번은 못 뺐다

| 값 | 편5 | 편6 | **편7** |
| --- | ---: | ---: | ---: |
| 목표 B | 20,004 | 20,009 | **22,953** |
| 초고 B | 21,021 | 24,813 | **28,507** |
| 목표 대비 | +5.1% | +24.0% | **+24.2%** |
| 최종 B | 21,046 | 20,909 | **24,113** |
| **역산 k** | 2.076 | 2.059 | **2.068** |
...
배정 원본이 17.1% 크므로 편6 배율로는 배정 절이 18,378 B 여야 했다. 실제와의 차 **+3,977 B** 가
초과의 **80.9%** 이고 남은 19.1%가 고정 3절이었다.

⇒ **§10-6-b 의 절차는 그대로 유효하지만 결론은 편마다 다르다.** 편6 은 대조군(편5)이 고정 3절을
5,323 B 로 쓰고 있어 ②가 원인 목록에서 빠졌는데, 편7 초고는 **6,153 B** 로 편5 · 편6 둘 다보다
컸다. 용어 표가 한 행 늘어난 것으로는 설명되지 않는 크기여서 두 갈래를 모두 줄였고 최종에서
고정 3절이 **5,477 B** 가 되었다. **대조군을 세우는 절차가 「②는 언제나 원인이 아니다」를
뜻하지 않는다.**

#### 10-7-b. 🔴 §10-6-f 의 예고는 맞았으나 최댓값은 예고 밖에서 나왔다

「원본이 짧은 절을 먼저 표시해 두어라」를 그대로 따랐고, 초고에서 여덟 절 가운데 **일곱이 밴드
밖**이었다 (편6 은 다섯이었다).

| 절 | 원본 B | 초고 B | 배율 | 예고에 걸렸나 |
| --- | ---: | ---: | ---: | --- |
| §3-3 (관문으로서의 절차) | 1,162 | 3,324 | **2.861** | ❌ **아니다** |
| §3-1 (세 국면의 순서) | 725 | 1,729 | **2.385** | ✅ 원본 최단 |
| §3-2 (검토의 원칙) | 910 | 2,161 | **2.375** | ✅ 원본 차단 |
| §1-1 (사람을 들이는 단계) | 1,072 | 2,541 | 2.371 | 애매하다 |
| §4 (진행 관리) | 1,962 | 3,972 | 2.024 ✅ | — |

⇒ **최고 배율 절의 원본은 짧지 않았다.** §3-3 에는 도식 하나와 기존 편으로 건너가는 접합 문단이
함께 들어갔고, 그 둘이 원본에 대응물이 없는 분량이다. **편8 은 짧은 원본과 함께
「도식 + 접합을 같이 넣는 절」도 미리 표시하라.** 배율만 보면 짧은 절이 튀지만 **절대 초과량**은
이쪽이 더 크다.

#### 10-7-c. `source-overlap` ①은 0건으로 닫혔고 판정이 편6 과 갈렸다

초고에서 ①이 6건(최장 29자) · ②가 1건(29자)이었다. §10-5-c 대로 건수가 아니라 성격으로
판정했다.

| 성격 | 건수 | 판정 |
| --- | ---: | --- |
| 표 머리 행의 세 열 이름 | 1 | **고쳐 썼다** — 원본이 정한 축이지만 바꿔도 뜻이 어긋나지 않는다 |
| 서술 문장 | 5 | **전부 고쳐 썼다** |
| ②의 완전한 문장 축자 복사 | 1 | **고쳐 썼다** |

⇒ **일곱 건 모두 「바꿀 수 있는데 바꾸지 않은 것」이었다.** 편6 이 여섯 건을 남긴 것과 다른데,
편6 의 남은 건은 **항목 이름 · 용어 정의**여서 바꾸면 지도편 용어표와 어긋났기 때문이다.
**건수를 앞 편과 비교하면 판정이 뒤집힌다** — 편6 의 6건은 정당하고 편7 의 6건은 전부 결함이었다.

⚠️ 복제를 고친 뒤 분량이 **44 B 늘었다** (편6 −5 · 편5 +25 · 편4 +158 · `CLAUDE.md` +232). 밴드
상단 여유가 214 B 로 남은 것은 고칠 때마다 `compose` 를 다시 돌린 결과다.

#### 10-7-d. 🔴 §6-2 가 편7 에 남겨 둔 접합부는 낱말이 없는 자리였다

§6-2 는 편7 의 상대를 「도메인 편 가운데 글로벌을 다룬 편」으로 적고 집필 전에 채우라고 했다.
규칙 10 대로 확인하니 `product-management-domains` 여덟 편에서 「글로벌」이 **두 편에 각 1회**
뿐이었고 둘 다 편7 이 건너갈 문맥이 아니었다.

⇒ [`planning-and-product-ops`](../../../content/blog/product-management/planning-and-product-ops.md) 의 **71행**(요구사항이 들어와 종료로 나가기까지) ·
**86 ~ 96행**(아홉 단계 표) · **98행**(분석을 두 번 하도록 못 박은 이유)으로 바꿨다. 같은 목록을
그쪽은 **순서**로 다루고 편7 은 **관문**으로 다룬다.

**편6 에 이어 두 편 연속으로 이 경고가 발동했다.** 편6 은 근거가 옳고 낱말이 없었고, 편7 은
**자리 자체가 비어 있었다** — 설계서가 편 이름을 적지 못한 칸은 그 자리가 없을 가능성을 함께
담고 있다. ⇒ **§6-2 의 「기존 시리즈」 칸이 편 이름이 아니면, 채우는 일이 아니라 찾는 일이다.**

#### 10-7-e. 편8 예산 — 재환산하지 않는다

다섯 편의 k(2.050 · 2.033 · 2.076 · 2.059 · **2.068**)가 여전히 0.043 폭 안이다. 초고가 두 편
연속 문턱을 넘었지만 최종 k 는 흔들리지 않았으므로 상수를 손대지 않는다. 편8 은 §3-2 의
**21,546 B**(밴드 20,273 ~ 22,820)를 그대로 쓴다.

⇒ **편8 은 이 시리즈의 마지막 편이다.** §6-1 의 형제편 outbound 세기를 발행 직전에 돌려
지도편이 일곱 편을 모두 가리키는지 확인하라.

### 10-8. 편8 실측 — ✅ **배치가 끝났다. 여섯 편의 k 가 0.043 폭 안에 그대로 있다**

#### 10-8-a. 🔴 대조군을 **둘** 놓아야 초과가 100% 갈린다

초고가 24,486 B 로 목표 21,546 대비 **+13.6%** 였다. 편6 · 편7 에 이어 규칙 16 이 세 편 연속
발동했다. §10-6-b 의 절차대로 앞 편 최종을 대조군으로 놓았는데, **그것만으로는 초과의 39%가
설명되지 않고 남았다.**

| 대조군 | 고정 3절 | 배정 절 배율 | 이 대조군으로 설명되는 초과 |
| --- | ---: | ---: | ---: |
| 편7 최종 | 5,477 | 1.917 | 1,794 B (61.0%) |
| **목표 자체** (편5 · 편6 · 편7 최종 평균) | **5,338** | **1.804** | **2,942 B (100.0%)** |

남은 39%는 **편7 자신이 목표를 +5.1% 넘겨 착지한 값**이다. 편7 이 §10-7-a 에서 편6 최종을
대조군으로 쓸 때도 같은 이월이 있었으나, 편6 의 초과(+4.5%)가 편7 의 초과(+5.1%)와 비슷해
차가 드러나지 않았다.

⇒ 🔴 **앞 편 최종만 대조군으로 삼으면 그 편의 초과분이 분해에 이월되어 보이지 않는다.**
목표를 함께 대조군으로 놓아야 원인이 100% 갈린다. 이것이 편6 · 편7 의 절차에 이번이 더한 것이다.
...
편7 은 배율 80.9% · 고정 3절 19.1% 였다. **고정 3절이 두 편 연속 원인 목록에 남았다** —
§10-7-a 의 「대조군을 세웠다고 ②가 자동으로 빠지지 않는다」가 유지된다.

#### 10-8-b. ✅ §10-7-b 의 예고가 이번에는 최댓값을 짚었다

편7 은 짧은 원본만 표시했다가 최고 배율을 **도식과 접합이 함께 들어간 절**에서 맞았고, 그래서
§10-7-b 가 둘을 모두 표시하라고 남겼다. 편8 은 그대로 따라 여덟 절을 미리 갈랐다.

| 절 | 원본 B | 초고 B | 배율 | 최종 B | 최종 배율 | 예고 |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| §5-1 (주간 공유) | 608 | 2,336 | **3.84** | 1,870 | **3.076** | ✅ 🔴 **짧은 원본 + 접합** — 유일하게 두 위험을 겹쳐 표시했다 |
| §5-2 (한 명의 결정권자) | 803 | 2,146 | 2.67 | 2,027 | 2.524 | ✅ 짧은 원본 + 도식 |
| §6-2 (세 사례) | 1,274 | 2,586 | 2.03 | 2,319 | 1.820 | ❌ 아니다 |
| §6-1 (열흘 계획) | 2,018 | 3,670 | 1.82 | 3,359 | 1.665 | ⚠️ 도식 |

⇒ **두 위험이 겹치는 절을 따로 표시한 것이 최댓값을 짚었다.** 최종에서도 §5-1 이 3.076 으로
편7 의 최고(2.861)보다 크다. **접합 문단은 원본에 대응물이 없으므로 짧은 원본에 얹히면 배율이
3을 넘는다** — 편7 의 §3-3(2.861)은 원본이 1,162 B 였고 편8 의 §5-1 은 608 B 였다.

#### 10-8-c. 🔴 `dup-scan` 이 잡은 것은 「정형 문구」가 아니라 답습이었다

초고에서 `dup-scan` 2건 · `source-overlap` ① 2건이었고 **넷 모두 결함이었다.**

| 검사 | 건수 | 성격 | 판정 |
| --- | ---: | --- | --- |
| `dup-scan` | 1 | 「이 편이 쓴 용어」 절의 도입 문장 (30자) | **고쳐 썼다** |
| `dup-scan` | 1 | 접합 문단의 축자 인용 (21자) | **고쳐 썼다** — 편7 의 접합은 요약이었다 |
| `source-overlap` ① | 2 | 서술 문장의 축자 복사 (27자 · 21자) | **전부 고쳐 썼다** |

🔴 **첫 건이 위험했다.** 「지도편의 용어표는 이 편에 낱말 N을 맡겨 두었다」는 시리즈의 정형
문구처럼 보였는데, 같은 자리를 여덟 편에서 `grep` 하니 **편7 만 그렇게 적고 편5 · 편6 은 다른
문구를 쓴다.** ⇒ **정형과 답습은 「다른 편이 몇 개 쓰는가」로 갈린다.** 두 편만 쓰면 정형이
아니라 앞 편을 베낀 것이다.

⚠️ `source-overlap` ③이 0건인데 이것은 **구조적 0이다.** 편8 배정 절(§5~§7)에 원본 도식이
없어 대조 대상 자체가 없고, 검사기가 센 원본 라벨 10개는 전부 편7 배정 구역(§1~§4)의 것이다.
규칙 17 대로 「왜 0인가」를 물어 확인했다.

⚠️ 복제를 고친 뒤 분량이 **32 B 줄었다** (편7 +44 · 편5 +25 · 편4 +158). 「복제를 지우면
분량이 는다」는 편3 이후 네 표본 중 **둘에서 반증**됐다 — 방향이 정해져 있지 않으므로 고친
뒤에 다시 재는 것 말고는 예측할 방법이 없다.

#### 10-8-d. 🔴 §6-1 의 outbound 세기 명령은 형제편만 세지 않는다

마지막 편이므로 §6-1 의 명령을 발행 직전에 돌렸고 **8** 이 나왔다. 세어야 할 형제편은 일곱이다.
목록을 열어 보니 여덟째는 `product-management-map` 으로 **다른 시리즈의 지도편**이었다.

⚠️ **이 명령은 카테고리 안의 모든 슬러그를 센다.** 형제편이 여섯만 있고 기존 시리즈 링크가
둘이어도 똑같이 8 이 나오므로, **수가 맞았다는 사실이 일곱 편을 다 가리킨다는 증거가 아니다.**
⇒ 슬러그 일곱 개를 하나씩 `grep -c` 로 대조해 전부 1 이상임을 확인했다.

**규칙 9 는 「세는 명령을 발행 전에 실행하라」인데 세는 대상이 명령에 적혀 있지 않았다.**
다음 배치는 지도편 확인을 **슬러그 목록과의 대조**로 적어라 — 총수 비교로는 판정되지 않는다.

#### 10-8-e. 접합부는 도메인 시리즈에서 새로 찾았다 — 세 편 연속이다
...
**편6 · 편7 에 이어 세 편 연속으로 §6-2 의 경고가 발동했다.** 셋의 양상이 조금씩 다르다.

| 편 | 무엇이 문제였나 |
| :---: | --- |
| 6 | 근거는 옳았고 **지목한 낱말이 0건**이었다 |
| 7 | **자리 자체가 없었다** — 「글로벌」이 두 편에 각 1회뿐이었다 |
| 8 | 칸이 비어 있었고, **찾으니 정확한 자리가 있었다** |

⇒ §10-7-d 의 「칸이 편 이름이 아니면 채우는 일이 아니라 찾는 일이다」가 유지되며, 편8 은 그
찾기가 **성공한 첫 표본**이다. 낱말이 0건이라는 사실이 접합면이 없다는 뜻은 아니다 — 편8 은
「커뮤니케이션」이 0건인 편에서 **같은 주제를 다른 낱말로 다룬 자리**를 찾았다.

#### 10-8-f. 배치 총괄 — 여덟 편 172,967 B

| 편 | 슬러그 | 발행 시점 B | 역산 k |
| :---: | --- | ---: | ---: |
| 1 | `pm-practice-map` | 21,803 | 고정 비용 방식 |
| 2 | `product-cycle-and-metric-hierarchy` | 24,596 | 재환산의 기준 |
| 3 | `why-data-and-metric-dictionary` | 17,849 | 2.050 |
| 4 | `data-driven-project-walkthrough` | 19,305 | 2.033 |
| 5 | `team-building-and-agile-org` | 21,046 | 2.076 |
| 6 | `roles-and-organizational-culture` | 20,909 | 2.059 |
| 7 | `global-collaboration-process` | 24,113 | 2.068 |
| 8 | `global-decision-and-communication` | **22,499** | **2.055** |

**여섯 편의 k 가 0.043 폭(2.033 ~ 2.076) 안에 있고 편8 이 그 한가운데다.** §3-2 가 표 축으로
보간한 1.95 보다 일관되게 높으나 **폭이 좁아 예측 가능하다** — 다음 `pm-po` 배치는 k 를
1.95 가 아니라 **2.05** 에서 출발시켜라.

🔴 **위 표의 「발행 시점 B」는 지금 파일을 재면 나오지 않는다.** 여덟 편을 다시 재니 합계가
**172,967 B**(평균 21,621)로 편마다 커져 있었다.

| 편 | 발행 시점 | 현재 | 차 |
| :---: | ---: | ---: | ---: |
| 1 (지도편) | 21,803 | 22,292 | **+489** |
| 7 | 24,113 | 24,177 | +64 |
| 8 | 22,499 | 22,499 | 0 |

⇒ **뒤 편이 나올 때마다 앞 편이 커진다.** 지도편은 일곱 편의 링크를 차례로 받았고, 편7 은 방금
편8 링크를 받아 링크 몫(63)만큼 늘었다. **§10 에 적힌 「최종 B」끼리는 비교할 수 있지만
지금 잰 값과는 비교할 수 없다** — 역산 k 를 나중에 다시 재면 전부 커진다.

배치 총량 예측은 §3-2 가 166,206 B 였고 현재 실측이 172,967 B 로 **+4.1%** 다.

#### 10-8-g. 가설 판정 — H-2 만 남았다

| # | 가설 | 판정 |
| ---: | --- | --- |
| H-1 | 인용이 두 배면 k 가 산문 축으로 끌린다 | ❌ 기각 (§10-2-c) |
| H-2 | 경력 메모 비율 14.6% 는 `pm-po` 계열의 편집 관행이다 | ⬜ **다음 `pm-po` 배치가 판정한다** — 이 배치는 원본 넷 모두에서 재현됐으나 같은 배치 안의 재현은 표본이 하나다 |
| H-3 | 07 을 한 편으로 둔 판단이 옳다 | ✅ 지지 (§3-0-a) |
| H-4 | 용어표가 넷이라 중복 제거 뒤 개수가 덜 는다 | ✅ 지지 · 이유는 틀렸다 (§10-1-b) |

#### 10-8-h. 다음 배치에 물려줄 규칙 — 18개에 셋을 더한다

| # | 규칙 | 이 배치의 근거 |
| ---: | --- | --- |
| 19 | 🔴 **초과를 분해할 때 대조군을 둘 놓아라** — 앞 편 최종과 목표 자체다. 앞 편만 쓰면 그 편의 초과분이 이월되어 보이지 않는다 | §10-8-a |
| 20 | 🔴 **접합 문단은 짧은 원본에 얹지 마라.** 원본에 대응물이 없어 배율이 3을 넘는다. 배정할 때 접합을 **원본이 긴 절**에 두어라 | §10-8-b · §10-7-b |
| 21 | 🔴 **「세는 명령」에는 세는 대상을 함께 적어라.** 총수 비교는 형제편과 남의 편을 구분하지 못한다 | §10-8-d |

앞 배치의 18개 가운데 이번 배치가 실제로 값을 한 것은 규칙 **9 · 10 · 13 · 14 · 16 · 17** 여섯이며,
그중 **16(절별 배율)과 10(낱말 실재)이 세 편씩 연속으로 발동했다.**
...
| [`../../TOOL-TRAPS.md`](../../TOOL-TRAPS.md) | 도구가 거짓을 보고하는 50가지 방식 |
