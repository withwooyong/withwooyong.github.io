...
지역 그래프 하나를 `getStaticProps` 로 넘길 때의 직렬화 크기를 편마다 재고 분포를 냈다.
...
비교 대상은 `CategoryTree` 다. 전체 트리를 페이지마다 싣는 안이 페이지당 약 18 KB 였고, 그것을 피하려고 「펼친 카테고리만 채운다」는 결정이 내려져 있다. 지역 그래프의 최대치 6.3 KB 는 그 기준의 3분의 1 수준이므로 **같은 이유로 거절될 크기가 아니다.**
...
| 인력 | 연결선으로 이어진 쌍 | 이어진 편을 가깝게 당긴다 | 연결선 수에 비례한다 |
...
| 층 | 무엇을 고정하는가 | 어디에 |
| --- | --- | --- |
| 추출 | 코드 블록 안 링크 제외 · 참조식 링크 포함 · 자기 링크 제외 · 방향 구분 | `tests/blog/graph.test.ts` (신규) |
