...
⇒ 발행 대상 **85개 원본** · 약 3.1 MB. 기존 변환 비율(원본 1개당 약 2.5편)로
**약 200편** 규모다.
...
전수 분류는 구현 **태스크 1**에서 원문을 읽고 수행하며, 폐기 전 각 건에 대해
「이 규칙이 가리키는 원본이 이미 소진됐는가」를 실측한다.
...
| 규칙 | 무엇 | 막았을 사고 | 판정 |
| --- | --- | --- | :---: |
| `CR-1.7` 금지선 45 | 내부 링크가 실재 슬러그를 가리키는가 | 죽은 링크 | HARD |
| 금지선 44·45 | **inbound 0 고립 편** (지도편 제외) | search-engineering 6편 고립 | HARD |
...
금지선 49·51~56은 규칙이 아니라 **이 환경에서 도구가 어떻게 실패하는가**다.
발행 규칙 목록에 있으면 매 발행마다 읽히지만, 실제로 필요한 시점은 그 도구를 쓸 때다.

| # | 내용 |
| ---: | --- |
| 49 | 파이프 뒤의 종료 코드는 마지막 명령의 것 — 종료 코드를 볼 명령은 단독 실행 |
| 51 | 1인칭은 grep으로 분리되지 않는다 — 매칭 줄을 열어 읽어라 |
| 52 | 금칙어 grep에 `-i` 필수 + 한글 표기형 포함 |
| 53 | 히어독으로 코드를 쓰면 이중 백슬래시가 한 겹 벗겨진다 |
| 54 | 서브에이전트가 유휴가 돼도 보고서가 오지 않는다 — `SendMessage`로 회수 |
| 55 | grep으로 CRLF를 검사하면 거짓 음성 — `tr -cd`로 바이트를 세라 |
| 56 | `core.autocrlf=true`면 `git diff`가 줄바꿈 변경을 숨긴다 |
