...
> **⚠️ 미구현 — 보류(D-1). 다시 설계할 때 아래 절이 아니라 「실측 4건」이 출발점이다.**
>
> 2026-08-26 에 이 절대로 히어로를 구현하고(선행 계획서 T9) 위험 축별로 리뷰한 결과
> **4건 모두 구현이 아니라 이 절 자체의 결함**이었다. 사용자가 현재 히어로를 유지하기로 해
> 단계 2 는 뒤로 미뤄졌고, 단계 3(검색)·4(아틀라스)가 먼저 갔다.
>
> | # | 이 절의 결함 |
> | --- | --- |
> | 1 | **GC-9 가 이 구조에서 정의되지 않는다.** `sticky top-0 h-screen` 이라 `p=0`~`1` 이 첫 화면을 떠나지 않는다 — `p=0` 으로 재면 액센트 0%, 사용자가 보는 시간으로 재면 **15.68%**(상한의 3.1배). **「첫 화면」을 먼저 다시 정의해야 한다** |
> | 2 | **LCP 가 서드파티에 묶인다.** 히어로 한글을 그리는 Pretendard 가 jsdelivr 에서 10건 · 216KB 렌더 블로킹. 게이트는 three.js(600KB)를 막고 있어 **위험 축이 어긋나 있었다** |
> | 3 | **`prefers-reduced-motion` 에서 h1 이 선행사를 잃는다.** `p=1` 고정이라 유일한 h1 이 「그 판단은 글 156편으로…」가 되고 문구 ①② 는 `aria-hidden` 이라 접근성 트리에도 없다 |
> | 4 | **`p` 의 33.3% 가 완전 정적이다.** 노드 점등 `0.6167`·엣지 `0.6548`·문구 ③ 경계 `0.6667` 로 끝난다. 그 `0.6667` 은 코드에 상수로 없다 — `stagger`·`NODES.length`·`LINES.length` 가 우연히 만든 값이라 **노드를 하나 늘리면 조용히 바뀐다** |
>
> 원문·재현 절차: `docs/superpowers/plans/2026-08-26-atlas-and-search.md` §「T9가 이월된 이유 — 실측 4건」(부수 발견 2건 포함).
...
⇒ **D-2 확정 (2026-08-27, 단계 4 구현 시점) — 태그는 그래프 노드로 넣지 않는다. 사이드바 필터로만 쓴다.**

| 항목 | 내용 |
| --- | --- |
| 결정 | `concept` 노드는 **카테고리 6개만**. 태그 64개는 노드가 아니라 필터 |
| 근거 | 바로 위 편중 표 — 최대 허브 `ai-agent` 가 **44편**을 잇고 2·3위가 **35 · 32**. 힘 기반 레이아웃에서 이 셋이 화면을 지배해 **「글끼리의 관계」가 아니라 「태그 성게」가 보인다** |
| 실물 | `lib/atlas/build.ts` 의 「concept 노드 — 카테고리만. 태그는 노드가 아니다(D-2)」 |

이 항목은 §15 미확정 표에서 여기로 옮겨졌다. §15.4 는 번호 참조 때문에 남겨 두되 종결 표시했다.
...
> **D-1 (2026-08-27) — 실제 순서는 아래 계획과 다르게 갔다. 단계 2 는 삭제가 아니라 보류다.**
>
> 사용자가 **현재 히어로를 유지**하기로 해 단계 2(메인 5섹션 · 히어로 B)가 뒤로 미뤄지고,
> 단계 3(검색)·4(아틀라스)가 먼저 갔다. 단계 2 의 나머지(`/work` 통합 · `/about` 신규 ·
> `product-lead*` 스텁화)도 함께 이월됐다 — 히어로 재설계의 결함 4건은 §6 상단에 있다.
>
> ```mermaid
> flowchart LR
>     P1["1 ✅<br/>디자인 토큰<br/>셸 · 테마"] --> P3["3 ✅<br/>/blog 재설계<br/>⌘K 검색<br/>Pagefind"]
>     P3 --> P4["4 ✅<br/>아틀라스 1차 데이터<br/>렌더러 · 렌즈<br/>검색 · 그래프 연동"]
>     P1 -.보류.-> P2["2 ⏸<br/>메인 5섹션 · 히어로 B<br/>/work · /about<br/>product-lead* 스텁화"]
>     P4 --> MERGE(["main 머지<br/>= 발행"])
>     P2 -.-> MERGE
> ```
>
> **단계 2 의 「끝에서 기준선 1회 갱신」은 그대로 유효하다** — 단계 4 에서 접근성 수정(`<main tabindex="-1">`)이
> `product-lead*` 9장을 바꿔 2026-08-27 에 한 번 갱신했다(9건 전부 `product-lead*`, 새 항목 0).
>
> 아래는 **원래 계획**이다. 기록으로 남긴다.
...
| ~~**태그를 그래프 노드로 넣을지**~~ | ~~**넣되 차수 상한을 둔다** (아래 15.4)~~ **→ 닫힘. D-2 로 「안 B — 사이드바 필터로만」 확정.** 결정은 §7.3, 경위는 §15.4 | ~~단계 4~~ **2026-08-27 종결** |
...
### 15.4 태그를 그래프 노드로 넣을 것인가 — ✅ 종결 (2026-08-27, D-2)

> **결론: 이 절이 권한 안 C 가 아니라 안 B 가 채택됐다. 태그는 노드가 아니라 사이드바 필터다.**
>
> | 항목 | 내용 |
> | --- | --- |
> | 확정 | **안 B** — `concept` 노드는 카테고리 6개만. 태그 64개는 필터 |
> | 뒤집힌 이유 | 안 C 의 「차수 상한」은 **렌더 규칙을 하나 더 늘리면서도 성게를 없애지 못한다.** 상한 12 를 걸어도 44차수 허브 3개가 여전히 레이아웃의 중심에 앉는다 |
> | 이 절이 안 B 의 대가로 적은 것 | 「노드 162개로 줄어 G3 의 「200+」에 미달」 — **그대로 실현됐다.** 실측 **노드 162 · 엣지 1,053**(`tests/atlas/build.test.ts` §실데이터). **G3 의 노드 하한은 미달을 감수한 결정이다** |
> | 실물 | `lib/atlas/build.ts` · `tests/atlas/build.test.ts` 「태그는 노드가 되지 않는다 — D-2」 |
>
> **절을 지우지 않는 이유** — §15 표가 번호로 참조하고, 뒤집힌 권고(안 C)와 그 근거가 남아야
> 나중에 「왜 상한 방식을 안 썼나」를 다시 묻지 않는다. 이 리포는 만료된 규칙을 번호째로 남긴다.
