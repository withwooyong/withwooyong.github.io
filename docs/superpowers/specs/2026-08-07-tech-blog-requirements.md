...
| CR-1.2 | **원본 메타 인용문(`> 작성 기준일 / 목적 / 원본`)은 본문에서 제거**한다 | **2026-08-18 자동 — 미구현.** 리터럴 패턴이라 본문 앞부분 grep으로 잡힌다. 실측: 미발행 원본 **141개**가 이 패턴을 갖고 발행본은 **0개**다. ⚠️ `source` 프론트매터는 2026-08-18 폐지됐으므로 **「frontmatter로 옮긴다」는 더 이상 유효하지 않다** |
...
| CR-2.3 | 답변마다 표·코드·도식 중 하나를 최소 1개 포함한다 — **2026-08-18 자동 — 미구현.** 대상은 `*qna*` 슬러그(발행본 13편, 예외 0건)로 잡고, `^## Q. ` 로 쪼갠 블록마다 표·코드펜스·mermaid 유무를 본다(현재 질문 251개). ⚠️ **`CR-2.6`의 「자주 묻는 질문」 H2 갈래는 아직 쓰인 적이 없다(0편). 검사기를 만들 때 그 형태도 덮어라** |
...
| ID | 보강 규칙 |
| --- | --- |
| CR-4.1a | H2 경계를 지키되 **목표 분량 15~25KB에 닿을 때까지 인접 H2를 묶는다.** 절 하나가 **25KB를 넘으면 H3 경계로 내려간다.** ⚠️ **2026-08-18 판정 — 하한만 자동, 상한은 사람이다.** 하한 13.3KB는 `tests/blog/content/schema.test.ts` 가 정본이고(금지선 11), **상한 25KB는 어떤 검사기도 보지 않는다** — 그 파일 `:15`가 「상한은 검사하지 않는다 — 금지선 13」이라 명시한다. **그래서 상한 수치는 여기가 정본이다** |
| CR-4.5 | 분할 후 **"도식과 그 설명이 같은 편에 있는가"를** 검사한다. 문서당 도식이 11~25개라 절 경계를 잘못 잡으면 갈라진다 |

> ⚠️ **`CR-4.1a`의 25KB와 금지선 13의 29.4KB는 충돌하지 않는다 — 다른 것을 잰다.**
> 둘을 한 덩어리로 보면 한쪽을 고칠 때마다 다른 쪽이 어긋나 왕복한다(금지선 29의 형태다).
>
> | | 금지선 13 | `CR-4.1a` |
> | --- | --- | --- |
> | 대상 | 이미 쓴 **발행본의 크기** | **분할할 때 어디서 자르나** |
> | 요구 | 「넘어도 된다. **크기를 이유로 내용을 빼지 마라**」 | 「절 하나가 25KB 넘으면 **H3 경계로 내려가라**」 |
> | 시점 | 발행 후 판정 | 분할 설계 시 |
>
> **13은 「줄이지 마라」이고 `CR-4.1a`는 「어디서 자르나」다.** 그래서 검사기가 하한만 보는 것이
> 옳다 — 상한을 HARD로 걸면 13을 어기게 된다.
