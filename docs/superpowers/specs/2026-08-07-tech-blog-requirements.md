...
| ID | 보강 규칙 |
| --- | --- |
| CR-4.1a | H2 경계를 지키되 **목표 분량 15~25KB에 닿을 때까지 인접 H2를 묶는다.** 절 하나가 25KB를 넘으면 H3 경계로 내려간다 |
...
| 슬러그 | 표시명 | 설명 | order | 편수 |
| --- | --- | --- | ---: | ---: |
| `ai-transformation` | AI 전환 조직 | AI를 개인기가 아닌 조직 역량으로 만드는 운영철학·역할설계·도입 로드맵 | 10 | 18 |
| `agentic-coding` | 에이전틱 코딩 | Claude Code로 짓는 개발 워크플로 — 컨텍스트 엔지니어링, MCP·Skills·Hooks, 멀티에이전트 | 20 | 12 |
...
| **CR-1.10** | 금칙어 때문에 **주체를 갈아 끼우지 마라.** 주체를 지우는 쪽이 없는 주체를 세우는 쪽보다 안전하다. 출처는 frontmatter `source`가 이미 담고 있다 |
