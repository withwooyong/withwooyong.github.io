# 차세대 CMS 재구축 — 프로덕트 로드맵

> **작성 기준일**: 2026-07-09
>
> **목적**: TVING「Platform Product Lead」공고의 **주요업무 1·2번**(콘텐츠·플랫폼 코어 엔진 로드맵 / 차세대 CMS 재구축 기획 총괄)에 대한 실행 설계. 면접에서 화이트보드 질문에 즉시 그림으로 답하기 위한 도식 중심 문서.
>
> **⚠️ 중요 — As-Is는 전부 "가설"입니다**: 이 문서의 현행(As-Is) 도식은 티빙 내부 자료가 아니라 **공개정보 + 일반적 OTT CMS 구조 + 작성자의 TVING 1세대·BTV NCMS 경험**에서 **추론한 가설**입니다. 면접에서는 반드시 "제 추정입니다. 부임 후 팀과 함께 진단하겠습니다"로 말합니다.
>
> **연계 문서**: [00_허브.md](00_허브.md) · [02_결제정산.md](02_결제정산.md) · [03_공통어드민.md](03_공통어드민.md) · [04_거버넌스_글로벌.md](04_거버넌스_글로벌.md) · [공고 분석](../posting_tving.md) · [면접 Q&A 102선](../tving_면접대비_PM관점.md)
...
| **MTS** | Media Transcoding System | 트랜스코딩을 수행하는 시스템 |
...
flowchart LR
    ...
    subgraph DOWN["CMS가 먹여살리는 하위 시스템"]
        C1["검색 엔진"]
        C2["추천 엔진"]
        C3["서비스 API"]
        C4["광고 시스템"]
...
> **⚠️ 가설입니다.** 아래 구성은 티빙 내부 문서가 아니라, ① 공개 기술블로그·채용공고에서 읽히는 조직 구조 ② 국내 OTT의 일반적 CMS 아키텍처 ③ 작성자가 TVING 1세대와 BTV NCMS에서 겪은 구조로부터 **추정**한 것입니다.
>
> 면접에서 이 도식을 쓸 때는 **"이건 제 가설이고, 진짜 그림은 부임 후 팀과 함께 그리겠습니다"** 를 반드시 붙입니다.

```mermaid
flowchart TB
    subgraph EXT["외부 소스"]
        E1["방송사/배급사<br/>파일 + 메타 엑셀"]
        E2["오리지널 제작"]
        E3["라이브/스포츠 피드"]
    end

    subgraph LEGACY["레거시 CMS - 가설"]
        L1["콘텐츠 등록 화면<br/>운영메뉴 다수"]
        L2["메타 편집 화면"]
        L3["편성 EPG 화면"]
        L4["카탈로그 배치 화면"]
        L5["권리/계약 화면"]
    ...
    subgraph STORE["저장소"]
        D1[("원본 RDB<br/>정합성 기준")]
        D2[("조회 저장소<br/>NoSQL 또는 검색엔진")]
    end

    subgraph INTEG["외부 연동 - 산재 가설"]
        I1["MTS 트랜스코딩"]
        I2["DRM 패키징"]
        I3["CDN 배포"]
        I4["자막 제작 외주"]
    ...
    style LEGACY fill:#fff4e6,stroke:#e8890c
    style D1 fill:#ffe6e6,stroke:#c92a2a
```

### 2-1. 이 가설이 참이라면 생기는 구조적 문제

| # | 문제 | 왜 아픈가 |
|---|---|---|
| P1 | **원본↔조회 저장소 배치 동기화** | 신작 노출이 배치 주기만큼 지연. 급편·긴급 수정이 즉시 반영 안 됨 |
| P2 | **외부 연동이 화면 로직에 흩어짐** | 트랜스코딩 실패 시 재시도·보상 로직이 없거나 사람이 처리. 실패가 조용히 묻힘 |
| P3 | **메타 등록이 수작업** | 카탈로그가 커질수록 인력이 선형으로 늘어남. 확장 불가 |
| P4 | **EPG와 카탈로그가 따로 논다** | 편성 변경이 노출에 자동 반영되지 않아 이중 입력 |
| P5 | **다국어가 후행 컬럼** | 글로벌 확장 시 스키마를 갈아엎어야 함 |
| P6 | **권리(라이선스)가 문서에 있고 시스템에 없음** | 만료 콘텐츠가 노출되는 사고 위험. 지역 제한 누락 |

> **면접 활용 포인트**: 이 표는 "레거시 분석을 어떻게 하시겠습니까"(Q23)의 답변 골격입니다.
>
> 문서·데이터·사람의 3층위로 파고들어, **"안 쓰는 기능을 그대로 옮기는 것"이 재구축 최대의 낭비**임을 전제로 무엇을 버릴지부터 정합니다.

---

## §3. Pain Point 검증 설계 — 부임 30일 안에 무엇을 실측하는가

가설을 정답처럼 다루지 않는다. **측정 → 확정 → 우선순위** 순서로 간다.

```mermaid
flowchart LR
    H["가설 6개<br/>P1~P6"] --> M1["작업 로그 계측<br/>2~4주"]
    H --> M2["운영자 인터뷰<br/>5개 직군"]
    H --> M3["기능 사용 로그<br/>메뉴별 접근 빈도"]
    ...
    style PR fill:#fff9db,stroke:#f08c00
```

### 3-1. 우선순위 매트릭스 — 무엇을 먼저 자동화할 것인가

계산식: **`영향도 = 월간 빈도 × 건당 소요시간 × 오류 발생 시 비용 계수`**

아래 수치는 **모두 예시(가정)**이며, 부임 후 실측으로 채운다.
...
> **읽는 법**: 좌상단(즉시 착수)은 난이도가 낮고 영향도가 큰 영역입니다. **권리 만료 추적·썸네일 자동 추출**이 첫 quick win 후보이고, **QC 검수**는 영향이 가장 크지만 난이도도 최상이라 Phase 2의 전략 과제로 미룹니다.
>
> 이 사분면은 30일 실측 후 점이 이동합니다. **지금의 배치는 가설입니다.**
...
| # | 원칙 | 구체적으로 |
|---|---|---|
| 1 | **원본과 조회를 분리하되 CDC로 잇는다** | 배치가 아니라 변경 로그 스트리밍. 조회 장애가 원본을 오염시키지 않음 |
| 2 | **편성(시간축)과 노출(공간축)을 분리하고 매핑 계층으로 연결** | EPG와 카탈로그는 목적이 다른 두 분류 체계 |
| 3 | **공통 메타 + 오버레이** | 언어·테넌트·지역별 차이를 컬럼 추가가 아닌 오버레이 행으로 흡수 |
| 4 | **AI는 초안, 사람은 승인 (HITL)** | 자동화율만 좇다 품질이 무너지지 않도록 승인 게이트를 남긴다 |
| 5 | **외부 연동은 워크플로우 엔진이 소유** | 재시도·보상·타임아웃을 화면 코드가 아니라 오케스트레이터가 책임 |
...
flowchart TB
    ...
    style AI fill:#f3f0ff,stroke:#7048e8
    style D1 fill:#ebfbee,stroke:#2f9e44
```

### 4-3. As-Is 가설 vs To-Be — 무엇이 달라지는가

| 축 | As-Is `가설` | To-Be | 근거 |
|---|---|---|---|
| 원본→조회 동기화 | 배치(주기적) | **CDC 스트리밍** | 노출 지연 제거. BTV에서 Kafka→ES 스트리밍 적재 경험 |
| 외부 연동 | 화면 코드에 산재 | **워크플로우 엔진 소유** | 재시도·보상·가시성 확보 |
| 메타 입력 | 사람 100% | **AI 초안 + 사람 승인** | 야나두 AX 방식 이식. 품질 게이트 유지 |
| EPG↔카탈로그 | 이중 입력 | **매핑 계층으로 자동 연결** | TVING 1세대에서 팀과 함께 만든 분리+매핑 구조의 계승 |
...
### 5-2. 설계 결정 3가지 — 왜 이렇게 나눴는가

**① `CONTENT_META`를 별도 테이블로 분리 (다국어 오버레이)**
...
또한 `source`(HUMAN / AI_DRAFT / MT)와 `approved` 플래그가 **AI 초안과 사람 승인을 같은 테이블에서 구분**하게 해준다. 이 두 컬럼이 §12의 자동화 처리율 지표를 계산 가능하게 만드는 핵심이다.

**② `SCHEDULE_SLOT`(편성)과 `CATALOG_NODE`(노출)를 분리하고 선택적으로 매핑**
...
### 6-1. 북극성 지표 — 수급→노출 리드타임을 단계별로 분해
...
### 6-3. 자동화 처리율 — AI 초안이 사람 수정 없이 승인된 비율

```sql
-- 무엇을 답하는가: "AI가 만든 초안 중 사람이 손대지 않고 통과시킨 비율은?"
-- 분모: AI 초안이 붙은 검수 과제 / 분자: 사람이 수정하지 않은 건
-- 주의: 이 값만 좇으면 검수를 대충 하게 된다 → 반드시 6-4(재작업률)와 함께 본다.
SELECT
    task_type,
...
### 6-4. 가드레일 — 노출 후 메타가 다시 수정된 콘텐츠 (재작업률)
...
### 6-5. 운영 조회 — 다국어 메타가 누락된 노출 콘텐츠

```sql
-- 무엇을 답하는가: "글로벌 서비스 중인데 해당 언어 메타가 없는 콘텐츠는?"
-- 글로벌 운영자의 대표적 pain point. 지금은 눈으로 찾는다(가설).
SELECT
    c.content_id,
    km.title                                             AS 한국어_제목,
...
### 6-6. 정합성 검증 — 라이선스가 만료됐는데 아직 노출 중인 콘텐츠

```sql
-- 무엇을 답하는가: "지금 이 순간 계약 위반 상태로 노출되는 콘텐츠가 있는가"
-- 사고가 나면 법무·계약 리스크. 재구축의 존재 이유 중 하나.
SELECT
    c.content_id,
    m.title,
    t.iso_code                    AS 지역,
...
> **면접 활용**: 이 쿼리 하나가 "권리 관리를 왜 시스템으로 옮겨야 하는가"를 설명합니다.
>
...
### 7-1. 시나리오 A — 배급사 콘텐츠 인제스트 (해피 패스 + 실패 보상)
...
sequenceDiagram
    ...
    Note over UI,DB: modified_by_human 값이<br/>§6-3 자동화율의 분자를 결정한다
...
### 7-2. 시나리오 B — 급편(急編) 대응: 편성 변경이 노출까지 자동 전파
...
sequenceDiagram
    ...
    alt 권리 없음 or 만료
        ...
        Note over SVC,SCH: 시스템이 계약 위반을 사전 차단<br/>As-Is에서는 사람이 기억해야 했음(가설)
    end
    RIGHTS-->>SVC: ALLOW
...
>
> 5번 — 권리 서비스가 편성 저장을 **거부**할 수 있다는 것. 사고를 사람의 기억이 아니라 시스템이 막습니다.
>
> 8번 — 편성 변경이 이벤트로 흘러 카탈로그에 자동 전파된다는 것. 이중 입력이 사라집니다.
...
### 9-1. 운영자 동선 (As-Is 가설 vs To-Be)

```mermaid
journey
    title 콘텐츠 운영자의 하루 - As-Is 가설
    section 오전
      배급사 메일에서 엑셀 다운로드: 2: MD
      엑셀을 CMS 양식으로 재가공: 1: MD
      콘텐츠 20건 수동 등록: 1: MD
    section 오후
      트랜스코딩 완료 여부 화면 새로고침: 2: MD
      실패건 담당자에게 메신저로 문의: 1: MD
      썸네일 하나씩 골라 업로드: 2: MD
      다국어 팀에 번역 요청 메일 발송: 2: MD
    section 저녁
      배치 반영 기다렸다 노출 확인: 2: MD
      오타 발견 - 재수정 후 재배치 대기: 1: MD
```

```mermaid
journey
    title 콘텐츠 운영자의 하루 - To-Be
    section 오전
...
>
> As-Is에서 1점이 찍힌 지점 — 엑셀 재가공, 수동 등록, 재수정 대기 — 이 곧 자동화 우선순위입니다.
>
> **자동화의 목표는 사람을 빼는 것이 아니라, 사람을 더 중요한 판단(검수·큐레이션)에 쓰는 것**입니다.
...
flowchart TD
    ...
    style R4 fill:#ebfbee,stroke:#2f9e44
    style R5 fill:#fff9db,stroke:#f08c00
    style J3 fill:#ffe6e6,stroke:#c92a2a
```

### 9-3. 시청자 동선 — CMS가 어디에 기여하는가
...
flowchart LR
    ...
    style CMSIMPACT fill:#e7f5ff,stroke:#1971c2
    style V7 fill:#ffe6e6,stroke:#c92a2a
```
...
### 10-1. CMS 코어 — 상용 헤드리스 CMS를 쓸 것인가
...
| **배치 ETL** | 분~시간 | 낮음 | 하 | ✕ (As-Is 문제 P1 그대로) |
...
| **기계번역 초벌** | DeepL / Google Translate / LLM | **바이** | TMS와 결합. 상세는 [번역관리 TMS 비교](../../learning/번역관리_TMS_솔루션_비교.md) |
| **품질 검수(QC)** | 자체 규칙 + LLM 판정 | **하이브리드** | 난이도 최상 → Phase 2 |

> **AI 원칙 — 드래프트는 AI, 승인은 사람.**
>
> 자동화율을 KPI로 걸면 조직은 반드시 검수를 대충 하게 됩니다. 그래서 자동화율 옆에는 **항상 재작업률·결함율이 짝으로 붙습니다**(§12).
>
...
flowchart TB
    subgraph PH0["Phase 0 - 진단"]
        direction LR
        P01["레거시 인벤토리"] --> P02["사용 로그 분석"] --> P03["버릴 기능 확정"]
    end

    subgraph PH1["Phase 1 - 파사드 세우기"]
        direction LR
        P11["API Gateway 앞에 세움"] --> P12["레거시로 전부 프록시"] --> P13["트래픽·에러 계측 시작"]
    end
    ...
    subgraph PH4["Phase 4 - 레거시 소멸"]
        direction LR
        P41["잔여 트래픽 0 확인"] --> P42["레거시 읽기 전용 전환"] --> P43["폐기"]
    end

    PH0 --> PH1 --> PH2 --> PH3 --> PH4

    style PH0 fill:#f8f9fa,stroke:#868e96
    style PH2 fill:#ebfbee,stroke:#2f9e44
    style PH3 fill:#fff9db,stroke:#f08c00
    style PH4 fill:#e7f5ff,stroke:#1971c2
```

### 11-2. 전환 중 트래픽 경로 (카나리)
...
flowchart LR
    ...
    MON -.임계 초과 시.-> RB["즉시 롤백<br/>라우팅 100% 레거시"]

    style NEW fill:#ebfbee,stroke:#2f9e44
    style RB fill:#ffe6e6,stroke:#c92a2a
```
...
gantt
    ...
    레거시 인벤토리·사용 로그 분석      :p02, 2026-09-01, 45d
    ...
    section Phase 4 소멸
    잔여 트래픽 정리·레거시 읽기전용    :p41, 2027-11-01, 60d
    레거시 폐기                        :milestone, m5, 2028-01-15, 0d
```
...
### 11-4. 각 Phase의 종료 조건 (Exit Criteria)

| Phase | 이걸 만족해야 다음으로 | 만족 못하면 |
|---|---|---|
| **0 진단** | baseline 지표 5종 측정 완료 + 버릴 기능 목록 합의 | 기간 연장. 측정 없이 설계 시작 금지 |
...
| **4 소멸** | 레거시 트래픽 0, 30일 유지 | 잔여 의존 추적 |
...
flowchart TD
    ...
    style BIZ fill:#f3f0ff,stroke:#7048e8
```
...
|---|---|---|---|
| **수급→노출 리드타임** | 북극성 | `published_at - acquired_at`, p50·p95 | §6-2 쿼리 |
| 단계별 소요시간 | 입력 | `ingest_step` 구간별 duration | §6-1 쿼리 |
| 자동화 처리율 | 입력 | 분모=AI초안 건 / 분자=사람 무수정 승인 건 | §6-3 쿼리 |
| 운영자 1인당 처리량 | 입력 | 월 처리 콘텐츠 수 ÷ 운영자 FTE | 작업 로그 |
| 셀프서비스 비율 | 입력 | 개발 티켓 없이 운영자가 완결한 작업 비율 | 티켓 시스템 조인 |
| **재작업률** | 가드레일 | 노출 1시간 후 메타가 수정된 콘텐츠 비율 | §6-4 쿼리 |
| **권리 위반 노출** | 가드레일 | 만료 후에도 노출 중인 콘텐츠 수 (목표: 0) | §6-6 쿼리 |
| 인제스트 실패율 | 가드레일 | 재시도 소진으로 REJECTED된 비율 | 상태기계 |

### 12-3. 중단 기준 (미리 정해두는 것)
...
>
> 상세 대본은 [면접 Q&A 102선](../tving_면접대비_PM관점.md) 그룹 3·4를 참조하세요.

**Q. 차세대 CMS, 어떻게 접근하시겠습니까?** `★★★`

> **"세 단계로 봅니다 — 진단, 지표 정의, 점진 전환입니다."**
>
> 먼저 현행 리드타임·수작업 비율·연동 실패율을 실측해 baseline을 만듭니다(§3). 다음으로 성공을 '수급→노출 리드타임'이라는 하나의 북극성으로 정의합니다(§12).
>
> 그리고 재구축은 빅뱅이 아니라 strangler 방식으로, 권리·카탈로그 같은 저위험 도메인부터 감싸며 대체합니다(§11). 재구축 중에도 수급과 편성은 하루도 멈출 수 없으니까요.
...
>
> 콘텐츠 타입마다 메타 구조가 달라서 공통 필드는 컬럼으로, 이질적인 부분은 JSONB로 흡수합니다. 다국어는 컬럼이 아니라 오버레이 행으로 두어 언어 추가가 데이터 삽입이 되게 합니다(§5-2).
>
...
>
> 우선순위는 감이 아니라 '빈도 × 건당 소요시간 × 오류 비용' 매트릭스로 정합니다(§3-1). 지금 제 가설로는 권리 만료 추적과 썸네일 자동 추출이 quick win이고, QC 검수는 영향은 가장 크지만 난이도도 최상이라 뒤로 뺍니다.
>
...
>
> API Gateway 뒤에서 신규 서비스로 트래픽을 5%부터 넘기고, 오류율이 레거시 대비 1.5배를 넘으면 즉시 100% 레거시로 되돌립니다(§11-2, §12-3).
>
...
>
> 권리 윈도우·편성-카탈로그 매핑·승인 게이트는 미디어 CMS의 본질이라 자체 구축합니다. 범용 헤드리스 CMS(Contentful·Strapi)에는 이 개념이 아예 없어서, 결국 그 위에 우리 도메인을 다시 얹게 됩니다(§10-1).
>
...
## §14. 이 문서의 한계 (스스로 밝히는 것)

정직함이 신뢰를 만든다. 면접에서 먼저 말하면 오히려 강해진다.

| 한계 | 어떻게 다룰 것인가 |
|---|---|
| As-Is는 전부 **추정**이다 | 부임 30일 실측으로 대체. 도식은 폐기 가능한 가설 |
