...
발행본 **184편**이다. 배치는 편8로 끝났고 그 뒤 열네 세션이 검사기의 사각지대를 닫아 왔다.
이번 세션도 콘텐츠와 코드를 고치지 않았다. 앞선 세 세션이 `slidev-poc/` 에서 손으로 해 본
검증을 **문서 한 편으로 모으는 것**이 이번 일이었고, 모으는 과정에서 결함 둘이 더 나왔다 —
둘 다 브라우저가 아니라 **파서가** 냈다.
...
| ⚠️ CI 스텝 수를 세는 법 | `gh run view --json jobs` 는 `build` 를 **27스텝**으로 보고한다. 러너가 `Set up job`·`Post Setup Node`·`Post Checkout`·`Complete job` **넷**을 붙이기 때문이다. 문서의 **23** 은 워크플로 정의의 수이며 둘 다 맞다 — **다른 수를 대조하고 「문서가 낡았다」고 판단하지 마라.** 🔴 **이번 세션이 정확히 그 실수를 했다** — 27 을 보고 「23 은 낡았다」고 사용자에게 보고했다가 이 행을 읽고 되돌렸다. 이 행이 없었다면 멀쩡한 문서를 고쳤을 것이다 |
| 강조 렌더 실패 | ✅ 발행본 **0회 · 184편** · 리포 문서 **0회 · 64개** |
| 깨진 링크 | ✅ 발행본 **0곳** · 리포 문서 **0곳** |
| 깨지는 도식 | ✅ 발행본 **0곳**(544개) · 리포 문서 **0곳**(130개) — **판정 도달 전량** · 합계 **674개** |
| 🔴 표의 열 수 | ❌ **어느 검사기도 보지 않는다.** 이번 세션에 셀 3개짜리 행이 **6개로 갈라진** 자리가 둘 있었고 마크업·링크·도식 검사 셋이 전부 통과시켰다. 아래 「반드시 먼저 알아야 할 것」을 보라 |
| 도식의 의미 | ✅ 조사를 마쳤고 검사기를 만들지 않기로 판정했다. 다시 꺼내려거든 아래 판정 절의 셋을 먼저 반박하라 |
| 발표 슬라이드 | ✅ 🆕 **형식 검증과 발행본 이식을 마쳤다.** `slidev-poc/` 에 Slidev **52.19.1** · `slides.md` **7장**(기능 검증) · `slides-es.md` **20장**(발행본 한 편 이식). **기능 일곱은 전부 동작하고 걸리는 것은 분량과 링크다.** 아래 「이번 세션이 한 일」을 보라 |
...
| 분량 하한 SOFT | **3편** — 남은 셋은 **정당하다고 판정했다.** 아래 판정 표를 보라 |
| 커밋 | 이번 세션은 `08925ee`(조사 문서 한 편 + `CLAUDE.md` 수치) · `bd4a7fc`(`main` 병합 · 도식 수 실측) · `8b884e3`(리뷰 교정 — 표 깨짐과 배율 오기)를 PR [#19](https://github.com/withwooyong/withwooyong.github.io/pull/19) 에 실었다. 그 앞에 앞선 세 세션의 `d383383` · `0b5d8cc` · `6385d09`(`slidev-poc/`)가 같은 브랜치에 있다. **콘텐츠와 코드는 건드리지 않았다** |
| 푸시 · 배포 | ✅ **`research/presentation-format` 을 푸시하고 PR [#19](https://github.com/withwooyong/withwooyong.github.io/pull/19) 을 열었다.** 🔴 **첫 PR 은 `CONFLICTING` 이라 CI 가 아예 돌지 않았다** — `main` 을 병합해 풀었다. 그 뒤 PR 단계 CI 는 run `34029167076`(2분 22초)과 `34030483379`(2분 18초) 둘 다 **success** 이며 `build` 성공 · `deploy` 와 `Upload artifact` 는 설계대로 **skipped** 다. ⏳ **merge 는 아직 하지 않았다. 사용자 승인이 필요하다** — merge 커밋과 배포 run 은 뒤이은 세션이 채운다 |
| 직전까지의 푸시 · 배포 | ✅ **PR [#18](https://github.com/withwooyong/withwooyong.github.io/pull/18) 이 merge 커밋 `0a0583d` 로 닫혔다.** 그 앞은 PR [#17](https://github.com/withwooyong/withwooyong.github.io/pull/17) → merge `572a849` · 배포 run `34018339741` **success**(123초), PR #16 의 run `34011795806`(130초), PR #15 의 run `34007702350`(150초), PR #14 의 run `34006877177`(150초), PR #13 의 run `34004539240`(153초)였다. 머지 방식은 **merge 커밋**이다: PR #11~#18 이 전부 부모가 둘이며, 이 리포는 squash 를 쓴 적이 없다 |
| 프로덕션 실측 | ✅ **카테고리 목록과 본문이 서로 다른 그래프를 그린다는 것을 화면에서 확인했다.** `/blog/ai-agent/` 는 앵커 **13개**(중심 1 + 이웃 12) · 중심이 **링크**(`/blog/ai-agent/langgraph-state-reducer/`) · 연결선 **36개** · 캡션 **두 줄 48px** 로 잘리지 않았고, 중심거리 표준편차 **15.72** · 이웃 간 최소 간격 **23.86** 이었다. 본문 페이지는 앵커 **7개** · 중심이 링크가 **아니고** 캡션 **32px** 여서 둘의 문맥이 섞이지 않았다 |
| 브랜치 | `research/presentation-format` 이다. 이 브랜치에는 `slidev-poc/`(node_modules 와 산출물은 `.gitignore`)가 들어 있다. 병합이 끝난 `docs/pr16-deploy-record` 는 **아직 지우지 않았고** 로컬과 원격 양쪽에 남아 있다. 🔴 **원본 작업 트리(`withwooyong.github.io`)와 이 작업 트리는 같은 리포를 공유한다** — 이번 세션 중에 원본의 브랜치가 `content/pm-po-domains` 로 바뀌어 있는 것을 보았다. 다른 세션이 갈아탄 것이므로 **브랜치를 만들 때 기점을 명시하라** |
| 다음 단계 | 아래 **「다음 세션의 작업」** — 우선순위 표를 먼저 보라 |

### 이번 세션이 한 일 — 세 세션의 Slidev 검증을 조사 문서 한 편으로 모았다
...
| **10** | 발표본을 **몇 편 더** 만든다 | 중간 | 낮음 | **필요** | ⬜ 넘침 검사기를 만들지 말지는 여기서 갈린다. 표본이 20장뿐인 지금은 0건이 참인지 알 수 없다. **어느 편을 슬라이드로 만들지 사용자가 정해야 착수할 수 있다** |
...
| **6** | 시리즈 A·B 밖의 **새 배치** | 매우 높음 | — | **필요** | 🎯 남은 큰 항목이다. 주제와 분량을 사용자가 정해야 착수할 수 있다 |
...
| 4 | **검사기가 무력해진다** | `check-mermaid` 가 발행본 **544개** · 문서 **130개**를 파서로 판정한다. HTML+SVG 로 바꾸면 그 674개가 **아무도 보지 않는 자리**가 된다. 대체 검사기부터 있어야 한다 |
...
2026-09-06 에 **표의 열 수**가 새 후보로 올라와 남은 것은 셋이며,
**기계만으로 닫을 수 있는 자리는 없다.**
...
| 🆕 **표의 열 수** | 위반 **2곳** (2026-09-06 실측 · 문서 한 편) | ⚠️ **어느 검사기도 보지 않는다.** 셀 3개짜리 행이 6개로 갈라져도 마크업·링크·도식 검사가 전부 통과시킨다. **표본이 한 문서뿐이라 지금은 검사기를 만들지 않는다** — 위 0-c 를 보라 |
| **도식의 색 — 실물 대조** | 도식 **674개** 중 브라우저로 잰 것은 **10개** | ⚠️ 팔레트 계산은 전량에 걸리지만, 「테마 변수가 닿지 않는 색」은 **렌더해 봐야** 드러난다. 이번 세션이 그렇게 하나를 찾았다. 남은 종류(`gantt`·`journey`·`erDiagram`·`quadrantChart`·`mindmap`)는 **문서에만 있어 발행되지 않으므로** 우선순위가 낮다 |

⚠️ **남은 셋은 사람의 판단이나 실물 확인을 먼저 요구한다.** 새 배치는 주제와 원본 위치를
사용자가 정해야 하고, 도식의 색은 브라우저로 재야 드러나며, 표의 열 수는 표본이 더 쌓여야
검사기를 만들지 말지를 판단할 수 있다.
