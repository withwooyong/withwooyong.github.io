...
>
> 🔴 **다음 세션의 임무는 하나다 — 원계획의 잔여분 `T14`·`T15` 를 끝까지 만든다.**
> 정본은 [`docs/superpowers/plans/2026-08-25-redesign-phase-1-2.md`](docs/superpowers/plans/2026-08-25-redesign-phase-1-2.md) 이고,
> **`T9`~`T13` 은 끝났다.** 남은 것은 **2개**다(§2).
>
> 🔴 **T14 에 착수하기 전에 [`reports/2026-08-28-t13-stubs.md` §R49](docs/superpowers/reports/2026-08-28-t13-stubs.md) 를 먼저 읽어라.**
> 계획서 §T14 의 삭제 목록이 **T11 이전에 쓰여 살아 있는 파일을 포함한다.** 그대로 실행하면
> `/work` 가 빌드 불가가 된다. 계획서에 취소선으로 봉인해 뒀으니 **그 표시를 지우지 마라.**
>
...
| **14** | 고아 자산 삭제 + 기준선 1회 갱신 | **4개** 제거(5개가 아니다 — 아래 🔴) | ❌ 미착수. **`--update` 는 여기서만** |
| **15** | Lighthouse CI — 경고로만 | 워크플로 단계 | ❌ 설정·워크플로 둘 다 없음 |

**순서를 바꾸지 마라.** ~~9~~ → ~~10~~ → ~~11~~ → ~~12~~ → ~~13~~ → **14** → 15.
T14 는 T13 이 라우트를 지워야 호출자 0건이 증명된다 — 그 선행 조건은 이제 **충족됐다.**

> ### 🔴 **T14 의 삭제 목록은 틀렸다 — 그대로 실행하면 `/work` 가 빌드 불가가 된다**
>
