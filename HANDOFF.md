...
| **B8** | **Q&A 4편** | **Q1~Q4** | **4** | **다음** — `05 §10-1` 10문항 이월 · **`ai-agent` 태그 몫 없음** |
| B9 | 지식 지도 | — | 1 | **맨 마지막** |
...
| # | 사항 | 근거 |
| --- | --- | --- |
| 1 | **`ai-agent` 태그 한도 7/7 소진 — Q3 몫이 없다** | 아래 「B8이 판단해야 할 것」. **B7 4편은 `ai-agent`를 쓰지 않아 7/7이 유지됐다** |
...
**B7 4편은 `ai-agent`를 쓰지 않아 7/7이 그대로 유지된다 — 선택지는 아래 셋 그대로다.**

| 선택지 | 대가 |
| --- | --- |
| ① Q3에 달지 않는다 | 설계서 배정 위반이지만 기발행본을 건드리지 않는다. **B6·B7은 이것을 전제로 진행했다** |
...
**B7에서 8건을 정정했다. 전부 설계서 본문에 ⚠️ 블록으로 삽입돼 있다.**
...
| 18 | **`ai-agent` 태그 한도 소진** | **B6 신규.** 위 §2의 B8 판단 사항. **B7은 `ai-agent`를 쓰지 않아 7/7이 유지된다** |
...
**`ai-agent`는 7편 한도이며 B6에서 정확히 7/7에 도달했다 — 여유 0. B7 4편은 쓰지 않아 그대로다.**
