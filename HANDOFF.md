...
| **5** | 도식의 의미 검사 **662개** | 매우 높음 | 중간 | 받았다 | 🎯 **다음 세션의 작업.** 사용자가 **「먼저 실태만 조사한다」를** 골랐다. 아래 착수 절차를 그대로 따르라 |
...
| **8** | 지역 그래프를 **카테고리 목록에도** 띄운다 | 낮음~중간 | 낮음 | **필요** | 🆕 미착수 · 사용자가 제기했다 (2026-09-06). 아래 **「지역 그래프가 본문에만 나오는 이유」를** 보라 |

### 🆕 지역 그래프가 본문에만 나오는 이유 — 화면 높이가 아니라 **데이터가 없다**

사용자가 「카테고리 목록에도 그래프가 보이게 하려면 어떻게 하는가」를 물어 조사했다
(2026-09-06). **원인을 화면 높이(`tall`)로 오해하기 쉬우니 아래를 먼저 읽어라.**
...
| 안 | 무엇 | 고칠 자리 | 값어치 |
| --- | --- | --- | --- |
| **A. 대표 편을 중심으로** | 그 카테고리에서 피인용이 가장 많은 편을 중심으로 삼아 기존 함수를 그대로 쓴다 | 페이지 한 곳에서 `getLocalGraph` 를 부르면 끝난다 | 가장 싸다. 다만 「이 카테고리의 지도」가 아니라 **「그 한 편의 이웃」이라서** 사용자가 기대한 것과 다를 수 있다 |
...
```
발행본 mermaid 도식 544개 (리포 문서 118개는 별도)
