...
> 마지막 갱신 2026-09-09 · `pm-practice` 배치 **편6/8 발행** · 발행본 **190편** ·
> **초과의 원인을 대조군이 갈랐다.**
>
...
발행본 **190편**이다. 앞 배치가 편8로 끝난 뒤 열네 세션이 검사기의 사각지대를 닫았고, 그 뒤
설계서 · 편1 · 편2 · 편3 · 편4 · 편5 · **편6** 이 이어졌다. 남은 것은 편7 · 편8 **두 편**이다.

✅ **§3-2 의 예산표가 네 편 연속으로 지지되었다.** 역산 k 가 편3 2.050 · 편4 2.033 · 편5 2.076 ·
편6 **2.059** 로 **0.043 폭 안**에 모였고 넷 다 밴드(1.81 ~ 2.09) 안이다. 편2 에서 고친 고정 비용
3,746 을 유지하므로 **편7 의 목표는 22,953**(밴드 21,578 ~ 24,328)이고 재환산하지 않는다.
...
⇒ **편7 · 편8 은 초고를 쓸 때 원본이 짧은 절을 먼저 표시해 두어라.**

| 항목 | 값 |
| --- | --- |
| 발행 완료 | **A 편1~9 + B 편1~편8 + `pm-practice` 편1~편6 (190편)** · 새 시리즈 **6/8** |
...
| 검색 인덱스 | ✅ `out/blog/search-index.json` · 🆕 **190편 · 헤딩 2,743개 · 321 KB** (2026-09-09 빌드 실측 · 편6 포함). 산출물 금칙어는 🆕 **547개 파일에서 HARD 0회**이고 sitemap 은 🆕 **277개 URL** 이다. 자기 검사 **16/16** |
...
### 이번 세션이 한 일 — 편6 을 발행하고 규칙 16 을 다시 발동했다
...
| **6-b** | 위 배치의 **집필 8편** | 매우 높음 | — | **설계서 승인** | 🎯 **편6 발행 완료 (6/8).** 다음은 **편7**(`global-collaboration-process`)이며 원본 `10-글로벌-협업` 의 §1~§4 다. **목표 22,953**(밴드 21,578 ~ 24,328) — 네 편이 상수를 지지했으므로 §3-2 의 값이 그대로 유효하다. 🔴 **발행할 때 자기 파일만 고치지 마라** — 지도편 편 목록 표의 자기 행을 링크로 바꾸고 **앞 편의 마지막 예고 문장**에 자기 링크를 걸어야 한다 (설계서 §10-4-b · 편5 · 편6 이 따랐다). 🔴 **§6-2 의 편7 접합부가 아직 비어 있다** — 「도메인 편 가운데 글로벌을 다룬 편」이라고만 적혀 있으니 **낱말 실재를 `grep` 으로 확인하고 행 번호를 채워라.** 편6 에서 이 확인이 실제로 발동했다: 초판이 지목한 낱말이 대상 편에 **0건**이었고 실제로는 다른 낱말을 쓰고 있었다. 🔴 **원본이 짧은 절을 초고 전에 표시하라** — 편6 초과의 절반이 그 자리에서 나왔다 (원본 372 B 에 배율 4.301) |
