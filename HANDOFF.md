...
> 마지막 갱신 2026-09-16 · 🔍 **발행본 도식 566개의 품질을 조사했다** · 발행본 **193편**
>
...
| 🆕 **15** | **후보 D** — Mermaid 566개에 `classDef` 강조와 `direction` 을 넣는다 | 낮음 | 낮음 | **약간** | 🎯 **사용자가 지적한 품질 문제의 해법이다.** 강조는 **566개 중 0개**, 방향 대상은 **137개**다. B 와 같은 성격이라 콘텐츠를 바꾸지 않고 검사기도 그대로 산다. ⏳ **몇 개에 넣을지가 결정 사항** — 전부인가, 강조할 노드가 있는 것만인가 |
| 🆕 **16** | **SVG 내부 텍스트 검사기** — 금칙어 사각지대 | 낮음 | 중간 | 불필요 | 🔴 **배포된 SVG 안의 문자열은 금칙어 검사를 한 번도 받지 않았다.** 지금은 대상이 하나뿐이라 위험이 낮지만, 14번·15번 뒤에 SVG 가 늘면 같은 배수로 커진다. **15번을 먼저 하면 이 항목의 시급성이 낮아진다** |
