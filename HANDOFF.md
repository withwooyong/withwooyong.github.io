...
| 표의 열 수 | ✅ 🆕 **`check-table` 이 본다.** 리포 전량 실측에서 표 **3,778개** 중 어긋난 행 **4곳**(발행본 2 · 문서 2)을 찾아 넷 다 고쳤다 — 기전 넷 중 **셋이 코드 스팬 안의 세로줄**이었고 처음 지목한 `sed` 백슬래시는 **0건**이었다. 자기 검사 22건 · 뮤턴트 TB1~TB7 · 훅과 CI 양쪽에서 돈다 |
