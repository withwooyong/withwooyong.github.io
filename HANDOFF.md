...
> 마지막 갱신 2026-09-16 (2차) · ⛔ **후보 D 를 사용자 판정으로 중단했다** · 🎯 **사용자 요청 넷을 17~20번으로 넘긴다** · 발행본 **193편**
>
> **이번 세션(2026-09-16, 2차) —** 15번(후보 D)을 브랜치 `feat/diagram-emphasis-direction` 에서
...
| 🆕 **16** | **SVG 내부 텍스트 검사기** — 금칙어 사각지대 | 낮음 | 중간 | 불필요 | 🔴 **배포된 SVG 안의 문자열은 금칙어 검사를 한 번도 받지 않았다.** 지금은 대상이 하나뿐이라 위험이 낮지만, 14번·15번 뒤에 SVG 가 늘면 같은 배수로 커진다. **15번을 먼저 하면 이 항목의 시급성이 낮아진다** · 🔼 **2026-09-16(2차): 15번이 중단되고 17번이 SVG 를 늘리므로 시급성이 다시 올랐다. 17번과 같은 PR 에 싣는 것을 권한다** |
| 🆕 **17** | **`## 4. 승인 지점을 하나로 모은다` 도식을 `diagram-design` SVG 로 다시 만든다** | 중간 | 낮음 | **실물 확인** | 🎯 **사용자 원문:** 「svg 이미지로 만들어진 한눈에 보기 아래 도식은 다크/화이트 모드 모두 인상적이었어. 근데 4. 승인 지점을 하나로 모은다 아래 도식은 색생만 조금 바뀌고 … 이전과 다를바가 없어」. **좌표:** `content/blog/agentic-coding/human-gate-and-model-routing.md` 183행 · 기준 자산 `public/images/blog/multi-agent-system.svg`(`64861c7b`) · 렌더는 `components/diagram-image.tsx`. **순서:** 이 도식 **하나만** 만들고, 사용자가 직접 여는 **링크**로 라이트 · 다크를 보인 뒤에 넓힌다. ⚠️ **14번과 어긋나는 점:** 1차 조사는 이 SVG 가 라이트 모드에서 검은 상자로 남는다고 적었는데 사용자는 두 모드 모두 인상적이라고 판정했다 ⇒ 적어도 이 SVG 의 고정 색은 사용자 기준으로 결함이 아니다. 새 SVG 도 같은 방식으로 만들되 **도식의 뜻과 본문 글자는 바꾸지 않는다.** 🔴 16번(SVG 텍스트 금칙어)을 같은 PR 에 싣는다 |
| 🆕 **18** | **확대 뷰어가 도식을 화면 크기에 맞춰 연다** | 낮음 | 중간 | 불필요 | 🎯 **사용자 원문:** 「크게보기는 기능을 눌렀을 때 화면사이즈에 맞게 도식이 나와야 되는데 엄청 크게 나오는 경우가 있어」. **좌표:** `components/diagram-image.tsx`(SVG) · `components/mermaid.tsx`(Mermaid). 🔎 **가설(미검증):** `diagram-image.tsx` 17행 주석이 「scale 1 = 컨테이너 **폭**에 꼭 맞춤」이고 되돌리기 버튼 라벨도 「화면 **폭**에 맞추기」다 — 폭만 맞추므로 세로로 긴 도식은 높이가 화면을 넘는다. 폭 배율과 높이 배율 중 작은 쪽(contain)으로 여는 것이 맞는지, **세로로 긴 도식과 가로로 긴 도식 둘로 먼저 재현**하라. ⚠️ `components/system-diagram-card.tsx` 에도 「크게 보기」가 있다 — 비블로그 페이지를 건드리면 `check-baseline`(GC-6)이 빨간불이 되므로 범위에 넣을지 먼저 본다 |
| 🆕 **19** | **확대 화면에서 다시 클릭하면 닫힌다** | 낮음 | 낮음 | 불필요 | 🎯 **사용자 원문:** 「크게보기화면에서 다시 클릭하면 팝업 화면 닫히게 해줘」. **좌표:** 18번과 같은 두 컴포넌트의 `Dialog`. ⚠️ 확대 뷰어는 이동(드래그 · 핀치)을 지원하므로 클릭과 겹친다. **권장안:** 움직이지 않은 탭 · 클릭만 닫고, 이동 뒤에 손을 뗀 것과 툴바 버튼 클릭은 닫지 않는다. 실물에서 이동 · 확대 · 닫기를 각각 확인한다 |
| 🆕 **20** | **캡션의 「도식」을 도식 종류 이름으로 바꾼다** | 낮음 | 낮음 | 불필요 | 🎯 **사용자 원문:** 「도식 하단에 "도식을 탭하면 확대해서 볼 수 있습니다" 부분에 "도식"이라는 문장을 좀더 명확하게 … 플로우차트, 간트차트, 등등 도식의 이름으로 변경해주면 좋겠어」. **좌표:** `components/mermaid.tsx` 185행 · `components/diagram-image.tsx` 85행(캡션), `mermaid.tsx` 163행(`aria-label` 「도식 크게 보기」). Mermaid 는 첫 선언(`flowchart` · `sequenceDiagram` · `stateDiagram-v2` · `gantt` …)에서 종류를 얻는다. 🔴 **발행본에 실제로 쓰인 종류는 `check-mermaid.mjs` 의 `extractDiagrams` 로 세어** 이름 표를 만든다 — 정규식으로 세지 마라(함정 56·58). SVG 이미지는 선언이 없으므로 이름을 어디서 얻을지 정한다(권장안: 모르면 한 가지 이름으로 둔다) |
