...
> 갱신: 2026-08-28 · 브랜치 `main` = `63d7de7` (배포 완료) · 직전 세션 커밋 **5개**(전부 문서)
>
> 🔴 **다음 세션의 임무는 하나다 — 원계획의 잔여분 `T9~T15` 를 끝까지 만든다.**
> 정본은 [`docs/superpowers/plans/2026-08-25-redesign-phase-1-2.md`](docs/superpowers/plans/2026-08-25-redesign-phase-1-2.md) 이고,
> **7개 전부 미착수**임을 파일 실재로 확인했다(§2).
>
...
| --- | --- | --- | --- |
| **9** | 히어로 B — 아틀라스 점등 | `components/hero.tsx` · `hero-atlas.tsx` · `lib/use-scroll-progress.ts` | ❌ 3개 다 없음. **`git log --all` 로도 커밋 0건** — 구현했다 이월된 게 아니라 커밋되지 않은 채 사라졌다 |
...
| 순서 | 무엇 | 왜 |
| --- | --- | --- |
| 1 | 계획서 `§Task 9` (L1501~1796) | 브리프 정본 |
| 2 | 계획서 `§Global Constraints` (L17~70) | **GC-9 정의를 바꿔야 하므로 원문이 필요하다** |
| 3 | `.superpowers/sdd/2026-08-25-redesign-phase-1-2/task-7*-report.md` | 셸이 히어로의 상단 제약이다 |
