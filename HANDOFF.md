...
> 발행했다. 원본 `11-데이터-드리븐` 의 §1~§4 를 배정받아 데이터가 필요한 이유 · 정량과
> 정성의 분업 · 퍼널 지표의 정의 · 정성 데이터를 모으는 다섯 방법을 다룬다.
>
...
발행본 **189편**이다. 앞 배치가 편8로 끝난 뒤 열네 세션이 검사기의 사각지대를 닫았고, 그 뒤
설계서 · 편1 · 편2 · 편3 · 편4 · **편5** 가 이어졌다. 남은 것은 편6~편8 **세 편**이다.

✅ **§3-2 의 예산표가 세 편 연속으로 지지되었다.** 역산 k 가 편3 2.050 · 편4 2.033 · 편5 **2.076**
으로 **0.043 폭 안**에 모였고 셋 다 밴드(1.81 ~ 2.09) 안이다. 편2 에서 고친 고정 비용 3,746 을
유지하므로 **편6 의 목표는 20,009**(밴드 18,845 ~ 21,172)이고 재환산하지 않는다.
⚠️ **편5 의 밴드 상단 여유는 121 B 였다** — 편4 의 436 B 보다 좁으므로 편6 에서는 복제 수정의
방향을 정할 때 길이를 함께 보라.

| 항목 | 값 |
| --- | --- |
| 발행 완료 | **A 편1~9 + B 편1~편8 + `pm-practice` 편1~편5 (189편)** · 새 시리즈 **5/8** |
...
| 🆕 편5 의 푸시 · CI | ✅ **PR [#24](https://github.com/withwooyong/withwooyong.github.io/pull/24) 를 열었고 PR 단계 CI run `34294874618` 이 success 였다** (`build` **success · 30스텝** — 워크플로 정의의 26 에 러너가 넷을 붙인 수다. `Upload artifact` 와 `deploy` job 은 설계대로 skipped). `mergeable` 은 `MERGEABLE` 이고 `gh run list --branch` 로 **run 이 실제로 생성된 것**을 따로 확인했다. ⏳ **merge 는 아직 하지 않았다 — 승인 대기 중이다** |
...
### 이번 세션이 한 일 — 편5 를 발행하고 상수를 세 번째로 지지했다

원본 `12-팀빌딩` 의 §1~§3 을 배정받아 편5
[`team-building-and-agile-org`](content/blog/product-management/team-building-and-agile-org.md) 를 냈다.
팀과 팀 빌딩의 정의 · 좋은 팀과 나쁜 팀을 가르는 축 · 애자일 조직과 그것이 국내에서 어려운
이유가 내용이고, 직무 · 직급 · 직책은 설계서 §4-1 대로 편6 에 남겼다.

| 항목 | 값 |
| --- | --- |
| 목표 B | 20,004 (밴드 18,840 ~ 21,167) |
| 초고 B | **21,021** — 목표 대비 **+5.1%** 로 규칙 16 의 10% 문턱을 넘지 않았다 |
| 최종 B | **21,046** — 복제 수정이 늘린 폭은 **+25 B** 다 |
| 역산 k | **2.076** — 편3 2.050 · 편4 2.033 과 **0.043 폭 안**이고 셋 다 밴드 안이다 |
| 밴드 상단 여유 | **121 B** — 편4 의 436 B 보다 좁다 |

⇒ **편6~편8 의 예산을 재환산하지 않는다.** §3-2 의 값이 그대로 유효하며 편6 의 목표는
**20,009**(밴드 18,845 ~ 21,172)다. 설계서 §3-4 의 재환산 지점은 둘 다 닫혔다.

#### 🔴 `source-overlap` ①의 건수를 앞 편과 비교하지 마라 — 성격이 다르면 처분이 정반대다

초고에서 ①(공백 보존)이 **6건 · 최장 24자**를 냈다. 편4 의 11건과 자릿수가 비슷하지만 판정은
반대였다 — 편4 는 바꿀 수 없는 **수치 행**이었고 이번은 전부 **서술 문장**이었다. 여섯 자리를
모두 고쳐 썼고 ①②③ 이 전부 0건이 되었다.

🔴 **그중 하나가 절 제목이었다.** 「성과가 좋아도 신뢰가 없으면 운으로 보인다」가 원본 표의
판정 문장을 그대로 옮긴 것이었는데, **제목이라서 본문을 읽을 때 자기 문장으로 읽힌다.**
「신뢰가 없으면 같은 결과도 운으로 읽힌다」로 바꿔 닫았다. ⇒ **원본 대조의 걸린 자리가
제목이면 본문보다 눈에 띄지 않는다는 것을 전제하고 보라.**

#### 산출과 검증

| 항목 | 값 |
| --- | --- |
| 커밋 | `b5b5ca0` — 발행본 1편 + 지도편 · 편4 링크 + 개수 네 자리 + 설계서 §10-5 + 이 문서 |
| PR | [#24](https://github.com/withwooyong/withwooyong.github.io/pull/24) · `MERGEABLE` |
| PR 단계 CI | run `34294874618` **success** · `build` 30스텝 · `deploy` 와 `Upload artifact` 는 설계대로 skipped |
| 로컬 빌드 | **78.5초** · sitemap **276 URL** · 검색 인덱스 **189편 · 헤딩 2,733개 · 320 KB** |
| 산출물 금칙어 | **545개 파일 · HARD 0회** |

🔴 **이 워크트리의 `out/` 은 여전히 EBUSY 로 잠겨 있다.** 빈 디렉터리인데 `rm -rf` 가
`Device or resource busy` 를 낸다. 빌드는 `git worktree add --detach` 로 임시 워크트리를 띄워
했고, `node_modules` 는 PowerShell 의 `New-Item -ItemType Junction` 으로 연결한 뒤 **패키지 수를
원본과 대조**(578 = 578)해 실제로 만들어진 것을 확인했다. 끝나고 `cmd /c rmdir` 로 링크만 지운
뒤 `worktree remove --force` 로 정리했다.

#### 🔴 검사기 다섯이 통과시키는 세 번째 링크 자리가 있었다

§10-4-b 가 지목한 두 곳(지도편의 편 목록 표 · 편4 의 마지막 예고 문장)은 발행과 같은 커밋에서
채웠고 고립 검사가 200케이스 전부 통과했다. **그런데 초고에는 자리가 하나 더 있었다** — 도입부
에서 편6 을 가리키려다 주소를 지도편으로 걸어 둔 링크다.

| 무엇이 문제인가 | 왜 검사가 잡지 못하나 |
| --- | --- |
| 「편6」이라 적힌 링크가 지도편으로 간다 | 있는 문서를 가리키는 **멀쩡한 링크**다. `check-links` 는 대상의 실재만 본다 |

편6 이 아직 없으므로 링크를 빼고 평문으로 두었다. ⇒ **없는 편을 지도편으로 대신 걸어 두는
습관을 만들지 마라.** 그 링크는 검사를 통과하면서 독자를 다른 곳으로 보낸다.
...
| **6-b** | 위 배치의 **집필 8편** | 매우 높음 | — | **설계서 승인** | 🎯 **편5 발행 완료 (5/8).** 다음은 **편6**(`roles-and-organizational-culture`)이며 원본 `12` §4~§7 이다. **목표 20,009**(밴드 18,845 ~ 21,172) — 편3·편4·편5 가 상수를 세 편 연속 지지했으므로 §3-2 의 값이 그대로 유효하고 재환산 지점은 둘 다 닫혔다. 🔴 **발행할 때 자기 파일만 고치지 마라** — 지도편 편 목록 표의 자기 행을 링크로 바꾸고 **앞 편의 마지막 예고 문장**에 자기 링크를 걸어야 한다 (설계서 §10-4-b · 편5 가 이 절차를 처음으로 따랐다). 🔴 **편6 은 §7-2 가 지목한 자리를 안고 있다** — 원본 §6 의 3분 비교를 한 열로 합치면 표가 말하는 차이가 사라진다. 집필 전에 낱말 실재를 `grep` 으로 확인하라 |
