...
>
> 함께 실은 것이 둘이다. PR [#20](https://github.com/withwooyong/withwooyong.github.io/pull/20) 의 merge 커밋과 배포 run 을 아래 「푸시 · 배포」 행에 채웠고,
> `CLAUDE.md` 가 자리를 비워 둔 **백슬래시 함정을 [50번](docs/TOOL-TRAPS.md)으로 썼다** — `sed` 치환은
> 히어독보다 한 겹을 더 먹으며, 소스의 1겹과 2겹이 **똑같이 0겹**으로 도착해 겹수를 늘리는
> 시도로는 판정할 수 없다.
>
> ---
>
...
| 강조 렌더 실패 | ✅ 발행본 **0회 · 186편** · 리포 문서 **0회 · 65개** |
| 깨진 링크 | ✅ 발행본 **0곳** · 리포 문서 **0곳** |
| 깨지는 도식 | ✅ 발행본 **0곳**(549개) · 리포 문서 **0곳**(132개) — **판정 도달 전량** · 합계 **681개** |
| 🔴 표의 열 수 | ❌ **어느 검사기도 보지 않는다** — 이번 세션이 세 검사기와 `tests/blog` 전체에서 열 수를 세는 코드를 찾아 **0건**임을 확인했다. 앞선 세션에서 셀 3개짜리 행이 **6개로 갈라진** 자리가 둘 있었고 마크업·링크·도식 검사 셋이 전부 통과시켰다. 원인과 재현은 [함정 50번](docs/TOOL-TRAPS.md)에 있다 |
...
| 검색 인덱스 | ✅ `out/blog/search-index.json` · **186편 · 헤딩 2,701개 · 315 KB** (2026-09-07 빌드 실측 — **편3 발행 전의 값이다**) |
...
⚠️ **남은 셋은 사람의 판단이나 실물 확인을 먼저 요구한다.** 새 배치는 설계까지 왔으나 집필이
사람의 판단을 계속 요구하고(익명화 · 출처 · 도식 근거), 도식의 색은 브라우저로 재야 드러나며,
표의 열 수는 표본이 더 쌓여야 검사기를 만들지 말지를 판단할 수 있다.
...
| `TOOL-TRAPS.md` 에 **셸의 백슬래시 확장분**이 없다 | ✅ 48·49 번은 채웠다 — 미추적 파일(48) · **도식 라벨 잘림(49)**. 남은 하나는 `sed` 치환도 백슬래시를 먹는다는 실측이며, 8번이 히어독만 다룬다. 요약만 있고 재현 절차가 없으므로 **「전체 49」는 그 확장분에 대해서는 참이 아니다** |
