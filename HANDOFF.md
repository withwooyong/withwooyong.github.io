...
**푸시 상태**: ✅ **T5 세션 종료 시 전부 푸시 완료** — T4분 2커밋(`37e413b`·`34cb459`)까지 함께 나갔다.
⚠️⚠️ **「푸시돼 있다」는 서술을 승계하지 말고 매번 `git log origin/main..HEAD`로 실측하라.**
**T5 HANDOFF 초판이 `37e413b`·`34cb459`를 「이미 푸시돼 있다」고 적었으나 실측하니 둘 다 미푸시였다** — 직전 HANDOFF의 문장을 검증 없이 승계한 결과다. **이 줄 자체가 그 실패의 기록이다.**
...
| T5-e | 편5 `:189` | 「앞 절 원칙 4」가 실제 앞 절이 아니다(원칙 4는 세 절 앞 `:109`) — 같은 글이 「앞 절」을 두 뜻으로 쓴다. `:136`의 「앞 절」은 정확하다 |
| T5-f | 편5 도식 3 | `PG --> IDX`가 무라벨이라 「ingest가 index.md를 갱신한다」 경로가 라벨에서 끊긴다 |
| T5-g | 편5 `:163` · `:169` | ⚠️ **Layer 1 코드펜스(SKILL.md)의 치환 2건이 어느 공시에도 안 덮인다.** `:286`의 공시는 「이 **층**의」(Layer 2) 한정이다. **`:158`이 「파일 전문」→「파일 내용」으로 낮춰져 축자 주장을 하지 않으므로 비차단**이나, 공시 구조가 층 단위라 Layer 1 펜스가 사각이다 |
| T5-h | 편5 `:475` | 귀속 표지 어형만 다르다 — 「이 글의 **것이다**」. 나머지 6곳은 전부 「이 글의 **정리다**」 |
| T5-i | 편5 `:442` | 「서버리스 함수 **또는 워크플로 엔진**」 — 편2 `:301`의 파이프라인 기재는 Lambda만이고 워크플로 엔진 변형은 편2가 `:325`에서 편4로 넘긴다. **두 편을 합친 요약이라 느슨하다** |
| T5-j | 편5 도식 1 | 화살표 라벨 「**공유 = git repo**」만 영문 혼용. 본문은 「git 저장소」(`:76`)·「레포 git」(`:280`)이다 |

> ⚠️ **T5-g~j는 최종 검증 `review`의 비차단 관찰이고 전부 「매우 낮음」이다.**
> **최종 검증에서 나온 비차단 관찰은 반영하지 않는 것이 절차다** — 손대면 다시 검증해야 한다.
> **그러나 이월에는 반드시 적는다.** T5에서 이 네 건이 1차 기록에서 빠져 뒤늦게 보완했다.
