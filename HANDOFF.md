...
| 검사기 | ✅ `scripts/` **아홉 종** + `tests/blog/` **5파일 54케이스** · 뮤턴트 **50/50 잡힘 · 생존 0** |
...
### ① `scripts/` 의 아홉 종 — 순서는 「증명한 뒤 스캔」이다
...
`npm run mutate` 로 뮤턴트 **50개**를 되살려 자기 검사가 잡는지 본다. 검사기를 고쳤으면 반드시
돌린다. 3~5분 걸리므로 백그라운드로 돌리고 **다른 파일 작업과 병행하지 마라.**
...
| **도식의 의미** | 도식 **649개** (발행본 544 · 문서 105) | 문법은 지금 검사한다. 「화살표가 끊겼는가」·「라벨이 본문과 어긋나는가」는 사람의 판단이며 `docs/superpowers/PUBLISHING-CHECKLIST.md` 에 있다 |
