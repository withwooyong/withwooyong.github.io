...
> **이번 세션은 도식 663개의 의미를 기계가 판정할 수 있는지 실태만 조사했다** (우선순위 5번).
> 사용자가 선택지 셋 중 「먼저 실태만 조사한다」를 골랐고, 착수 절차대로 일회성 스크립트를
> 스크래치패드에 두고 세었다. **결론은 검사기를 만들지 않는 것이다.** 위반이 0건이어서가
> 아니라, **판정을 세 번 고칠 때마다 위반의 78~100%가 사라졌기** 때문이다 — 456건에서
> 시작해 56건이 남았고 그 56건도 전부 정당한 도식 관용구였다. 콘텐츠와 코드는 한 줄도
> 바뀌지 않았고 검사기 종 수 · 뮤턴트 수 · `tests/blog` 도 그대로다.
> 조사는 [`2026-09-06-diagram-semantics-survey.md`](docs/superpowers/reports/2026-09-06-diagram-semantics-survey.md) 에 있다.
> 🔴 **자기 검사 케이스 둘이 결함 있는 판정에서도 통과했다.** 하나는 볼 지표를 덜 보았고
> 다른 하나는 결함이 드러나지 않는 입력을 썼다. **되돌려 FAIL 을 확인하는 절차가 없었다면**
> 성분 분리 44건이 거짓인 채로 결론이 되었을 것이다.
>
> **직전까지의 상태 —** 직전 세션이 지역 그래프를 카테고리 목록 페이지에도 띄웠다
> (우선순위 8번 · A 안). 그 세션의 PR [#17](https://github.com/withwooyong/withwooyong.github.io/pull/17) 은 merge 커밋 `572a849` 로 닫혔고
> 배포 run `34018339741` 이 **success**(전체 123초)였다. `tests/blog` 가 **14파일 194케이스**로 ·
> 뮤턴트가 **96개**로 늘어난 것이 그 세션의 결과다.
> 🔴 그 세션에서도 **테스트도 빌드도 산출물 문자열도 통과한 뒤에 브라우저가 결함 하나를 더
> 냈다** — 캡션이 `h-8`(2줄)에 고정돼 있어 이웃 수가 통째로 잘려 있었다. 그 앞 세션에서는
> 팔레트 계산이 위반 0 을 낸 뒤에 브라우저가 다크 대비 **1.18** 을 냈다
> ([함정 47번](docs/TOOL-TRAPS.md)). **세 세션 연속으로 마지막 결함은 계산이 아니라 실물이 냈다.**

## 지금 상태

발행본 **184편**이다. 배치는 편8로 끝났고 그 뒤 열세 세션이 검사기의 사각지대를 닫아 왔다.
앞 두 세션이 **읽는 경험**을 손댄 데 비해 이번 세션은 아무것도 고치지 않았다. 남은 사각지대
하나(도식의 의미)가 정말 사각지대인지를 먼저 물었고, **검사기를 만들지 않는다**는 답이 나왔다.
세지 않고 만들었다면 거짓 양성 400건을 예외 처리하는 일이 그 뒤에 따라왔을 것이다.
...
| 강조 렌더 실패 | ✅ 발행본 **0회 · 184편** · 리포 문서 **0회 · 59개** |
| 깨진 링크 | ✅ 발행본 **0곳** · 리포 문서 **0곳** |
| 깨지는 도식 | ✅ 발행본 **0곳**(544개) · 리포 문서 **0곳**(121개) — **판정 도달 전량** |
...
| 커밋 | 이번 세션 `36b5909`(조사 보고서 · 문서 갱신 · 수치 정정)와 이 인수인계를 PR [#18](https://github.com/withwooyong/withwooyong.github.io/pull/18) 에 실었다. **코드는 건드리지 않았다.** 그 앞은 `ebdf12a`(코드 · 검사 · 수치 문서) · `5f3bced` · `2b23915` · `f4db64d` 가 PR [#17](https://github.com/withwooyong/withwooyong.github.io/pull/17) → merge `572a849` 로 닫혔고, `211fd55`(PR #16 → merge `16c1107`) · `45b2c12`(PR #15 → merge `827c3e2`) 가 그 앞이다 |
| 푸시 · 배포 | ✅ **`research/diagram-semantics` 를 푸시하고 PR [#18](https://github.com/withwooyong/withwooyong.github.io/pull/18) 을 열었다** (`origin/main` = `572a849` 에서 갈라진 선형 커밋 셋). PR 단계 CI 는 run `34021022251` **success**(전체 **2분 35초** · `build` **success** · `deploy` **skipped** · `Upload artifact` **skipped** — 설계대로다). ⚠️ 그 앞 run `34020945588` 은 **실패가 아니라 취소**다 — 뒤이은 푸시가 같은 ref 의 run 을 대체했다. ⏳ **merge 는 아직 하지 않았다. 사용자 승인이 필요하다** — merge 커밋과 배포 run 은 뒤이은 세션이 채운다. 아래 행이 직전 세션까지의 기록이다 |
| 직전까지의 푸시 · 배포 | ✅ **PR [#17](https://github.com/withwooyong/withwooyong.github.io/pull/17) 이 merge 커밋 `572a849` 로 닫혔다** (부모 둘: `16c1107` + `f4db64d`). 배포 run `34018339741` 이 **success** 였고 전체 **2분 3초** · `build` **success** · `deploy` **success** 였다. PR 단계의 CI 는 run `34017972226` **success**(전체 **2분 30초**)이며 `Upload artifact` 스텝과 `deploy` job 이 설계대로 **skipped** 되었다. 그 앞은 PR #16 의 run `34011795806`(130초), PR #15 의 run `34007702350`(150초), PR #14 의 run `34006877177`(150초), PR #13 의 run `34004539240`(153초)였다. 머지 방식은 **merge 커밋**이다: PR #11~#16 에 더해 #17 도 부모가 둘이며, 이 리포는 squash 를 쓴 적이 없다 |
...
| 브랜치 | `research/diagram-semantics` 다. 병합이 끝난 `docs/pr16-deploy-record` 는 **아직 지우지 않았고** 로컬과 원격 양쪽에 남아 있다. 그 밖에는 **원격이 `main` 하나뿐**이며, 병합이 끝난 브랜치 36개를 지운 상태가 그대로다 |
| 다음 단계 | 아래 **「다음 세션의 작업」** — 우선순위 표를 먼저 보라 |

### 이번 세션이 한 일 — 도식 663개의 의미를 세고, **검사기를 만들지 않기로 판정했다**
...
### 직전 세션이 한 일 — 지역 그래프를 카테고리 목록에도 띄웠다
...
### 그 앞 세션이 한 일 — 도식 662개의 색을 팔레트로 바꾸고 대비를 검사기에 넣었다
...
### 그보다 앞선 세션이 한 일 — 기록의 재귀를 한 칸 좁히고 브랜치를 정리했다
...
| **6** | 시리즈 A·B 밖의 **새 배치** | 매우 높음 | — | **필요** | 🎯 **다음 세션의 작업.** 남은 항목이 이것뿐이다. 주제와 분량을 사용자가 정해야 착수할 수 있다 |
...
| 4 | **검사기가 무력해진다** | `check-mermaid` 가 발행본 **544개** · 문서 **121개**를 파서로 판정한다. HTML+SVG 로 바꾸면 그 665개가 **아무도 보지 않는 자리**가 된다. 대체 검사기부터 있어야 한다 |
...
| **도식의 색 — 실물 대조** | 도식 **665개** 중 브라우저로 잰 것은 **10개** | ⚠️ 팔레트 계산은 전량에 걸리지만, 「테마 변수가 닿지 않는 색」은 **렌더해 봐야** 드러난다. 이번 세션이 그렇게 하나를 찾았다. 남은 종류(`gantt`·`journey`·`erDiagram`·`quadrantChart`·`mindmap`)는 **문서에만 있어 발행되지 않으므로** 우선순위가 낮다 |

⚠️ **남은 둘은 사람의 판단이나 실물 확인을 먼저 요구한다.** 새 배치는 주제와 원본 위치를
사용자가 정해야 하고, 도식의 색은 브라우저로 재야 드러난다.
