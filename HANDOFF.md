...
> 마지막 갱신 2026-09-12 · 🗑️ **발표본 갈래를 통째로 걷어냈다** · 발행본 **192편**
>
...
| 검사기 | ✅ `scripts/` **열네 종**(🆕 `check-engines` 합류 · 파일은 18개이고 판정하지 않는 넷을 뺀 수다) + `tests/blog/` **14파일 200케이스** · 뮤턴트 **127개** |
...
| CI | ✅ `build` **34스텝** + `deploy` 1스텝 · **`main` 푸시와 PR 양쪽에서 돈다** (PR 은 33스텝). 🆕 `Prove engines checker` 와 `Check dependency engines against CI Node` 가 더해졌다 — 두 트리가 **모두 설치된 뒤에** 온다 |
