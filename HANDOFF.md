...
> 마지막 갱신 2026-09-06 · **리포 전체의 도식 라벨 잘림을 닫았다** · 발행본 **186편**(변동 없음).
> **이번 세션은 콘텐츠를 만들지 않았고 코드 한 자리를 고쳤다** — `components/mermaid.tsx` 와
> `lib/mermaid-theme.ts`. 직전 세션이 실물에서 발견만 하고 넘긴 항목이다.
>
> 🔴 **원인은 폰트 이름이 아니라 「재는 자리」와 「그리는 자리」가 다르다는 것이었다.**
> mermaid 는 라벨 폭을 재려고 임시 컨테이너를 **`document.body` 바로 아래**에 붙이는데
> (`render` 에 `svgContainingElement` 를 주지 않는 경로), 도식이 실제로 그려지는 자리는
> `font-sans` 가 걸린 본문 안이다. 이 저장소는 `next/font` 를 쓰므로 `--font-inter` 변수가
> `_app.tsx` 의 `<div>` 에만 있어, 두 자리의 계산된 글꼴이 갈렸다 —
> 재기는 `"Noto Sans KR"` 로, 그리기는 Inter 체인으로 했다.
> 🔴 그 경로에서는 `initialize({ fontFamily })` 가 **인라인 스타일로도 걸리지 않는다.**
> 인자를 넘기는 쪽은 `svgContainingElement` 가 있는 분기뿐이다.
>
> ⚠️ **첫 가설을 브라우저가 기각했다.** `document.body.style.fontFamily` 를 본문과 같게 맞추고
> 재렌더시켰는데 폭이 **소수점까지 그대로**였다(재렌더 자체는 SVG 의 다크 `fill` 로 확인했다).
> 확정은 **같은 문자열을 두 컨텍스트에서 직접 잰 것**이 냈다 — 「순이익」이 body 기본 폰트에서
> **38.65**, Inter 체인에서 **42** 였고, 앞의 값이 `foreignObject.width` 와 **소수점까지 일치**했다.
> 이어서 도식만 body 폰트로 강제하니 넘침이 **17 → 0**, 되돌리니 다시 **17** 이었다.
>
> **고친 방식 —** 상속에 맡기지 않고, 도식이 그려질 자리에서 `getComputedStyle` 로 **이미
> 해석된** 폰트 스택을 읽어 mermaid 에 넘긴다. 계산된 값은 `var()` 가 풀린 뒤이므로 body 아래의
> 측정 컨테이너에서도 똑같이 해석된다. 판정은 `lib/mermaid-theme.ts` 의
> `resolveDiagramFontFamily` **순수 함수**가 하고, 웹폰트 로드 전 측정을 막으려고
> `document.fonts.ready` 를 기다린다.
>
> **실측 —** 여섯 편 · 도식 **29개** · 라벨 **361개**에서 **라이트와 다크 양쪽 모두 넘침 0** 이다.
>
...
> | `langgraph-checkpointer-hitl` (상태 도식 포함) | 41 | — | **0 / 0** |
> | `autogen-conversation-agents` (시퀀스 포함) | 21 | — | **0 / 0** |
>
> 🔴 **포털이 함께 닫혔다.** 확대 뷰어(`Dialog`)는 `body` 아래에 그려져 본문 폰트를 상속받지
> 못하는데, 이 수정은 폰트 스택을 **SVG 자체에 박으므로** 그쪽도 같은 글꼴이 된다 —
> 확대 뷰어 라벨 **18개 넘침 0** 이고 계산된 글꼴이 본문과 문자열까지 같다.
> 「포털은 색과 글꼴을 스스로 지정해야 한다」가 [함정 47번](docs/TOOL-TRAPS.md)과 같은 자리다.
>
...
> 텍스트 모드로 쓰다 인코딩 오류가 나면 **파일이 비워진 뒤 쓰기가 실패한다.** 미커밋 변경이
> 없어 `git checkout HEAD --` 로 복구했고, 이후로는 바이트 치환으로 넣었다.
> ⇒ **서로게이트가 있을 수 있는 파일은 `'rb'`/`'wb'` 로 다뤄라.**
>
> **직전 세션이 한 일 —** `pm-practice` 배치의 **편2**(`product-cycle-and-metric-hierarchy` ·
> 24,596 B)를 발행하고 설계서의 예산 공식을 고쳤다.
> 🔴 **k 는 옳았고 고정 비용 상수가 틀렸다** — 초고가 목표를 16.6% 넘겼을 때 절 목록이 아니라
> **절별 배율**을 먼저 쟀더니 초과의 **59.4%가 배율 밖**에서 나왔다. 상수 2,455 가 같은 카테고리
> 기존 본편 셋(3,429 · 3,278 · 4,530)의 어느 값에도 닿지 않았고, 이 카테고리가 「이 편이 쓴 용어」
> 절을 갖는 것이 관례이기 때문이었다. ⇒ **목표 B = 3,746 + 배정 원본 B × 1.95 + 링크 몫** 으로
> 고쳤고 **편3~편8 이 일률적으로 +1,291 B 씩 올랐다.** 가설 H-1(인용이 크면 k 가 산문 축으로
> 끌린다)은 분해 셋(2.104 · 1.933 · 1.972) 어느 것도 산문 축 아래로 가지 않아 **기각**했다.
> ⚠️ 압축은 두 번 필요했다 — 전면 재작성이 **313 B**, 문단 삭제가 **1,713 B** 였다.
> ⚠️ `check-links` 가 185개만 스캔하고 위반 2곳을 냈다(편2 가 `git add` 전이었다).
> ⚠️ CR 계수를 `grep -c $'\r'` 로 재서 없는 문제를 쫓았다 — 정본은 `tr -cd '\r' | wc -c` 다.
>
> **그 앞 세션이 한 일 —** 편1 지도편(`pm-practice-map` · 21,803 B)을 발행했고 문서 셋을
> 반증했다 — 설계서 §9-2 의 `series.ts` 누락 · 「README 세 자리」가 실제로는 **네 자리** ·
> 가설 H-4 의 이유(중복 5쌍이 전부 `07` 한 파일 안이고 원본 넷 사이는 0쌍).
> 그 앞 세션은 사용자가 **`pm-po` 선별 4개**를 고르고 분할 설계서를 냈다.
...
> 대상 수집이 `git ls-files` 라 untracked 파일은 애초에 목록에 없다.
>
> **직전까지의 상태 —** 도식 663개의 의미를 조사해 **검사기를 만들지 않기로** 판정했고
> (우선순위 5번), 그 PR [#18](https://github.com/withwooyong/withwooyong.github.io/pull/18) 이 merge 커밋 `0a0583d` 로 닫혔다.
> 배포 run `34021838010` 이 **success**(전체 **2분 38초** · `build` 2분 21초 · `deploy` 10초)였다.
> 그 변경은 문서뿐이라 `out/` 에 들어가지 않아 **사이트 화면에는 달라진 것이 없었다.**
>
> 🔴 **네 세션 연속으로 마지막 결함은 계산이 아니라 실물이 냈다** — 캡션 `h-8` 잘림 ·
> 다크 대비 **1.18** · 도식 비례 **0.25** · 그리고 이번의 **라벨 잘림**이다.
> 검사기 열 종이 전부 초록인 동안 화면만 틀려 있었다. **콘텐츠를 만들지 않은 세션에도
> 실물을 열어라** — 이번 결함은 이 배치가 만든 것이 아니라 그 전부터 배포되어 있었다.
...
| 검사기 | ✅ `scripts/` **열 종** + `tests/blog/` **14파일 200케이스** · 뮤턴트 **98/98 잡힘 · 생존 0 · 치환실패 0** |
...
### 이번 세션이 한 일 — 리포 전체의 **도식 라벨 잘림**을 닫았다
...
### 앞선 세션이 한 일 (지역 그래프) — 지역 그래프를 카테고리 목록에도 띄웠다
...
| **6-b** | 위 배치의 **집필 8편** | 매우 높음 | — | **설계서 승인** | 🎯 **편2 발행 완료 (2/8).** 다음은 **편3**이며 🔴 **§3-2 의 새 예산표를 쓴다** — 목표는 15,871 이 아니라 **17,162**(밴드 16,203 ~ 18,120)다. k 는 편2 가 **1.95 로 확정**했고 고정 비용 상수가 3,746 으로 바뀌었다 |
...
| ~~9~~ | ~~도식 라벨의 **마지막 글자가 잘린다**~~ | 낮음 | **높음** | 불필요 | ✅ **이번 세션이 닫았다.** 리포 전체 549개 도식에 걸려 있었고 이미 배포된 상태였다. 실측 여섯 편 · 라벨 361개 · 라이트와 다크 모두 **0** |
...
## 도구 함정 — 전체 49건은 [`docs/TOOL-TRAPS.md`](docs/TOOL-TRAPS.md) 에 있다
