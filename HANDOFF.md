...
### 9-3. ⚠️ **여전히 남은 것 — `결함율` 5자리**

**T6-h.** 맞춤법상 ㅁ 받침 뒤는 「**률**」이다. 실측 **5자리**(HANDOFF의 「6자리」는 오류였다):

```bash
grep -rn '결함율' content/blog/ai-transformation/
```

⚠️ **편11은 이 낱말을 한 번도 쓰지 않았다**(0건) — 새로 만들지는 않았다.
**별도 커밋으로 처리하면 diff가 깨끗하다.**
