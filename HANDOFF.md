...
> 마지막 갱신 2026-09-17 · 🖼️ **17번 순서 ② 둘째 편 — `claude-md-scope-layers` 의 플로우차트 둘** · 발행본 Mermaid **558** · 발행본 **193편**
>
> **이번 세션(2026-09-17, 3차) —** `main`(`a2db4cf9`)에서 브랜치 `feat/diagram-svg-agentic-coding-2` 를 새로 갈라, 17번 순서 ② 둘째 편 `content/blog/agentic-coding/claude-md-scope-layers.md` 의 Mermaid 둘을 SVG 로 옮겼다.
>
> | 항목 | 내용 |
> | --- | --- |
> | 직전 PR | 앞 세션의 작업은 PR [#34](https://github.com/withwooyong/withwooyong.github.io/pull/34) 로 merge 됐다 — merge 커밋 `a2db4cf9` (2026-09-16T22:25:17Z) · 배포 run `35157595417` 의 build · deploy 모두 success |
> | 고른 근거 | `extractDiagrams` 로 세니 `agentic-coding` 에 도식이 남은 편은 26편 · 79개였다. 첫 편 `context-budget-session` 이 가장 많이 링크하는 편(4회)이 `claude-md-scope-layers`(도식 2개)라 이것을 골랐다 |
> | 한 일 | `정보는 세 층에서 온다`(TD, 세 갈래로 나뉘었다 모이는 층)를 `claude-md-information-layers-flow.svg` 로, `3덩어리 × 7단계`(LR, 덩어리에 단계가 딸린 나무)를 `claude-md-three-chunks-flow.svg` 로 옮겼다. 라벨 글자는 원문과 같고 「층 — 출처」는 두 줄로 나눴다. 원문의 방향 없는 선(`---`)은 화살표 없는 선으로 그려 덩어리 순서의 화살표와 구분했다. 이미지 제목은 `"플로우차트"` 다. 이 편의 Mermaid 는 **0개**다 |
> | 형식 | 앞 세션 스크래치패드의 `flow-lib.mjs` 를 복사해 범례에 `arrow` 항목(마커가 달린 선)을 더했고, 생성기 `gen-scope-layers.mjs` 는 리포에 넣지 않았다 |
> | 확인 | 개발 서버에서 두 도식을 라이트 · 다크로 보고 겹침과 잘림이 없음을 확인했다. 발행 전 검사 19종 종료 코드 0 · Mermaid 판정 도달 **558 = 558** · `check-forbidden` 파일 **203 → 205** · HARD 0 · `tests/blog` 15파일 219케이스 · `tsc` 0 · 빌드 0 · `check-forbidden:built` HARD 0 · **GC-6 불변 15개**(새 Tailwind 클래스 없음, 기준선 갱신 안 함) |
> | 수치 | 발행본 Mermaid **560 → 558**(flowchart 538 → 536). `CLAUDE.md`(합계 698 → 696) 와 `lib/diagram-zoom.ts` 주석을 고쳤다 |
>
> ⚠️ 이 편의 작업 트리는 **CR 349개**(CRLF)였고 정본 blob 은 0 이었다. 발행본 193편 중 89편이 작업 트리에서 CRLF 다. 고친 파일은 LF 로 썼고 커밋 blob 의 CR 은 커밋 뒤 `git cat-file blob` 으로 셌다.
>
> ⏹️ **17번 넓히기는 2026-09-17 사용자 결정으로 중단했다** — 셋째 편 이후는 하지 않는다.
>
...
> | 항목 | 내용 |
> | --- | --- |
> | 직전 PR | 앞 세션의 작업은 PR [#33](https://github.com/withwooyong/withwooyong.github.io/pull/33) 으로 merge 됐다 — merge 커밋 `82c8e7c7` (2026-09-16T15:36:51Z) · 배포 run `35116526617` 의 build · deploy 모두 success. 그 PR 은 자기 번호를 적지 못하므로 이 PR 에서 적는다 |
> | 한 일 | 플로우차트 넷을 `task-decomposition-flow.svg` · `prompt-to-agent-evolution-flow.svg` · `context-management-tools-flow.svg` · `session-continuity-flow.svg` 로 옮겼다. 노드와 간선 라벨은 원문과 글자까지 같고, 이미지 제목은 `"플로우차트"` 다. 판단 노드는 강조색 마름모, 작업으로 되돌아가는 간선은 점선, 노드 태그는 `STEP` · `CMD` · `FILE` · `RESUME` 이다. 이 편의 Mermaid 는 **0개**다 |
> | 형식 | 🔴 **리포에 생성 스크립트가 없다** — 기준 SVG 셋은 좌표를 손으로 적은 파일이다. 이번에는 세션 스크래치패드에 `flow-lib.mjs`(노드 · 마름모 · 간선 라벨 · 범례 함수)를 두고 편별 생성기로 만들었으며, 리포에는 넣지 않았다. 산출 형식은 `human-gate-state.svg` 와 같다(배경 `#2d3142` · 노드 `#393e53` · 강조 `#f08a59` · Geist 계열 글꼴 · 하단 LEGEND) |
> | 확인 | 가장 복잡한 `세 가지 관리 도구`(판단 노드 · 세 갈래 · 되돌아가는 간선)를 먼저 만들어 개발 서버에서 라이트 · 다크를 본 뒤 나머지 셋을 만들고, 넷 모두 두 모드에서 겹침과 잘림이 없음을 봤다. 캡션은 네 개 모두 「플로우차트를 탭하면…」이다. 발행 전 검사 19종 종료 코드 0 · Mermaid 판정 도달 **560 = 560** · `check-forbidden` 파일 **199 → 203** · `tests/blog` 15파일 219케이스 · `tsc` 0 · 빌드 0 · `check-forbidden:built` HARD 0 · **GC-6 불변**(새 Tailwind 클래스 없음, 기준선 갱신 안 함) |
> | 수치 | 발행본 Mermaid **564 → 560**(flowchart 542 → 538). `CLAUDE.md`(합계 702 → 698) 와 `lib/diagram-zoom.ts` 주석을 고쳤다 |
>
> ⚠️ 이 편의 작업 트리 CR 은 0 이었다(정본 blob 도 0). 대조를 위해 `git stash` → `pop` 을 한 번 했더니 `autocrlf` 가 변경 파일을 CRLF 로 풀었다 — 커밋 blob 은 LF 로 들어간다. **CR 계수는 작업 트리가 아니라 `git cat-file blob` 으로 보라.**
>
> 다음은 17번 순서 ② 의 둘째 편이다 — `agentic-coding` 에서 도식이 남은 편을 `extractDiagrams` 로 세어 고른다.
>
> ---
>
> **직전 세션(2026-09-17, 1차) —** 같은 브랜치에서 **17번 판정(통과)** 을 받고 넓히기를 시작했다.
>
...
> | 항목 | 내용 |
> | --- | --- |
> | 판정 | 사용자가 `human-gate-state.svg` 실물을 보고 「1 — 통과, 다른 편에도 적용」을 골랐다 |
> | 한 일 | 같은 편의 남은 Mermaid 하나(`## 2. 단계별 계약 아홉` 의 sequenceDiagram)를 `public/images/blog/agent-contract-sequence.svg` 로 옮겼다. 참여자 여섯 · 호출 16 · `alt` 분기 하나의 라벨이 원문과 같고, `-->>` 는 점선(응답)이다. 강조는 사람 참여자와 그 생명선뿐이다. 이 편의 도식은 이제 **전부 SVG** 다 |
> | 확인 | 개발 서버에서 라이트 · 다크 두 모드로 렌더를 봤다. 첫 판은 맨 오른쪽 `배포` 의 자기 호출 라벨이 캔버스 밖으로 잘려, 라벨을 생명선 왼쪽으로 옮겼다. 발행 전 검사 15종 종료 코드 0 · Mermaid 판정 도달 **564 = 564** · `tests/blog` 15파일 219케이스 · `tsc` 0 |
> | 수치 | 발행본 Mermaid **565 → 564**(sequenceDiagram 15 → 14). `CLAUDE.md` 와 `lib/diagram-zoom.ts` 주석을 고쳤다 |
>
> ⚠️ **개발 서버가 500 을 냈다.** 전날 띄운 `next dev` 가 `Jest worker encountered 2 child process exceptions` 로 모든 요청에 500 을 냈다. `npm run build` 가 같은 `.next` 를 덮어쓰기 때문이다. **빌드를 돌렸으면 개발 서버를 끄고 `.next` 를 지운 뒤 다시 띄워라.**
>
> ---
>
> **직전 세션(2026-09-16, 3차) —** 같은 브랜치 `feat/diagram-design-zoom-viewer` 에서 요청 넷과 16번을 했다.
>
> | 번호 | 한 일 | 확인 |
> | --- | --- | --- |
> | 18 | 확대 뷰어를 **폭 배율과 높이 배율 중 작은 쪽(contain)** 으로 연다. 종전에는 자연 크기로 열어 7918px 도식이 화면의 약 4.4배였고, 되돌리기도 폭만 맞춰 세로로 긴 도식이 넘쳤다. 두 뷰어의 중복 코드를 `components/diagram-zoom-dialog.tsx` 하나로 합쳤고 판정은 `lib/diagram-zoom.ts` 의 순수 함수다 | 가로형 2257×88 · 7918×290, 세로형 상태 다이어그램, 모바일 390px 에서 다크 · 라이트 모두 넘침 0 |
> | 19 | 스크롤 영역의 **제자리 클릭(6px 이내)** 이면 닫는다. 끌어서 이동 · 스크롤바 · 툴바 버튼은 닫지 않는다 | 실제 마우스로 확대 버튼 · 드래그 · 클릭을 각각 눌러 봤다 |
> | 20 | 캡션과 `aria-label` 을 종류 이름으로 바꿨다. Mermaid 는 첫 선언에서, SVG 는 **마크다운 이미지 제목**(`![alt](src "구조도")`)에서 얻고 조사는 받침으로 고른다. 발행본 종류는 `extractDiagrams` 로 셌다 — flowchart 542 · sequenceDiagram 15 · stateDiagram-v2 8 | 캡션 네 종류(플로우차트를 · 시퀀스 다이어그램을 · 상태 다이어그램을 · 구조도를) |
> | 17 | `## 4. 승인 지점을 하나로 모은다` 의 Mermaid 를 `public/images/blog/human-gate-state.svg` 로 바꿨다. 기준 SVG 의 팔레트 · 글꼴 · 태그 · 범례를 따르고, 상태 여섯 · 전이 열두의 라벨은 원문과 글자까지 같다. 강조는 `사람대기` 하나, 되돌아가는 전이는 점선 | ⏳ **사용자 판정 전이다. 다른 편으로 넓히지 마라** |
> | 16 | `check-forbidden` 이 `public/images/blog/*.svg`(소스)와 `out/images/blog/*.svg`(`--built`)를 스캔한다. 자기 검사 67/67 | SVG 라벨에 금칙어를 심으면 종료 코드 1 · 파일 196 → 198 |
>
> 🔴 **`check-baseline` 이 비블로그 13건을 「변경」으로 냈고 기준선을 갱신했다.** `git stash -u` 로 변경 전을
> 다시 빌드해 대조하니 `index.html` 은 **buildId · 전역 CSS 해시 · webpack 런타임 해시** 셋만 가리면
> 바이트까지 같았다. 새 Tailwind 클래스가 전역 CSS 파일 하나에 들어가 **모든 페이지의 CSS 파일명**이
> 바뀐 것이다 — 렌더된 내용은 불변이다. ⇒ **컴포넌트에 새 Tailwind 클래스를 쓰면 GC-6 은 매번 걸린다.**
> 대조로 원인을 확인한 뒤 `check-baseline:update` 한다.
>
> ⚠️ **뮤테이션 전량 실행은 125개 중 122 잡힘 · 생존 0 · 치환 실패 3(C1 · C2 · C4)** 이었다. 셋 다 개행을
> 담은 뮤턴트이고, 손대지 않은 `check-forbidden.mjs` 의 작업 트리 CR 이 424 · 정본 blob 이 0 이었다 —
> CLAUDE.md 의 `core.autocrlf` 함정 그대로다. 16번으로 그 파일을 다시 쓴 뒤 C1~C4 · C6 을 개별로 돌려
> 다섯 모두 잡혔다. **최종 뮤턴트 126개**(`ZM1`~`ZM4` · `C6` 추가) · `tests/blog` **15파일 219케이스**.
> 🔴 로컬에서 `mutate` 를 다시 돌리면 `stash pop` 이 CRLF 로 풀어 놓아 같은 셋이 또 치환 실패할 수 있다.
> **빨강을 코드 변경으로 읽기 전에 CR 을 세라.**
>
> 범위에서 뺀 것: `components/system-diagram-card.tsx`(포트폴리오의 「크게 보기」)는 건드리지 않았다.
> 비블로그라 GC-6 대상이고, 그 창은 이미 `max-h-[90vh] overflow-auto` 로 높이가 묶여 있다.
>
> ---
>
> **직전 세션(2026-09-16, 2차) —** 15번(후보 D)을 브랜치 `feat/diagram-emphasis-direction` 에서
> 50편 · 도식 67개에 적용했으나, 사용자가 실물을 보고 중단시켰다. **그 브랜치는 로컬 전용 · 미푸시로
> 남겨 두었다**(커밋 8개, 마지막 `1962b2bc` — 중단 경위 · 함정 59·60 · 뮤턴트 TH21~TH27 이 거기
> 있다). 이 브랜치 `feat/diagram-design-zoom-viewer` 는 `main`(`71c030cf`)에서 새로 갈라
> **인수인계만** 담았다. 발행본과 코드는 한 글자도 바뀌지 않았다.
>
> 🔴 **사용자 판정이 아래 「답은 전량 변환은 하지 않는다. 해법은 Mermaid 안에 있다」를 뒤집었다.**
>
> | 대상 | 사용자 원문 |
> | --- | --- |
> | `## 한눈에 보기 — 사람 하나와 에이전트 다섯` 아래 SVG (`multi-agent-system.svg`) | 「다크/화이트 모드 모두 내가 생각하기에 인상적이었어」 |
> | `## 4. 승인 지점을 하나로 모은다` 아래 Mermaid (후보 D 적용본) | 「색생만 조금 바뀌고, 내가 도식의 가독성 및 품질이 떨어진다고 했던 이전과 다를바가 없어」 |
>
> ⇒ **`classDef` 로 색을 얹는 것은 사용자가 말한 품질 문제의 해법이 아니다.** 기준은 SVG 쪽이다.
> 그 SVG 는 커밋 `64861c7b` 의 메시지에 「`diagram-design` 으로 만든」이라고 적혀 있다.
>
> 요청 넷은 아래 우선순위 표 **17~20번**에 사용자 원문과 좌표를 함께 적었다. 그 밖에 리포 밖에서
> `~/.claude/settings.json` 의 `effortLevel` 을 `xhigh` → **`high`** 로 바꿨다 — 공식 문서상
> Opus 5 의 API 기본값이 `high` 이고, 사용자가 high 로 작업하겠다고 했다.
>
> ---
>
> **직전 세션(2026-09-16, 1차) —** 아래는 그 세션의 기록이다.
>
> 🔍 **발행본 도식 566개의 품질을 조사했다**
>
> **이번 세션(2026-09-16) —** 리포의 파일을 바꾸지 않고 **조사만 한 세션**이다.
> 사용자의 물음은 「`human-gate-and-model-routing` 편의 SVG 처럼 기존 도식을 전부
> `diagram-design` 으로 바꿔도 되는가」였고, 「`## 4. 승인 지점을 하나로 모은다` 아래처럼
> 품질이 떨어지는 도식이 있다」는 지적이 함께 있었다.
> 전량은 [`2026-09-16-blog-diagram-quality-survey.md`](docs/superpowers/reports/2026-09-16-blog-diagram-quality-survey.md) 에 있다.
>
> **답은 「전량 변환은 하지 않는다. 다만 지적은 옳고, 해법은 Mermaid 안에 있다」다.**
>
> | 무엇을 쟀나 | 결과 |
> | --- | --- |
> | 발행본 Mermaid 블록 | **566개** · 179/193편 (문서는 **136개**) |
> | 블록 크기 | 중앙값 **7줄** (p75 10 · p90 13 · 최대 47) |
> | 노드 강조(`classDef`·`style`·`:::`) | 🔴 **566개 중 0개** — 한 번도 쓰지 않았다 |
> | 세로로 늘어지는 도식 | **137개** (flowchart `TD` 128 + stateDiagram `direction` 미지정 9) |
> | 지목된 `stateDiagram` | 355 × 742 — 본문 폭 895 px 중 **355 px 만** 쓴다. 라벨 넘침은 **0건** |
>
> 🔴 **`classDef` 가 0개라는 것이 이번 조사의 핵심이다.** 지목된 편의 본문은 「사람이 개입하는
> 자리는 `사람대기` **하나뿐**」이라고 적는데 도식에서는 여섯 상자가 모두 같은 하늘색이다.
> **글이 말하는 것을 그림이 받쳐 주지 못한다.** 이것은 형식의 문제가 아니라 기능을 쓰지 않은
> 것이므로, SVG 로 옮기지 않고 `classDef` 두 줄과 `direction` 한 줄로 닫힌다.
>
> 🔴 **다크 모드가 전량 변환을 막는 진짜 이유다.** 배포된 페이지를 라이트 모드로 바꿔 실물을
> 확인했더니 본문과 제목은 밝아지는데 **SVG 만 어두운 채로 남아** 흰 페이지에 검은 상자가
> 박힌다. `multi-agent-system.svg` 에는 `prefers-color-scheme` 이 **0건**이고 색이
> `#2d3142`·`#f08a59` 로 박혀 있다. ⇒ **닫기 전에 SVG 를 늘리면 늘린 만큼 라이트 모드가 나빠진다.**
>
> 🔴 **그리고 `@media (prefers-color-scheme: dark)` 를 SVG 에 넣는 해법은 이 사이트에서 어긋난다.**
> `components/theme-toggle.tsx` 를 읽으면 테마의 진실원이 `localStorage` 의 `portfolio-theme`
> 이고 OS 설정은 **저장값이 없을 때의 초기값일 뿐**이다. 사용자가 토글로 라이트를 골라도 OS 가
> 다크면 SVG 만 다크로 남는다. `<img>` 안의 미디어 쿼리는 사이트의 테마 상태를 볼 수 없다.
>
> 🆕 **`HANDOFF.md` 의 제약 넷 가운데 둘이 이미 뚫려 있었다.** 1번(raw HTML 미렌더)과
> 2번(분량 예산)은 `![alt](/images/blog/….svg)` **이미지 참조**가 우회한다 — `64861c7b` 가
> 이미 그렇게 발행했다. 막고 있는 것은 **3번(다크 모드)과 4번(검사기)뿐**이다.
>
> 🔴 **새 사각지대를 찾았다 — SVG 안의 텍스트는 어느 검사기도 보지 않는다.**
> `check-forbidden` 은 `content/blog`, `--built` 는 `out/blog` + `_next/data` 를 보는데
> SVG 는 `out/images/blog/` 로 나간다. `tests/blog` 의 도식 이미지 자산 5케이스는 **경로 형식과
> 파일 실존만** 본다. ⇒ 배포된 SVG 안의 문자열은 **금칙어 검사를 한 번도 받지 않았다.**
> 지금은 우연히 문제가 없지만 SVG 를 늘리면 사각지대가 같은 배수로 커진다.
>
> 🔴 **이 조사가 정정한 수치 넷.** 전부 「이어받았거나 정규식으로 센」 값이었다.
>
> | 무엇 | 종전 | 실측 | 어디에 |
> | --- | ---: | ---: | --- |
> | 발행본 Mermaid 블록 | 552 | **566** | `CLAUDE.md` |
> | 발행본 · 문서 블록 | 549 · 132 | **566 · 136** | `HANDOFF.md` 제약 4번 |
> | 발행본 편 수 | 192 | **193** | `HANDOFF.md` 상단 (README 는 193 으로 맞다) |
> | 세로로 늘어지는 도식 | 152 | **137** | 이 조사의 첫 계산 |
>
> 🔴 **마지막 행은 이 세션 자신의 오류다.** `sequenceDiagram` 15개는 방향이라는 개념이 없는데
> 「`LR` 이 지정되지 않은 것」으로 세어 포함했다. **종류가 섞인 집합에서 「지정되지 않은 것」을
> 세면 해당 없는 것까지 들어온다.**
>
> 🔴 **도식을 셀 때 정규식을 쓰지 마라 — 인용 블록 안의 것을 놓친다.** 문서 블록을 정규식
> `/^\s*```mermaid\s*$/` 로 세어 135 를 얻었으나 검사기는 136 을 냈다. 자리는
> `2026-08-08-agentic-coding-category-split-design.md:563` 의 <code>> ```mermaid</code> 하나이고,
> `^\s*` 가 공백만 허용해 `>` 로 시작하는 줄을 건너뛴 것이다. ⇒ **`check-mermaid.mjs` 의
> `extractDiagrams` 를 직접 불러라.** 발행본 566 은 두 방법이 일치했으나, **맞아떨어진 것이
> 곧 그 방법이 옳다는 뜻은 아니다.**
>
> ⚠️ **도식의 넘침을 `getBBox()` 로 재지 마라.** 로컬 좌표를 돌려주므로 `viewBox` 와 직접
> 대조하면 멀쩡한 라벨 22개가 전부 위반으로 올라온다(실측). 이 세션이 그렇게 「라벨이 잘렸다」고
> 한 번 잘못 보고했다. 화면 좌표는 `getBoundingClientRect()` 로 재고 SVG 자신의 사각형과 대조한다.
>
> 🆕 **겪은 함정 셋을 56~58 로 남겼다.** ㉚ 정규식이 인용 블록 안의 코드 울타리를 세지 못한다 ·
> ㉛ `getBBox()` 는 로컬 좌표라 `viewBox` 와 대조하면 멀쩡한 라벨 22개가 전부 위반이 된다 ·
> ㉜ 「해당 없음」을 「지정하지 않음」으로 세면 대상이 부풀어 오른다. 55번 곁가지도 보강했다 —
> 이미지 자산 검사 다섯이 닫은 것은 「파일이 있는가」뿐이다.
> 🔴 **`CLAUDE.md` 표에는 새 행을 넣지 않았다.** 그 파일이 **273줄**로 절차가 정한 상한 200 을
> 이미 넘었고, 절차가 「새 행을 넣을 때 뺄 한 행을 함께 고르라」고 하기 때문이다. 대신 기존
> 「링크·마크업을 정규식으로」 행을 보강했다 — 56번이 그 행의 **거울상**이다. 57번은
> `docs/TOOL-TRAPS.md` 에만 있고 이 문서 상단이 요약을 갖고 있다.
>
> ✅ **푸시와 배포를 확인했다.** `main` 에 두 커밋을 올렸다.
>
> | 커밋 | 무엇 |
> | --- | --- |
> | `0eda626e` | 조사 문서 · `HANDOFF` 갱신 · `CLAUDE.md` 도식 수 정정 |
> | `5f1ee8d3` | 함정 56~58 · 55번 곁가지 보강 · 분류 표와 가리키는 수치 갱신 |
>
> 배포 run [`35062998242`](https://github.com/withwooyong/withwooyong.github.io/actions/runs/35062998242) 이 **success** 다
> (전체 **2분 49초** · `build` **success · 34스텝 · 2분 31초 · skipped 0 · 실패 0** ·
> `deploy` **success · 3스텝 · 10초**). 34 는 워크플로 정의의 **30** 에 러너가 넷을 붙인 수다.
>
> 🔴 **그 배포로 화면이 달라지지는 않는다** — 변경이 문서뿐이라 `out/` 에 들어가지 않는다.
>
> ⚠️ **직전 배포 run 두 개의 상태가 갈려 있다.** `64861c7b`(run `35053894019`)이 **failure**,
> 그 다음 `2ab356c3`(run `35057213201`)이 **success** 다. 앞선 세션이 GC-6 기준선을 갱신해
> 닫은 것이며 지금 `main` 은 초록이다.
>
> ⏳ **남은 것은 사용자의 방침 결정이다.** 아래 「`diagram-design` 적용 검토」의 **후보 D** 와
> 「다음 세션의 작업」의 새 행을 보라. 발행본과 코드는 한 글자도 바뀌지 않았다.
>
> ---
>
> **앞선 세션(2026-09-13) —** 코드를 고치지 않았고 **원격 참조만 정리한 세션**이다.
> 2026-09-12 세션이 로컬 브랜치만 지우고 원격을 그대로 두어 `origin` 에 열 개가 남아 있었고,
> 사용자 승인을 받아 지웠다. 원격에 남은 것은 이제 `refs/heads/main` 하나뿐이다.
>
> | 무엇을 확인했나 | 명령 | 결과 |
> | --- | --- | --- |
...
>
> 🔴 **`git branch -r --merged` 의 판정을 결론으로 쓰지 않았다.** 그것은 브랜치의 tip 이
> `main` 의 조상인지만 보므로, 브랜치마다 `main` 에 없는 커밋 수를 따로 셌다.
> 그리고 **원격의 상태를 `git branch -r` 로 읽지 마라** — 그것은 로컬의 remote-tracking
> 캐시이고 `--prune` 을 붙인 `fetch` 전까지는 사라진 브랜치를 그대로 보여준다.
> 원격에 직접 묻는 것은 `git ls-remote --heads origin` 이다.
>
> ✅ **병합된 PR 은 브랜치를 지워도 커밋을 계속 보여준다.** 지운 뒤 PR #32 를
> `gh pr view 32 --json commits` 로 다시 읽어 커밋 목록이 그대로 나오는 것을 확인했다.
>
> 🔴 **이 세션은 리포의 파일을 바꾸는 실질 작업이 없어 기록 커밋을 `main` 에 직접 올렸다.**
> `CLAUDE.md` 는 문서 갱신을 실질 작업의 PR 에 실으라고 하지만, 실질 작업이 **원격 참조를**
> **지우는 것뿐**이면 함께 실을 PR 이 애초에 만들어지지 않는다. PR 을 열면 그것이 곧 같은
> 파일이 금지한 「문서 갱신만을 위한 PR」이 되어, 그 PR 자신의 merge 커밋과 배포 run 을
> 어느 문서에도 적지 못하게 된다. ⇒ **실질 작업이 리포 밖에만 있으면 `main` 에 직접 커밋한다.**
>
> 🔴 **「지금 상태」 표의 두 행이 낡아 있어 다시 세어 고쳤다.** 「검사기」 행이 「열네 종 ·
> 뮤턴트 127」로, 「CI」 행이 「`build` 34스텝」으로 남아 있었다. 발표본 갈래를 걷어내며
> 줄어든 값이 **상단 요약에만 반영되고 표에는 반영되지 않은 것**이다. 실측은 검사기
> **열세 종** · 뮤턴트 **121개** · `tests/blog` **14파일 200케이스** · `build` **30스텝**이다.
> 케이스 수는 `grep` 이 아니라 `npx vitest run` 의 요약 줄에서, 스텝 수는 정규식이 아니라
> **YAML 파서**로 읽었다.
>
> ✅ **푸시와 배포를 확인했다.** 커밋 `2a6351f7` 을 `main` 에 직접 올렸고 배포 run
> [`34727234645`](https://github.com/withwooyong/withwooyong.github.io/actions/runs/34727234645) 이 **success** 다 (전체 **3분 22초** ·
> `build` **success · 34스텝 · 2분 29초 · skipped 0** · `deploy` **success · 3스텝 · 9초**).
> 34 는 워크플로 정의의 **30** 에 러너가 넷을 붙인 수이며 둘 다 맞다.
> 🔴 **그 배포로 화면이 달라지지는 않았다** — 변경이 문서뿐이라 `out/` 에 들어가지 않는다.
>
> 🆕 **`grep` 이 NUL 한 바이트에 침묵하는 방식이 한 겹 더 있었다.** 위 수치를 세다가
> `grep -oE 'id: "[^"]+"' scripts/mutate.mjs | wc -l` 이 **0** 을 냈다 — `-o` 는
> `Binary file … matches` 를 내지 않으므로 **경고조차 없다.** `-a` 를 붙이니 **121** 이다.
> `docs/TOOL-TRAPS.md` 52번의 곁가지에 실측을 보탰고 `CLAUDE.md` 의 줄 수를 1,556 에서
> **1,572** 로 고쳤다. 건수는 **53건 그대로**다 — 기존 곁가지의 보강이기 때문이다.
>
> ---
>
> **아래 문단들은 그 앞 세션(2026-09-12, 둘째)의 기록이다.**
>
> **그 세션의 머리글** — 2026-09-12 · 🗑️ **발표본 갈래를 통째로 걷어냈다** · 발행본 **192편**
>
> **이번 세션(2026-09-12, 둘째) —** 사용자가 배포된 발표본 넷의 삭제를 지시했다.
> 근거는 두 가지다. **문서 품질이 마음에 들지 않는다**는 것과, **만들 때마다 실물을
> 확인시키지 않아** 사용자가 한 번도 보지 않은 채 119장이 쌓였다는 것이다.
> 필요해지면 다시 만들어 달라고 요청하겠다고 했다.
>
> | 지운 것 | 내용 |
> | --- | --- |
> | `slidev-poc/` | 발표본 소스 여섯 · `decks.json` · `pages/` · `components/` · `snippets/` |
> | `scripts/check-slides.mjs` · `scripts/build-slides.mjs` | 검사기 한 종과 빌더 하나 |
> | `package.json` | `check-slides` · `check-slides:verify` · `build-slides` |
> | `.github/workflows/deploy.yml` | 발표본 스텝 넷 (`build` job **34 → 30** 스텝) |
> | `scripts/mutate.mjs` | `C5` · `SL1`~`SL5` 여섯 뮤턴트와 `CHECKS` 한 줄 |
> | `scripts/check-forbidden.mjs` | 발표본 소스 13개를 모으던 `sourceTargets` 의 절반 |
> | `scripts/check-engines.mjs` | 둘째 대상 트리. 대조군을 Node 20 에서 **16** 으로 낮췄다 |
> | `public/robots.txt` | `Disallow: /slides/` |
>
> 🔴 **수가 처음으로 줄었다.** 검사기 **열네 종 → 열세 종** · 뮤턴트 **127 → 121** ·
> 뮤테이션이 돌리는 검사 **열네 → 열세** · 워크플로 `build` **34 → 30** 스텝.
> 세 문서가 「늘어나기만 한다」는 전제로 쓰여 있었으므로 그 전제를 `CLAUDE.md` 에 적어 고쳤다.
>
> 🔴 **`check-engines` 의 대조군이 헛돌 뻔했다.** 종전 대조군은 「Node 20 으로 재면 위반이
> 나온다」였는데 그 위반이 **전부 발표본 트리에서 나온 것**이었다. 트리를 걷어낸 채로 두면
> 대조군이 0 을 내면서도 케이스는 통과한다 — 실측으로 확인하고 16 으로 낮췄다.
>
> ⚠️ **`slidev-poc/node_modules` 와 `dist-probe`(합계 503 MB)는 추적되지 않아 남아 있다.**
> git 이 모르는 파일이라 되돌릴 수 없으므로 지우지 않았다. 정리는 사용자 승인 뒤에 한다.
>
...
> 에 그대로 남아 있고, `routerMode: hash` 를 적어야 하는 이유와 넘침을 재는 네 단계 절차는
> 이 문서의 아래쪽에 그대로 둔다.
>
> ---
>
> **아래 문단들은 그 앞 세션(2026-09-12, 첫째)의 기록이다.**
>
> 🧹 **미푸시 커밋을 닫고 작업 트리를 정리했다** ·
...
> ✅ **merge 커밋은 `48a91c6` 이고 배포 run [`34438737873`](https://github.com/withwooyong/withwooyong.github.io/actions/runs/34438737873)
> 이 success 다.** 둘 다 `git log origin/main` 과 `gh run list --branch main` 에서 읽었다.
>
...
> 로컬이 Node 24 라 넷 모두 **38초에** 빌드되어 이 어긋남은 로컬에서 드러날 수 없었다.
...
>
> 🔴 **전량 뮤테이션은 「3~5분」이 아니라 119개 기준 약 28분이다.** 뮤턴트와 검사가 함께 늘어
> 소요가 곱으로 커졌는데 아무도 다시 재지 않았다. ⇒ **세션 시작에 백그라운드로 걸어라.**
> 한 자리만 볼 때는 격리 실행으로 충분하며, 그때 **치환이 실제로 일어났는지를 먼저 세라** —
> 문자열을 못 찾으면 원본이 그대로 돌아 「FAIL 이 안 났다」가 「케이스가 지킨다」로 읽힌다.
>
> 🔴 **`README.md` 의 뮤턴트 수가 98 로 낡아 있었다** — `CLAUDE.md` 가 112 를 적는 동안
> 갈라져 있었고, 「세 문서가 갈라진다」는 경고의 현재 사례였다. 둘 다 118 로 맞췄고,
> 그 뒤 `SL5` 를 더해 **119** 가 되었다.
>
> 🔴 **배포하고 나서야 장별 URL 이 전부 404 인 것이 드러났다.** `/slides/<슬러그>/` 는 그려지는데
> `/slides/<슬러그>/2` 는 본체의 Next.js 404 페이지로 떨어졌다. slidev 의 기본값이 history
> 라우팅이고 **GitHub Pages 는 사이트 루트의 `404.html` 만 쓰기 때문**이다 — slidev 가 슬러그마다
> 만들어 둔 `404.html` 은 무시된다. Netlify · Vercel 은 디렉터리별 폴백을 쓰므로 **같은 산출물이
> 거기서는 멀쩡하다.** 발표본 다섯의 frontmatter 에 `routerMode: hash` 를 넣어 닫았고,
> `check-slides` 에 판정을 더해 규칙이 문서가 아니라 검사기에 살게 했다 (자기 검사 **9 → 17케이스**).
...
>
> **편8 세션 —** `pm-practice` 배치 **편8/8 발행 — 시리즈 완결** ·
> **대조군을 둘 놓아야 초과가 100% 갈린다.** 편8 [`global-decision-and-communication`](content/blog/product-management/global-decision-and-communication.md)(**22,499 B** · 목표 21,546 · 밴드 20,273 ~ 22,820 · 역산 k **2.055**)를
...
> `34318644234` · `34318935183` 이 둘 다 **success** 였다. PR 은 merge 커밋 `85b96bc` 으로
> `main` 에 닫혔고 배포 run `34321271566` 도 **success** 이며, 배포된 편8 페이지(`HTTP 200`)와
> `search-index.json` 의 **192편**을 실물로 확인했다.
>
...
| 검사기 | ✅ `scripts/` **열세 종**(파일은 **16개**이고 판정하지 않는 `generate-sitemap`·`map-terms`·`mutate` 셋을 뺀 수다) + `tests/blog/` **14파일 200케이스** · 뮤턴트 **121개**. 🆕 **2026-09-13 에 다시 셌다** — 발표본 갈래를 걷어내며 줄어든 값이 상단 요약에만 반영되고 이 행은 「열네 종 · 127개」로 남아 있었다. 뮤턴트는 `id: "` 를 세고 `uniq -d` 로 중복이 없음을 확인했으며, 케이스 수는 `grep` 이 아니라 `npx vitest run` 의 요약 줄에서 읽었다 |
...
| CI | ✅ `build` **30스텝** + `deploy` 1스텝 · **`main` 푸시와 PR 양쪽에서 돈다** (PR 은 29스텝). `Prove engines checker` 와 `Check dependency engines against CI Node` 가 들어 있고, 발표본 갈래를 걷어낸 뒤 대상 트리는 **리포 루트 하나**다. 🆕 **2026-09-13 에 YAML 파서로 다시 셌다** — 발표본 스텝 넷이 빠지며 34 에서 30 이 되었는데 이 행만 34 로 남아 있었다 |
| ⚠️ CI 스텝 수를 세는 법 | `gh run view --json jobs` 가 보고하는 수는 워크플로 정의의 수보다 **넷 많다**. 러너가 `Set up job`·`Post Setup Node`·`Post Checkout`·`Complete job` 을 붙이기 때문이다. 문서의 수(지금은 **34**)는 워크플로 정의의 것이며 둘 다 맞다. 🔴 **정의의 수는 정규식이 아니라 YAML 파서로 세라** — **다른 수를 대조하고 「문서가 낡았다」고 판단하지 마라.** 🔴 **이번 세션이 정확히 그 실수를 했다** — 27 을 보고 「23 은 낡았다」고 사용자에게 보고했다가 이 행을 읽고 되돌렸다. 이 행이 없었다면 멀쩡한 문서를 고쳤을 것이다 |
...
| 🆕 편8 의 푸시 · CI | ✅ **PR [#27](https://github.com/withwooyong/withwooyong.github.io/pull/27) 을 열었고 PR 단계 CI run `34318644234` 이 success 였다** (`build` **success · 30스텝 · 2분 28초** — 워크플로 정의의 26 에 러너가 넷을 붙인 수이며 `success 29 · skipped 1` 이다. skipped 하나가 `Upload artifact` 이고 `deploy` job 도 통째로 skipped 인 것은 `github.event_name != 'pull_request'` 조건대로다). `mergeable` 은 `MERGEABLE` 이고 `gh run list --branch` 로 **run 이 실제로 생성된 것**(1개)을 따로 확인했다 — 충돌하는 PR 은 빨간 것이 아니라 run 이 0개다. ⚠️ **브랜치 기점이 `origin/main` 이 아니라 로컬 `main` 의 미푸시 커밋 `2b9a987` 이다** — 편6 · 편7 과 같은 지시로, 직전 세션의 문서 갱신을 이 PR 에 함께 실었다. 🆕 **PR 은 merge 커밋 `85b96bc` 으로 `main` 에 닫혔고**(부모 둘: `eb92250` + `f5ef194`, 실린 커밋 셋: `2b9a987` + `4931fce` + `f5ef194`) **배포 run `34321271566` 도 success 다** (전체 **179초** · `build` **success · 30스텝 · 157초** · `deploy` **success · 3스텝 · 11초** — push 이벤트라 `Upload artifact` 와 `deploy` 가 설계대로 돌았고 skipped 가 하나도 없다). 인수인계를 채운 두 번째 커밋 `f5ef194` 의 PR 단계 run `34318935183` 도 **success** 였다 (전체 **164초** · `build` **30스텝 · 159초** · skipped 하나는 `Upload artifact` 다). 🆕 **배포된 실물을 직접 확인했다** — 편8 페이지가 `HTTP 200 · 188,111 B` 이고 `<title>` 이 편8 의 것이며, 배포된 `search-index.json` 이 `HTTP 200 · 332,701 B · 192편`으로 로컬 빌드 실측(**192편 · 325 KB**)과 일치한다. ⚠️ **페이지 크기만은 대조군이 없다** — 편8 을 빌드한 임시 워크트리가 정리되어 `out/` 이 두 워크트리 어디에도 남아 있지 않으므로, 편7 때처럼 「로컬 빌드와 같은 크기」라고 말할 근거가 이번에는 없다 |
...
| 🆕 작업 트리 (2026-09-13) | ✅ **워크트리 하나 · 로컬 브랜치 `main` 하나 · 원격 브랜치 `main` 하나**다. 2026-09-12 세션이 로컬 열한 개를 `git branch -d` 로 지웠고(`-d` 가 미병합 브랜치를 거부하므로 그 성공 자체가 병합 검증이다), 🆕 **2026-09-13 세션이 원격에 남아 있던 열 개를 사용자 승인을 받아 지웠다.** 원격의 실제 상태는 `git ls-remote --heads origin` 으로 확인했다 — `git branch -r` 이 보여주는 것은 원격이 아니라 **로컬의 remote-tracking 캐시**다 |
| 🆕 원격 동기화 (2026-09-13) | ✅ `main` 과 `origin/main` 이 같은 커밋이다. 🆕 **앞 세션이 적지 못한 두 건을 채웠다** — PR [#32](https://github.com/withwooyong/withwooyong.github.io/pull/32) 의 merge 커밋 [`c3b0f9b3`](https://github.com/withwooyong/withwooyong.github.io/commit/c3b0f9b3) 과 그 배포 run [`34688560748`](https://github.com/withwooyong/withwooyong.github.io/actions/runs/34688560748) · **success** · **2분 15초**다. 둘 다 문서에서 옮겨 적지 않고 `gh run view` 와 `git ls-remote` 에서 읽었다. 🔴 **직전 세션들이 문서 커밋을 만들고 푸시하지 않은 적이 있다** — 문서 커밋은 배포에 영향이 없어 아무도 알아채지 못했고, 사흘 뒤 `git status -sb` 의 `ahead 1` 로 드러났다. ⇒ **세션을 닫기 전에 `ahead` 를 읽어라** |
| 다음 단계 | 아래 **「다음 세션의 작업」** — 우선순위 표를 먼저 보라 |

### 이번 세션이 한 일 — `engines` 사각지대를 검사기로 닫았다. 🔴 **되돌려 보지 않았으면 못 잡는 판을 만들 뻔했다**

직전 세션이 남긴 사각지대 하나(「`slidev-poc/node_modules` 의 `engines` 를 아무도 보지 않는다」)를
검사기 `check-engines` 로 닫았다. 검사기 종 수가 **열네 종**, 뮤턴트가 **127개**가 되었다.

| 무엇 | 값 |
| --- | --- |
| 새 검사기 | `scripts/check-engines.mjs` · 자기 검사 **26/26** · 본 스캔 **647개** 판정 · 0.75초 |
| 이관 | `ciNodeMajor`·`rangeAllowsMajor` 와 케이스 ㉒~㉖ 을 `check-mermaid` 에서 **옮겼다**(복사 아님). 그쪽은 26 → **21케이스** |
| 뮤턴트 | `CM11`·`CM12` → `E1`·`E2` 로 개명하고 `E3`~`E10` 을 더했다. 전량 **127개 · 잡힘 127 · 생존 0 · 치환실패 0 · 36분 23초** |
| CI | `build` **32 → 34스텝**(PR 33). 두 트리가 모두 설치된 **뒤에** 온다 |

#### 🔴 처음 고른 설계가 실측으로 반증됐다 — 「직접 의존성만」은 이 사례를 못 잡는다

착수할 때 「이번에 CI 를 죽인 넷은 전부 `slidev-poc` 의 직접 `dependencies` 였다」고 읽었다.
인수인계의 「넷 전부 `dependencies` 이므로 `--omit=dev` 로 빠지지 않는다」를 그렇게 읽은 것인데,
실제로는 **`dependencies` 갈래를 통해 들어온다**는 뜻이었다. 넷은 전부 `@slidev/cli` 아래에서
hoisting 된 **transitive** 이고 `slidev-poc/package.json` 에는 `@slidev/cli` 만 있다.

| 대상 범위 | 판정 대상 | Node 22 위반 | Node 20 위반 |
| --- | ---: | ---: | ---: |
| 직접 의존성 | 14개 | 0건 | **0건** ❌ |
| 설치 트리 전량 | **647개** | 0건 | **4건** ✅ = 손으로 찾았던 그 넷 |

⇒ **자기 검사 21/21 을 낸 판이 이 사례를 못 잡고 있었다.** 드러난 것은 되돌림 하나뿐이다 —
워크플로의 `node-version` 을 20 으로 낮췄는데 종료 0 이 나왔다. **「이 사례를 잡는가」는
케이스가 아니라 되돌림이 답한다.** 그래서 대조군을 자기 검사에 상설로 넣었다(㉖ — Node 20 으로
재면 위반이 나오는지). 그것이 없으면 다음 사람은 0 이 결론인지 고장인지 가르지 못한다.

#### 되돌려 본 셋

| 무엇 | 어떻게 | 결과 |
| --- | --- | --- |
| Node 20 판정 | 워크플로를 20 으로 | 종료 **1** · 위반 **4건** · 복원 뒤 바이트까지 동일 |
| 트리 없음 가드 | `slidev-poc/node_modules` 를 치움 | 종료 **2** · `missing-tree` |
| 뮤턴트 열 | 전량 실행 | `E1`~`E10` 전부 **잡힘** |

#### 🔴 `rangeAllowsMajor` 에 거짓 양성이 셋 있었다 — 넓히는 순간 터진다

`jsdom` 하나만 보던 동안은 드러나지 않았다. 전량으로 넓히니 `engines.node` 가 `*` 인 셋
(`fraction.js`·`glob`·`minimatch`)과 `>=v12.22.7`(`saxes`)이 위반으로 올라왔다 — 정규식이
숫자를 못 찾은 것을 **불허로 읽었기** 때문이다. 넷 다 진짜 위반이 아니다.
⇒ **「모르는 형식」을 불허로 두면 대상을 넓힐 때 소음이 된다.** 고친 뒤 Node 22 위반은 0건이다.

#### 🔴 직전 뮤테이션의 종료 코드는 0 이 아니라 1 이었다 — 명령이 그것을 가렸다

`date && npm run mutate > log; echo "EXIT=$?"; date` 로 걸었더니 **마지막 `date` 의 코드**가
전체 종료 코드가 되어 배경 작업 알림이 「exit code 0」으로 왔다. 실제로는 치환 실패 3건 때문에
**1** 이었다. ⇒ **종료 코드를 읽을 명령은 마지막에 두거나 `exit $CODE` 로 넘겨라.**

원인은 이미 아래 「뮤턴트를 `git checkout --` 로 복구하면」에 적힌 것 그대로였다 —
작업 트리의 `check-forbidden.mjs` 가 CR **436**, 정본이 **0** 이었고 `from` 에 개행이 든
`C1`·`C2`·`C4` 만 매칭에 실패했다. `git cat-file blob` 으로 LF 본을 덮어써 닫았고
(`git diff` 는 비어 있어 커밋에는 들어가지 않는다) 다음 실행에서 치환 실패가 **0** 이 되었다.

🆕 **치환 실패를 미리 셀 수 있다.** `from` 의 **런타임 값**에 개행이 있는지와 대상 파일의 CR 을
곱하면 된다 — 예측 3건이 실측 3건과 일치했다. 🔴 **소스의 표기로 세면 오탐한다** — `M5` 의
`from` 은 `"\\n"` 이라 화면에는 개행처럼 보이지만 실제로는 백슬래시와 n 두 글자다.
⇒ **새 뮤턴트의 `from` 은 한 줄로 잡아라.** 개행이 들어가면 이 환경에서 조용히 빠진다.

#### 낡아 있던 수 다섯을 함께 고쳤다

| 어디 | 적혀 있던 값 | 실제 |
| --- | ---: | ---: |
| `README.md` CI 스텝 | 28 | **34** (32 였던 동안에도 낡아 있었다) |
| `README.md` `check-forbidden:verify` | 63건 | **65건** |
| `README.md` `check-baseline:verify` | 16건 | **19건** |
| `HANDOFF.md` 「지금 상태」 검사기 | 열두 종 · 뮤턴트 112 | **열네 종 · 127** |
| `HANDOFF.md` 「지금 상태」 CI | 28스텝 | **34스텝** |

🔴 **`self-test` 유무로 「검사기」를 가르면 종 수가 둘 많아진다.** `map-terms` 와 `mutate` 는
자기 검사를 가졌지만 판정하지 않는다. 기준은 **위반을 찾아 종료 코드를 내는가**다.

#### 🆕 🔴 CHANGELOG 절을 월별 파일로 옮기면 **상대 링크가 깨진다**

본체는 최신 3개 절만 두는 규칙이라 새 절을 쓰면 가장 오래된 절을 `docs/changelog/` 로
내려야 한다. 그런데 링크의 **기준 디렉터리가 리포 루트에서 `docs/changelog/` 로 바뀌므로**
루트 기준으로 적힌 상대 경로가 전부 어긋난다. 실측으로 편8 절을 옮기자 **3곳**이 깨졌다.

| 옮기기 전 (본체) | 옮긴 뒤 (`docs/changelog/`) |
| --- | --- |
| `content/blog/…/편8.md` | `../../content/blog/…/편8.md` |
| `docs/changelog/2026-09.md` | `2026-09.md` (자기 자신이 된다) |

⇒ **절을 옮겼으면 `npm run check-links:docs` 를 반드시 다시 돌려라.** 옮기는 작업 자체는
내용을 한 글자도 바꾸지 않으므로 「고칠 것이 없다」고 넘기기 쉽다. 검사기가 없었으면
본체에서는 멀쩡하던 링크 셋이 조용히 죽은 채로 남았을 것이다.

### 앞선 세션이 한 일 — 발표본 배포를 Task 6 까지 끝냈다. 🔴 **CI 의 Node 가 20 이면 죽는다**

직전 세션이 멈춘 **Task 1 Step 8** 부터 이어받아 Task 6 까지 갔다. 계획서의 기댓값과 실측이
**여덟 자리에서 모두 일치**했고, 어긋난 것은 계획서 자신의 오기 하나(뮤턴트 id)였다.

| Task | 무엇을 했나 | 계획서 기댓값 대 실측 |
| --- | --- | --- |
| 1 (Step 8·9) | 뮤턴트 `SL1`~`SL4` 가 잡히는지 · id 겹침 | 넷 모두 `check-slides` 가 잡음 · 겹침 0 · **뮤턴트 116** |
| 2 | `package.json` 에 `build-slides` 등록 · 빌드 · 판정 | 발표본 4개 · **38초**(근사 56초) |
| 3 | `check-baseline` 이 `slides/` 를 뺀다 | 자기 검사 **19/19** · 대상 **15개** — 둘 다 기댓값 그대로 |
| 4 | `check-forbidden` 이 발표본 소스를 본다 | 자기 검사 **65/0** · 스캔 **195 → 208개** |
| 5 | `robots.txt` · 워크플로 스텝 넷 | build job **28 → 32스텝** |
| 6 | 전량 검증 · 문서 갱신 | 아래 |

#### 🔴 계획서에 없던 발견 — 발표본의 **운영** 의존성이 Node 22 를 요구한다

착수 전 `npm ci` 의 lock 파일을 확인하다 드러났다. 넷 전부 `dependencies` 이므로
`--omit=dev` 로 빠지지 않는다.

| 패키지 | `engines.node` |
| --- | --- |
| `commander@15.0.0` | `>=22.12.0` |
| `postcss-nested@8.0.1` | `^22 \|\| ^24 \|\| >=26` |
| `unplugin-vue-markdown@32.1.1` | `>=22` |
| `vite-plugin-static-copy@4.1.1` | `^22 \|\| >=24` |

**로컬이 Node 24 라 넷 모두 38초에 빌드된다.** 그래서 이 어긋남은 로컬에서 드러날 수 없었다.
사용자가 「워크플로 전체를 22 로」를 골라 `node-version` 을 한 줄 고쳤다.

⚠️ **본체 빌드도 함께 22 에서 돈다.** 산출물이 달라지면 GC-6 가 잡는데, 그 판정은 **PR 단계의
`Check non-blog baseline` 스텝**이 한다. 로컬에는 Node 20 이 없어 미리 재지 못했다.
⇒ **PR 의 CI 에서 그 스텝을 가장 먼저 보라.** 빨간불이면 원인은 이 한 줄이다.

🔴 **대조는 `check-mermaid` 의 자기 검사 ㉖ 에 이미 있는데 그것이 보는 것은 리포 루트의
`node_modules` 다.** `slidev-poc/node_modules` 는 아직 아무도 보지 않는다 — **남은 사각지대다.**

#### 계획서와 다르게 한 것 셋

| 무엇 | 계획서 | 실제 | 왜 |
| --- | --- | --- | --- |
| 뮤턴트 id | `F9` | **`C5`** | `F` 는 `fix-markup` 계열이고 `check-forbidden` 은 `C1`~`C4` 를 쓴다. `F9` 로 두면 다른 검사기의 뮤턴트로 오인된다 |
| Task 3·4 의 뮤턴트 확인 | 전량 실행 각 1회 | **격리 실행** | 전량이 28분이라 세 번 돌리면 84분이다. 확인할 명제는 「이 케이스가 이 뮤턴트를 잡는가」 하나이므로 나머지 116개는 무관하다. 상호작용은 Task 6 의 전량이 본다 |
| Task 2 Step 3 | `out/` 이 있으면 건너뛴다 | **`out/` 을 잠깐 옮겨 확인** | 건너뛰면 그 가드가 검증되지 않은 채 남는다. 어차피 Step 4 에서 재빌드한다 |

#### 되돌려 본 것 여섯 — 통과만 보고 넘긴 자리가 없다

| 무엇 | 어떻게 | 결과 |
| --- | --- | --- |
| `build-slides` 의 `out/` 가드 | `out/` 을 옮겼다 | 종료 코드 **2** |
| `check-slides` | `index.html` 하나를 지웠다 | **1** · 복구 뒤 **0** |
| `check-forbidden` 의 새 대상 | 발표본에 금칙어를 심었다 | HARD **3건** · 복구 뒤 **0** · `git status` 비어 있음 |
| 뮤턴트 `B8` | 격리 실행 | 19/19 → **18/19**, 잡은 것은 케이스 ⑱ |
| 뮤턴트 `C5` | 격리 실행 | 65/0 → **64/1** |
| 발표본 넷의 화면 | 정적 서버 + 브라우저 | 첫 장이 그려진다 · 요청 **960건에 404 0건** · 본체도 멀쩡 |

🔴 **격리 실행은 「치환이 실제로 일어났는지」를 먼저 센다.** 문자열을 못 찾으면 원본이 그대로
돌아 「FAIL 이 안 났다」가 「케이스가 지킨다」로 잘못 읽힌다. 스크립트가 `FROM 출현 1회` 를
확인하지 못하면 종료 코드 2 로 멈춘다.

#### 이번에 확정한 수 — 이어받지 말고 매번 세라

| 항목 | 값 | 어떻게 셌나 |
| --- | ---: | --- |
| 뮤턴트 | **118** | `B8` · `C5` 를 더하기 전 러너 요약이 116 이었고 `grep` 과 일치했다 |
| 검사기 종 수 | **13** | `scripts/*.mjs` 17개에서 판정하지 않는 **넷**을 뺀다 |
| 판정하지 않는 `.mjs` | **4** | `generate-sitemap` · `map-terms` · `mutate` · 🆕 `build-slides` |
| 뮤테이션이 돌리는 검사 | **13** | `mutate.mjs` 의 `CHECKS` 배열 |
| CI `build` 스텝 | **32** (PR 은 31) | YAML **파서**로 셌다. 정규식이 아니다 |
| 뮤테이션 소요 | **약 28분** | 「3~5분」이 낡아 있었다 |

🔴 **`README.md` 의 뮤턴트 수가 98 로 낡아 있었다.** `CLAUDE.md` 가 112 를 적는 동안 갈라져
있었고, 「세 문서가 갈라진다」는 경고의 **현재 사례**였다. 이번에 둘 다 118 로 맞췄다.

### 직전 세션이 한 일 — 발표본 배포의 설계와 계획을 쓰고 Task 1 을 구현했다

「다음 세션의 작업」에 남아 있던 셋 가운데 **11번(발표본 Pages 배포)** 을 골랐다. 배포 경로와
본체와의 분리 방식이 사용자 결정이었으므로 착수 전에 넷을 물어 확정했다.

| 결정 | 확정된 값 |
| --- | --- |
| 발표본의 위치 | 같은 도메인의 `/slides/<slug>/` 이되 본체에서 링크하지 않고 `robots.txt` 로 색인을 막는다 |
| 산출물 경로 | CI 에서 매번 빌드한다. 산출물을 커밋하면 627파일 19 MB 가 리포에 들어간다 |
| 금칙어 범위 | `check-forbidden` 을 확장한다. 별도 검사기에 두면 금칙어 목록의 소비처가 갈라진다 |
| 실패 전파 | 슬라이드 빌드가 깨지면 본체 배포도 막는다. `continue-on-error` 를 쓰지 않는다 |

**이 세션의 실측 다섯.** 전부 직접 쟀다.

| 항목 | 값 | 비고 |
| --- | ---: | --- |
| `npm ci --omit=dev` | 12.2초 | 로컬 캐시가 있는 상태다. CI 의 첫 설치는 더 걸린다 |
| 발표본 넷 빌드 | 56.3초 | `playwright-chromium` 없이 넷 모두 성공한다 |
| 산출물 크기 | 19 MB | 발표본마다 4.6 MB 로 고르다 |
| 산출물 파일 수 | 627 | js 560 · css 48 · html 8 |
| 발표본 소스의 금칙어 | 18종 0건 | 대조군에 심은 다섯 종을 프로브가 실제로 잡았다 |

🔴 **`check-baseline` 이 슬라이드를 「추가」로 잡는다는 것을 착수 전에 발견했다.** `isTarget` 은
`out/` 아래에서 `blog/` 로 시작하지 않는 모든 `.html` 을 대상으로 잡고 `diff` 는 추가도 위반으로
센다. 발표본 넷은 `index.html` 과 `404.html` 을 여덟 개 만들므로, 고치지 않고 배포하면 **직전
세션이 되살린 GC-6 가 즉시 빨간불**이 된다. 계획서 Task 3 이 그것을 닫는다.

**커밋 셋.** `58ef146` 설계서 · `bd030bc` 구현 계획 · `6dc5a39` Task 1 구현이다.

| 문서 | 무엇이 들어 있나 |
| --- | --- |
| [`2026-09-09-slides-deploy-design.md`](docs/superpowers/specs/2026-09-09-slides-deploy-design.md) | 결정 넷과 버린 대안 · 배포 구조 · 검사기 변경 상세 · 위험 여섯 · 실측 |
| [`2026-09-09-slides-deploy.md`](docs/superpowers/plans/2026-09-09-slides-deploy.md) | Task 여섯. 실제 코드와 검증 명령이 전부 들어 있다 |

⏳ **Task 1 은 Step 7 까지만 끝났다.** 만든 것은 `slidev-poc/decks.json`(목록의 진실원) ·
`scripts/check-slides.mjs`(판정) · `scripts/build-slides.mjs`(빌더, Task 2 에서 검증한다) 셋이고,
`package.json` 에 스크립트 셋과 `mutate.mjs` 에 뮤턴트 넷을 더했다. 자기 검사는 **9/9** 이며
`⑤` 를 일부러 깨뜨려 **8/9 로 FAIL 이 나오는 것까지 확인했다** — 통과만 보고는 케이스가 헛도는지
알 수 없다.

🔴 **남은 것은 Task 1 Step 8 이다 — 뮤테이션 전량이 `18/118` 에서 중단되었다.** 세션을 마치며
끊었으므로 `SL1`~`SL4` 가 실제로 잡히는지는 **아직 아무도 확인하지 않았다.** 뮤턴트를 등록한 것과
그 뮤턴트가 잡히는 것은 다른 말이며, 이 리포는 등록해 놓고 생존한 사례를 이미 겪었다.

⚠️ **중단 뒤 `lib/blog/graph.ts` 가 `M` 으로 남았는데 뮤턴트가 아니었다.** `git diff` 는 비어
있었고 내용도 정본과 같았다. **`autocrlf=true` 인 이 워크트리에서는 CRLF 가 정상**이라
(`loader.ts` 306 · `graph-layout.ts` 212 · `check-baseline.mjs` 424) LF 로 되돌린 것이 오히려
어긋난 상태였다. `git checkout --` 로 되돌리니 `M` 이 사라졌다. ⇒ **CR 이 0 인 것을 「깨끗함」의
증거로 읽지 마라.** 이 워크트리의 기준선은 0 이 아니다. 판정은 동료 파일과의 대조로 한다.

**다음 세션은 계획서의 Task 1 Step 8 부터 이어가면 된다.** 상태가 전부 계획서와 git 에 있다.

### 그 앞 세션이 한 일 — 꺼져 있던 검사기 하나를 되살렸다. 원인은 **크기가 같은 파일** 안에 있었다

`check-baseline`(GC-6)은 2026-08-18 부터 CI 에서 주석 처리된 채였고, 로컬에서는 언제나
**14건 위반**을 내어 신호가 되지 못했다. 직전 세션이 남긴 갈래 셋 가운데 ③ **환경 독립
재설계**를 골라 닫았다. 자세한 경위는 아래 「알려진 문제」의 해당 절에 있다.

**조사에서 판정이 갈렸다.** 그때 CI 주석은 「무엇이 다른지 먼저 실측하라」고 남겼는데,
아무도 그 실측을 하지 않은 채 「산출물이 환경에 따라 다르다」는 넓은 진단만 이어받았다.
실제로 재 보니 갈린 것은 **JS 청크 파일명의 콘텐츠 해시 하나**였다.

| 무엇을 어떻게 쟀나 | 결과 |
| --- | --- |
| 배포된 실물 = ubuntu CI 산출물. 같은 커밋(`8dfbf9c`)의 로컬 빌드와 `cmp` | `404.html` 은 **크기가 바이트까지 같은데** byte 1147 부터 다르다 |
| 그 자리를 읽는다 | `_app-38524cbfc4dc35b8.js`(CI) 대 `_app-64080f6e0509c202.js`(로컬) |
| 자산 종류별로 대조 (대조 대상 수를 함께 센다) | css 1/1 동일 · media 1/1 동일 · **chunks 10개 중 9개 동일** |
| 청크 해시를 마스킹한 뒤 비블로그 전량 | **15/15 일치** (검사기 자신의 `normalize` 로 재측정) |

세 번째 줄에서 한 번 헛디뎠다. 처음엔 `404.html` 로 폰트를 대조해 「같다」를 얻었는데
**양쪽 다 빈 값**이었다. 이 리포가 적어 둔 「대조할 것이 실제로 있는지를 먼저 세라」에
그대로 걸린 것이며, 대상 수를 함께 출력하도록 바꾸고 나서야 실제 대조가 됐다.

**되살리며 함께 갖춘 것.** 이 검사기에는 자기 검사가 없어 뮤테이션이 돌리지 않았고,
그래서 「검사기 종 수」에도 세어지지 않았다. `--self-test` **16케이스**와 뮤턴트 **B1~B7**
일곱을 만들고 `normalize()`·`diff()` 를 순수 함수로 뽑았다. 마스킹을 넣기 전에 자기 검사를
먼저 돌려 **①②만 실패하고 나머지 14가 통과**하는 것을 확인했다 — 케이스가 헛돌지 않는다는
증거를 남기려는 것이다.

⇒ 검사기가 **열한 종에서 열두 종**으로, 뮤턴트가 **105개에서 112개**로, CI `build` job 이
**26스텝에서 28스텝**으로 늘었다. 이 세 수가 적힌 자리를 같은 커밋에서 고쳤다.

**푸시 · PR · CI.**

| 항목 | 값 |
| --- | --- |
| 커밋 | `61fc280` (11파일 · +596 / −156) |
...
### 앞선 세션이 한 일 — 편8 을 발행해 시리즈를 닫고, 대조군을 둘 놓는 절차를 만들었다
...
| ~~11~~ | ~~발표본을 GitHub Pages 에 **실제로 배포한다**~~ | 낮음 | 중간 | 받았다 | ✅ **이번 세션이 계획서 여섯 Task 를 전부 닫았다.** 검사기 `check-slides` 를 세우고 `check-baseline`·`check-forbidden` 둘을 고쳤으며, CI 스텝이 28 에서 **32** 가 되었다. 🔴 **CI 의 Node 를 20 에서 22 로 올렸다** — 발표본의 운영 의존성 넷이 그것을 요구하는데 로컬이 Node 24 라 드러나지 않던 자리다. ⏳ **남은 것은 푸시와 PR 이며, PR 의 `Check non-blog baseline` 스텝을 가장 먼저 보라** — 본체 빌드도 22 에서 돌므로 산출물이 달라졌다면 거기서 잡힌다 |
...
| ~~7~~ | ~~`diagram-design` 후보 A · C~~ | 중간에서 높음 | 낮음 | 받았다 | ⬜ **사용자 판단으로 보류했다** (2026-09-06). 아래 **「후보 A 를 하지 않기로 한 근거」를** 읽고, 다시 꺼내려거든 그 여섯 줄을 먼저 반박하라. ⚠️ **2026-09-16 조사가 제약 1·2 의 우회로를 찾았으나 A 의 보류 근거는 건드리지 않았다** |
| 🆕 **14** | **다크 모드 해법을 정한다** — SVG 도식이 라이트 모드에서 검은 상자가 된다 | 중간 | **높음** | **필요** | 🎯 **2026-09-16 조사가 실물로 확인했다.** `prefers-color-scheme` 해법은 이 사이트에서 어긋난다 (테마의 진실원이 `localStorage`). 남은 것은 **두 벌**(자산 2배)과 **인라인 SVG**(새 컴포넌트, 금칙어 사각지대도 함께 닫힘) 둘이며 **사용자 결정이 필요하다.** 🔴 **이것을 닫기 전에는 SVG 를 늘리지 마라** |
| ~~15~~ | ~~**후보 D** — Mermaid 566개에 `classDef` 강조와 `direction` 을 넣는다~~ | 낮음 | 낮음 | 받았다 | ⛔ **2026-09-16(2차)에 사용자 판정으로 중단했다.** 50편 · 67개에 적용한 실물을 보고 「색생만 조금 바뀌고 … 이전과 다를바가 없어」라고 했다. 적용본은 로컬 브랜치 `feat/diagram-emphasis-direction`(`1962b2bc`, 미푸시)에 있고 **되살리지 않는다.** 후속은 17번이다. 아래는 중단 전의 기록이다 — 🎯 **사용자가 지적한 품질 문제의 해법이다.** 강조는 **566개 중 0개**, 방향 대상은 **137개**다. B 와 같은 성격이라 콘텐츠를 바꾸지 않고 검사기도 그대로 산다. ⏳ **몇 개에 넣을지가 결정 사항** — 전부인가, 강조할 노드가 있는 것만인가 |
| ~~16~~ | ~~**SVG 내부 텍스트 검사기** — 금칙어 사각지대~~ | 낮음 | 중간 | 불필요 | ✅ **2026-09-16(3차)에 닫았다** — 맨 위 「이번 세션」 표를 보라. 아래는 닫기 전의 기록이다 — 🔴 **배포된 SVG 안의 문자열은 금칙어 검사를 한 번도 받지 않았다.** 지금은 대상이 하나뿐이라 위험이 낮지만, 14번·15번 뒤에 SVG 가 늘면 같은 배수로 커진다. **15번을 먼저 하면 이 항목의 시급성이 낮아진다** · 🔼 **2026-09-16(2차): 15번이 중단되고 17번이 SVG 를 늘리므로 시급성이 다시 올랐다. 17번과 같은 PR 에 싣는 것을 권한다** |
| 🆕 **17** | **`## 4. 승인 지점을 하나로 모은다` 도식을 `diagram-design` SVG 로 다시 만든다** | 중간 | 낮음 | **실물 확인** | ⏹️ **2026-09-17 사용자 결정으로 넓히기 중단** — 남은 Mermaid 558개는 그대로 둔다. 도식마다 배치를 손으로 짜야 해 한 번에 끝낼 수 없기 때문이다. 만든 SVG 8개(플로우차트 6 · 상태 1 · 시퀀스 1)는 유지한다. 이하는 중단 전 기록이다. ✅ **2026-09-17 에 사용자 판정 통과** — 넓힌다. **순서:** ① 같은 편 `human-gate-and-model-routing` 의 남은 도식(✅ 2026-09-17, `agent-contract-sequence.svg`) → ② 같은 카테고리 `agentic-coding` 27편 · 도식 84개, **편 하나를 한 커밋**으로 하고 편마다 실물을 보인다. 첫 편은 본문이 링크하는 `context-budget-session`(도식 4개, ✅ 2026-09-17(2차) 플로우차트 넷 · 브랜치 `feat/diagram-svg-agentic-coding`), 둘째 편은 그 편이 가장 많이 링크하는 `claude-md-scope-layers`(도식 2개, ✅ 2026-09-17(3차) · 브랜치 `feat/diagram-svg-agentic-coding-2`). 남은 것은 25편 · 도식 77개다 → ③ 나머지 카테고리. **종류마다 생성 스크립트 형식이 다르다** — 상태 · 시퀀스는 한 번씩 만들었고 flowchart(발행본 542개)는 아직 한 번도 만들지 않았으므로, ②의 첫 편에서 flowchart 형식을 먼저 보인다. 판정 전 기록: `public/images/blog/human-gate-state.svg` 를 2026-09-16(3차)에 만들었다. 아래는 만들기 전의 기록이다 — 🎯 **사용자 원문:** 「svg 이미지로 만들어진 한눈에 보기 아래 도식은 다크/화이트 모드 모두 인상적이었어. 근데 4. 승인 지점을 하나로 모은다 아래 도식은 색생만 조금 바뀌고 … 이전과 다를바가 없어」. **좌표:** `content/blog/agentic-coding/human-gate-and-model-routing.md` 183행 · 기준 자산 `public/images/blog/multi-agent-system.svg`(`64861c7b`) · 렌더는 `components/diagram-image.tsx`. **순서:** 이 도식 **하나만** 만들고, 사용자가 직접 여는 **링크**로 라이트 · 다크를 보인 뒤에 넓힌다. ⚠️ **14번과 어긋나는 점:** 1차 조사는 이 SVG 가 라이트 모드에서 검은 상자로 남는다고 적었는데 사용자는 두 모드 모두 인상적이라고 판정했다 ⇒ 적어도 이 SVG 의 고정 색은 사용자 기준으로 결함이 아니다. 새 SVG 도 같은 방식으로 만들되 **도식의 뜻과 본문 글자는 바꾸지 않는다.** 🔴 16번(SVG 텍스트 금칙어)을 같은 PR 에 싣는다 |
| ~~18~~ | ~~**확대 뷰어가 도식을 화면 크기에 맞춰 연다**~~ | 낮음 | 중간 | 불필요 | ✅ **2026-09-16(3차)에 닫았다** — 맨 위 「이번 세션」 표를 보라. 아래는 닫기 전의 기록이다 — 🎯 **사용자 원문:** 「크게보기는 기능을 눌렀을 때 화면사이즈에 맞게 도식이 나와야 되는데 엄청 크게 나오는 경우가 있어」. **좌표:** `components/diagram-image.tsx`(SVG) · `components/mermaid.tsx`(Mermaid). 🔎 **가설(미검증):** `diagram-image.tsx` 17행 주석이 「scale 1 = 컨테이너 **폭**에 꼭 맞춤」이고 되돌리기 버튼 라벨도 「화면 **폭**에 맞추기」다 — 폭만 맞추므로 세로로 긴 도식은 높이가 화면을 넘는다. 폭 배율과 높이 배율 중 작은 쪽(contain)으로 여는 것이 맞는지, **세로로 긴 도식과 가로로 긴 도식 둘로 먼저 재현**하라. ⚠️ `components/system-diagram-card.tsx` 에도 「크게 보기」가 있다 — 비블로그 페이지를 건드리면 `check-baseline`(GC-6)이 빨간불이 되므로 범위에 넣을지 먼저 본다 |
| ~~19~~ | ~~**확대 화면에서 다시 클릭하면 닫힌다**~~ | 낮음 | 낮음 | 불필요 | ✅ **2026-09-16(3차)에 닫았다** — 맨 위 「이번 세션」 표를 보라. 아래는 닫기 전의 기록이다 — 🎯 **사용자 원문:** 「크게보기화면에서 다시 클릭하면 팝업 화면 닫히게 해줘」. **좌표:** 18번과 같은 두 컴포넌트의 `Dialog`. ⚠️ 확대 뷰어는 이동(드래그 · 핀치)을 지원하므로 클릭과 겹친다. **권장안:** 움직이지 않은 탭 · 클릭만 닫고, 이동 뒤에 손을 뗀 것과 툴바 버튼 클릭은 닫지 않는다. 실물에서 이동 · 확대 · 닫기를 각각 확인한다 |
| ~~20~~ | ~~**캡션의 「도식」을 도식 종류 이름으로 바꾼다**~~ | 낮음 | 낮음 | 불필요 | ✅ **2026-09-16(3차)에 닫았다** — 맨 위 「이번 세션」 표를 보라. 아래는 닫기 전의 기록이다 — 🎯 **사용자 원문:** 「도식 하단에 "도식을 탭하면 확대해서 볼 수 있습니다" 부분에 "도식"이라는 문장을 좀더 명확하게 … 플로우차트, 간트차트, 등등 도식의 이름으로 변경해주면 좋겠어」. **좌표:** `components/mermaid.tsx` 185행 · `components/diagram-image.tsx` 85행(캡션), `mermaid.tsx` 163행(`aria-label` 「도식 크게 보기」). Mermaid 는 첫 선언(`flowchart` · `sequenceDiagram` · `stateDiagram-v2` · `gantt` …)에서 종류를 얻는다. 🔴 **발행본에 실제로 쓰인 종류는 `check-mermaid.mjs` 의 `extractDiagrams` 로 세어** 이름 표를 만든다 — 정규식으로 세지 마라(함정 56·58). SVG 이미지는 선언이 없으므로 이름을 어디서 얻을지 정한다(권장안: 모르면 한 가지 이름으로 둔다) |
| ~~8~~ | ~~지역 그래프를 **카테고리 목록에도** 띄운다~~ | 낮음에서 중간 | 낮음 | 받았다 | ✅ **그 앞 세션이 A 안으로 닫았다.** 원인을 남긴 아래 조사 절을 보라 |
| ~~13~~ | ~~`check-baseline` 이 상시 빨간불이라 신호가 되지 못한다~~ | 중간 | 중간 | 받았다 | ✅ **이번 세션이 ③ 환경 독립 재설계로 닫았다.** 원인은 JS 청크 파일명 해시 하나였고 마스킹 뒤 두 환경이 15/15 일치했다. 자기 검사 16케이스 · 뮤턴트 B1~B7 · CI 두 스텝을 함께 갖췄다 |

**셋 가운데 하나(발표본 배포)를 닫았고 둘이 남았다. 이번 세션이 하나를 더 열었다.**

| 후보 | 비용 | 위험 | 무엇이 먼저 필요한가 |
| --- | :---: | :---: | --- |
...
| ~~발표본 슬라이드의 **넘침 검사기**~~ | — | — | 🗑️ **대상이 사라져 폐기했다** (2026-09-12). 사용자가 발표본 넷의 삭제를 지시해 갈래를 통째로 걷어냈다. 설계서까지 나와 있었으나 함께 지웠다 — 잴 슬라이드가 없는 검사기는 대조군이 없어 자기 검사가 헛돈다. 다시 만들 때는 아래 「넘침을 어떻게 쟀나」와 이 세션이 실측한 것(활성 장 선택자는 `.slidev-page[data-slidev-no="N"]` 이고, 장수의 진실원은 소스의 구분자가 아니라 `@slidev/parser` 다)을 먼저 읽어라 |
...
| ~~🆕 원격의 병합 완료 브랜치 정리~~ | 매우 낮음 | 낮음 | ✅ **2026-09-13 세션이 닫았다.** 사용자 승인을 받아 열 개를 지웠고 원격에는 `main` 하나만 남았다. 지우기 **전에** 브랜치마다 `main` 에 없는 커밋이 0 개인 것을, 지운 **뒤에** tip 열 개가 여전히 `origin/main` 에서 도달 가능한 것을 각각 확인했다 — 두 확인은 시점이 다르므로 하나로 대신할 수 없다 |
...
> 🗑️ **2026-09-12 에 발표본 갈래를 통째로 걷어냈다.** 아래는 다시 만들 때 쓸 기록으로 남긴다.
...
#### 🆕 제약 넷을 2026-09-16 에 다시 쟀다 — **둘이 이미 뚫려 있었다**

| # | 제약 | 지금 | 무엇이 바꿨나 |
| ---: | --- | --- | --- |
| 1 | raw HTML 미렌더 | ✅ **우회로가 있다** | `![alt](/images/blog/….svg)` **이미지 참조**는 `rehype-raw` 없이 동작한다. `components/markdown.tsx:119` 가 `img` 를 `DiagramImage` 로 보낸다. `64861c7b` 가 이미 그렇게 발행했다 |
| 2 | 분량 예산이 흔들린다 | ✅ **같은 우회로가 닫는다** | 이미지 참조는 `.md` 에 한 줄이라 밴드에 영향이 없다 |
| 3 | 다크 모드가 사라진다 | 🔴 **유효하고, 실물로 확인했다** | 라이트 모드에서 SVG 만 어두운 채로 남는다. 상단 요약을 보라 |
| 4 | 검사기가 무력해진다 | 🔴 **유효하다** | 수치를 **발행본 566 · 문서 136** 으로 갱신한다 (종전 549 · 132 는 낡았다) |

🔴 **그리고 제약이 하나 늘었다 — SVG 안의 텍스트는 어느 검사기도 보지 않는다.**
...
`links.test.ts` 의 주석도 이것을 인정하고 있다: 「`check-forbidden --built` 의 산출물 스캔도
`out/images/` 를 훑지 않는다」. ⇒ **SVG 를 늘리기 전에 이 자리를 먼저 정하라.**
...
| **C. 대표 도식 몇 개만 승격** | 편별 헤드라인 도식 | 1번 제약 때문에 `.md` 가 아니라 **컴포넌트**로 옮겨야 한다. 구조를 먼저 설계해야 하므로 A·B 뒤 ⚠️ **1번 제약은 2026-09-16 에 우회로가 확인되어 이 근거가 바뀌었다** — 위 갱신 표를 보라 |
| 🆕 **D. Mermaid 에 강조와 방향을 넣는다** | 발행본 **566개** 중 강조 대상과 방향 대상 **137개** | 🎯 **2026-09-16 조사가 새로 연 후보다.** B 와 같은 성격이다 — 콘텐츠를 한 글자도 바꾸지 않고 **렌더만** 개선하며, 검사기가 그대로 살아 있고 다크 모드도 그대로 따라온다 |

#### 🆕 후보 D — 사용자가 지적한 품질 문제의 해법은 여기에 있다 (2026-09-16)

**사용자의 지적:** 「`## 4. 승인 지점을 하나로 모은다` 하위 도식 같은 품질이 떨어지는 도식들이
있어서 검토해 볼 필요성이 있는 것 같다.」 **지적은 옳다.** 다만 원인은 형식이 아니다.

| 관찰 | 수치 | 판정 |
| --- | ---: | --- |
| 지목된 `stateDiagram` 자연 크기 | 354.6 × 742 | 🔴 세로가 가로의 **2.1배** |
| 본문 컨테이너 폭 | 895 px | 🔴 그중 **355 px 만** 쓴다 |
| 라벨 넘침 | **0건** | ✅ 잘림은 없다 |
| 노드의 시각 위계 | 여섯 노드가 전부 동일 | 🔴 본문이 강조하는 자리가 드러나지 않는다 |

리포 전량에서 같은 것을 세었다. **표기를 넷 다 세어도 0 이다.**

| 기능 | 566개 중 사용 |
| --- | ---: |
| `classDef` · `style … fill/stroke` · `:::className` · `class` 문 | **각각 0 · 합집합 0** |
| `flowchart` 가로 지정 (`LR`·`RL`) | 414 / 542 |
| `flowchart` 세로 (`TD`·`TB`·`BT`) | **128** |
| `stateDiagram` 의 `direction LR`·`RL` | **0** / 9 |
| **세로로 늘어지는 대상 합계** | **137** |

⇒ **`classDef` 두 줄과 `direction` 한 줄이면 형식을 바꾸지 않고 닫힌다.** 후보 C 로 가면
한 줄이면 될 것을 14 KB 재생성 대상으로 바꾸게 된다.

#### 🎯 권장 순서 — D 를 A·C 보다 앞에 둔다

| 순서 | 무엇 | 왜 이 순서인가 |
| ---: | --- | --- |
| 1 | **다크 모드 해법을 정한다** | 닫지 않고 SVG 를 늘리면 늘린 만큼 라이트 모드가 나빠진다 |
| 2 | **후보 D** — Mermaid 안에서 고친다 | 비용이 가장 싸고 검사기가 그대로 산다 |
| 3 | **후보 C** — 남는 것만 SVG 로 | D 로 닫히지 않는 것만 남는다 |

3단계의 대상을 기계 기준으로 좁히면 이렇다.
...
🔴 **다만 이 순서는 「A·C 보류를 뒤집었다」는 뜻이 아니다.** 보류의 근거였던 「결함이 실측되지
않은 자리를 그림체 때문에 갈아엎지 않는다」는 그대로 유효하다. 이번에 실측된 결함은
**발행본 도식**에 있었고 **SVG 전환 없이 닫히는 종류**였다. 홈 도식(후보 A)에서는 여전히
결함이 실측된 적이 없으므로 **A 는 보류 그대로다.**
...
### 0-c2. 🔴 **자기 검사가 없는 검사기는 「검사기 종 수」에서 조용히 빠진다**

`check-baseline` 은 `scripts/` 에 있고 `package.json` 에 등록돼 있는데도 세 문서가 모두
「검사기 열한 종」이라고 적는 동안 **그 열한에 들어 있지 않았다.** 기전은 단순하다.

```
scripts/*.mjs 는 15개
  → 그중 판정하는 것이 검사기다
    → 그중 :verify 가 있는 것만 mutate 의 CHECKS 에 오른다
      → 세는 사람은 CHECKS 를 센다        ← 여기서 빠진다
```

빠진 채로 **2026-08-18 부터 2026-09-09 까지 CI 에서 꺼져 있었다.** 아무도 목록에서 그것을
찾지 않았으므로 아무도 그 사실을 알아채지 못했다.

⇒ **검사기를 셀 때 `mutate.mjs` 의 `CHECKS` 만 보지 마라.** `ls scripts/*.mjs` 로 파일을 먼저
세고, 판정하지 않는 것(`generate-sitemap`·`map-terms`·`mutate`)을 빼는 순서로 세라. 두 수가
어긋나면 그 차이가 곧 **자기 검사가 없는 검사기**다.
...
`npm run mutate` 는 한 자리만 볼 때 쓰기에는 너무 오래 걸린다. 사본을 만들어 치환하고 돌린다.

🔴 **「3~5분」이라고 적혀 있던 것을 정정한다.** 2026-09-09 실측으로 **18개를 처리하는 데
여러 분**이 걸렸고 전량 118개는 그보다 한참 길다. 뮤턴트가 늘고(98 → 112 → 118) 검사가
늘어(열둘 → 열셋) 소요가 **곱으로** 커지는데 그 수치를 아무도 다시 재지 않았다.
⇒ **전량 실행은 세션 끝이 아니라 시작에 백그라운드로 걸어라.** 끝에 돌리면 이번처럼
중단하게 되고, 중단하면 무엇이 잡혔는지 알 수 없다.
...
### ① `scripts/` 의 열두 종 — 순서는 「증명한 뒤 스캔」이다

아래 건수는 **2026-09-09 에 각 `:verify` 를 직접 돌려 읽은 값**이다. 문서에서 이어받지 마라 —
직전 판에는 `compose` 와 `check-counts` 가 「—」로 적혀 있었으나 둘 다 자기 검사를 갖고 있다.
...
| `compose` | 성분별 분해 (분량 예산) | 15건 |
| `check-counts` | README·CHANGELOG 의 발행본 수 | 18건 |
| `check-markup` | 렌더되지 않는 강조 | 24건 |
| `fix-markup` | 위 위반의 교정 | 19건 |
| `check-links` | 깨진 링크 (R1·R2·R3) | 23건 |
| `check-mermaid` | 그려지지 않는 도식 (R1) | 26건 |
| `check-table` | 셀 수가 머리 행과 어긋난 표 행 | 22건 |
| `build-search-index` | 검색 인덱스 (가드 다섯) | **16건** |
| 🆕 `check-baseline` | 비블로그 산출물의 불변 (GC-6) | **16건** |

`scripts/*.mjs` 는 **15개**지만 검사기는 열두 종이다. `generate-sitemap`·`map-terms`·`mutate`
셋은 판정하지 않는다. 🔴 **파일을 세는 것과 검사기를 세는 것은 다른 일이며**, 그 차이 때문에
`check-baseline` 이 목록에서 빠진 채 1년 가까이 CI 에서 꺼져 있었다.

`npm run mutate` 로 뮤턴트 **112개**를 되살려 검사 **열두 개**(위 검사기 열두 종 중 `check-counts` 를
제외한 열하나 + `blog-unit` Vitest)의 자기 검사가 잡는지 본다. 검사기를 고쳤으면 반드시 돌린다.
2026-09-09 실측으로 **112개 전부 잡힘 · 생존 0 · 치환 실패 0** 이었다.
백그라운드로 돌리고 **다른 파일 작업과 병행하지 마라** — 러너가 검사기 파일을 임시로 바꾸므로
그 사이 다른 검사를 돌리면 뮤턴트가 든 파일을 검사하게 된다.
...
| **도식의 색 — 실물 대조** | 도식 **702개** 중 브라우저로 잰 것은 **10개** (🆕 종전 기재 681 은 낡았다) | ⚠️ 팔레트 계산은 전량에 걸리지만, 「테마 변수가 닿지 않는 색」은 **렌더해 봐야** 드러난다. 앞선 세션이 그렇게 하나를 찾았다. 남은 종류(`gantt`·`journey`·`erDiagram`·`quadrantChart`·`mindmap`)는 **문서에만 있어 발행되지 않으므로** 우선순위가 낮다 |
| 🆕 **SVG 안의 텍스트** | 현재 **1개** (`multi-agent-system.svg`) | 🔴 **어느 검사기도 열지 않는다.** `check-forbidden` 은 `content/blog`, `--built` 는 `out/blog` + `_next/data` 를 보는데 SVG 는 `out/images/blog/` 로 나간다. `tests/blog` 의 5케이스는 **경로와 실존만** 본다. 지금은 대상이 하나뿐이라 위험이 낮지만 **SVG 를 늘리면 같은 배수로 커진다** — 이것은 `Ted_yanadoo.png` 가 산출물 366곳에 남는 동안 소스 스캔이 0 을 반환하던 것과 같은 구조다 |
| 🆕 **도식의 시각 위계** | 발행본 **566개** 중 강조를 쓴 것 **0개** | 🔴 **기계가 판정할 수 없는 자리다.** 「본문이 강조하는 노드가 도식에서도 드러나는가」는 사람의 판단이며 `docs/superpowers/PUBLISHING-CHECKLIST.md` 쪽이다. 다만 **「강조가 하나도 없다」는 기계가 셀 수 있고 실제로 0 이었다.** 위 후보 D 를 보라 |
...
### ✅ `check-baseline` — ③ 환경 독립 재설계로 닫았고 CI 에서 되살렸다

직전 세션이 남긴 판단거리 셋 가운데 **③을 골랐다.** 근거는 조사 단계에서 나왔다.

**14건은 전부 의도한 변경으로 설명된다.** 전역 두 파일이 모든 페이지를 바꾸었고
(`827190f` 의 `body` 전경색 · `f943d2b` 의 `tailwind.config.js`), 거기에 `9051e90`(wiki-shell) ·
`56e8ebd`·`94af97c`(로드맵 문서) · `f87676d`(v2) 가 겹쳤다. `tving-brief` 는 롤백 다음 날
`24868d9` 로 들어온 새 페이지다.

🔴 **직전 세션이 적은 「기준선에 지금 없는 `notion/index.html` 이 남아 있다」는 사실이 아니었다.**
그 파일은 지금도 있고 **해시까지 일치한다** — `public/` 에서 그대로 복사되어 청크 참조가 없기
때문이다. 갱신 뒤에도 그 한 줄만 옛 해시 `080a859966f5ccc1` 그대로다.

**진짜 원인은 하나였다.** 배포된 실물이 곧 ubuntu CI 산출물이므로 같은 커밋(`8dfbf9c`)의
Windows 로컬 빌드와 대조했다.
...
| 파일 **크기** | **바이트까지 같다** | 크기 대조로는 영영 드러나지 않는다 |

청크 해시를 `buildId` 와 같은 방식으로 마스킹하니 **비블로그 15개가 두 환경에서 전량 일치**했다
(검사기 자신의 `normalize` 로 재측정해 15/15). `deploy.yml` 주석이 되살리는 조건으로 제시했던
「ubuntu 에서 기준선 생성」과 「CI 안에서 두 번 빌드」는 **둘 다 필요하지 않았다.**

**함께 갖춘 것.**

| 무엇 | 값 |
| --- | --- |
| 자기 검사 `check-baseline:verify` | **16케이스.** 넷이 마스킹이 넓어지지 않았음을 본다 (CSS·폰트·본문 16진수를 지우지 않는지, 본문이 바뀌면 여전히 잡는지) |
| 뮤턴트 | **B1~B7 일곱 개.** 마스킹이 **사라지는 쪽** 하나와 **넓어지는 쪽** 셋을 양방향으로 되살린다. 넓어지는 쪽이 더 위험하다 — 전량이 조용히 통과하며 초록불이 된다 |
| 순수 함수로 뽑은 것 | `normalize()` 와 `diff()`. 파일을 읽는 자리 안에 있으면 통째로 지워도 케이스가 전부 통과한다 |
| CI | `Prove baseline checker`(빌드 앞) · `Check non-blog baseline`(산출물 스캔 뒤) 두 스텝 |

🔴 **이 검사기는 「검사기 열한 종」에 세어지지 않고 있었다.** `scripts/` 에 있고 `package.json` 에
등록돼 있는데 `:verify` 가 없어 뮤테이션이 돌리지 않았고, 그래서 목록에서 조용히 빠진 채
**2026-08-18 부터 2026-09-09 까지 CI 에서 꺼져 있었다.** ⇒ **`scripts/*.mjs` 를 세는 것과
검사기를 세는 것은 다른 일이다.** 자세한 것은 [함정 53번](docs/TOOL-TRAPS.md) 에 적었다.
...
| 세션 | 어디 | 적혀 있던 값 | 실제 |
| --- | --- | ---: | ---: |
| 2026-09-09 (`check-baseline` 세션) | `HANDOFF.md` 「도구 함정 — 전체 N건」 | **51건** | **52건** — 함정 52번을 더한 세션이 `CLAUDE.md`(52 로 맞음)만 고치고 이 헤더를 빠뜨렸다. 같은 세션이 53번을 더해 지금은 **53건 1,556줄**이다 |
| 2026-09-09 (`check-baseline` 세션) | `HANDOFF.md` 「알려진 문제」의 `notion/index.html` 진단 | 기준선에만 있고 산출물에 없다 | **둘 다 있고 해시까지 일치한다** — 수치가 아니라 **사실 서술**이 틀린 사례다. 검사기가 삭제를 보고하는지 코드로 확인해 드러났다 |
...
## 도구 함정 — 전체 🆕 **58건**은 [`docs/TOOL-TRAPS.md`](docs/TOOL-TRAPS.md)(1,820줄) 에 있다
