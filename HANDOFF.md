...
> **이번 세션은 지역 그래프 위젯을 만들어 배포했다** (PR [#11](https://github.com/withwooyong/withwooyong.github.io/pull/11) merge · `e492613`).
> 본문 사이드바 하단에 현재 편을 중심으로 한 링크 그래프를 그린다.
> 🔴 **로컬에서 열 개 명령이 전부 초록이었는데 첫 CI 가 죽었다** — 아래 「CI 만 잡은 것」을 보라

## 지금 상태

발행본 **184편**이다. 배치는 편8로 끝났고 그 뒤 열 세션이 검사기의 사각지대를 닫아 왔다.
직전 세션이 CI 의 실행 시점을 고쳤고, 이번 세션은 **두 번째 기능**을 만들었다 —
링크는 921쌍이나 있었지만 독자가 그것을 볼 방법은 본문 안의 문장뿐이었다.

| 항목 | 값 |
| --- | --- |
| 발행 완료 | **A 편1~9 + B 편1~편8 (184편)** · 시리즈 B 완결 |
| 검사기 | ✅ `scripts/` **열 종** + `tests/blog/` **11파일 126케이스** · 뮤턴트 **65/65 잡힘 · 생존 0** |
...
| 깨지는 도식 | ✅ 발행본 **0곳**(544개) · 리포 문서 **0곳**(116개) — **판정 도달 전량** |
...
| 빌드 시간 | **55.9초** (2026-09-05 실측, 위젯 포함) — 이것이 기준선이다. 아래 「기준선을 남기는 이유」를 보라 |
| 분량 하한 SOFT | **3편** (세션 시작 5편) — 남은 셋은 **정당하다고 판정했다.** 아래 판정 표를 보라 |
| 커밋 | `e492613`(PR #11 merge) · 그 아래 `3f37672`·`4029095`·`69b7221`·`f943d2b`·`ed087d8`·`ce4656b`·`c6f4ba1`·`8f57400` |
| 푸시 · 배포 | ✅ **success** (run `33976481159` · `deploy` 11초 실측). 머지 방식은 **merge 커밋**이다 — 이 리포는 PR #5~#11 에서 squash 를 쓴 적이 없다 |
| 프로덕션 실측 | ✅ `withwooyong.github.io` 에서 위젯 224×261 · 이웃 6 · 연결선 19 · 노드 간 최소 **47.8 px** · 겹침 0 |
| 다음 단계 | 아래 **「다음 세션의 작업」** — 우선순위 표를 먼저 보라 |

### 이번 세션이 한 일 — 지역 그래프 위젯
...
⇒ **증분만 적고 기준선을 안 적으면 다음 세션이 같은 판단을 다시 한다.** 위 표의 55.9초가
그 기준선이다.
...
| **5** | 도식의 의미 검사 **660개** | 매우 높음 | 중간 | **필요** | 미착수 |
...
| 4 | **검사기가 무력해진다** | `check-mermaid` 가 발행본 **544개** · 문서 **116개**를 파서로 판정한다. HTML+SVG 로 바꾸면 그 660개가 **아무도 보지 않는 자리**가 된다. 대체 검사기부터 있어야 한다 |
...
| --- | ---: |
| `core.autocrlf` | `true` |
| **커밋된 blob 의 CR** | **222개** — 작업 트리만의 문제가 아니다 |
| 어긋난 줄의 실제 모양 | `"\r## 2026-09-04 — …"` — CR 이 줄 **앞**에 붙는다 |
...
| **도식의 의미** | 도식 **660개** (발행본 544 · 문서 116) | 문법은 지금 검사한다. 「화살표가 끊겼는가」·「라벨이 본문과 어긋나는가」는 사람의 판단이며 `docs/superpowers/PUBLISHING-CHECKLIST.md` 에 있다 |
...
### 지역 그래프 위젯에서 남긴 것 — 리뷰가 LOW 로 판정해 고치지 않았다

셋 다 프로덕션 화면에는 드러나지 않는다. 고치지 않기로 한 근거와 함께 남긴다.

| 자리 | 증상 | 왜 남겼나 |
| --- | --- | --- |
| `components/blog/local-graph.tsx:45-64` | 🔴 `reactStrictMode` 를 **켜는 순간** 마운트 애니메이션이 갇힌다. 1회차가 `started.current` 를 세우고 rAF 를 걸면 cleanup 이 그것을 취소하고 2회차는 즉시 반환한다. 화면에 남는 것은 「멈춘 애니메이션」이 아니라 **초기 원형 배치**이며, 힘 계산 결과가 아니다 | `next.config.js` 에 그 옵션이 없어 기본값 `false` 다. **켜면 조용히 깨지므로** 켤 때 이 줄을 먼저 보라 |
| `lib/blog/graph-layout.ts:64` | 없는 `centerId` 에 `indexOf.get(centerId) ?? 0` 으로 폴백해 0번 노드를 중심에 놓는다. 같은 상황에서 `graph.ts:119` 는 throw 한다 — **두 모듈의 방침이 갈린다** | `layout` 은 브라우저에서 돌아 throw 하면 화면이 깨진다. 방침을 통일하려면 어느 쪽으로 맞출지부터 정해야 한다 |
| `lib/blog/loader.ts:236` | `getLocalGraph` 가 `readPosts()` 를 한 번 더 부른다. fs+gray-matter 만 **46~51 ms/회**, 184편이면 약 **+9초** | 이번 변경 **이전에도** `getStaticProps` 가 `readPosts()` 를 5회 불렀다. 원인은 loader 에 문서 캐시가 없다는 것이지 이번 변경이 아니다 |

**E2E 에서 확인하지 않은 것 둘.** Enter 키로 노드 이동(네이티브 `<a href>` 이고 href 값은 맞다) ·
창 폭 1023px 에서 사이드바가 사라지는 것(`lg:block` 의 기존 동작이라 이번 변경과 무관하다).
