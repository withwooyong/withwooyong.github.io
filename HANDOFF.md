...
> **이번 세션은 지역 그래프를 카테고리 목록 페이지에도 띄웠다** (우선순위 8번 · A 안).
> 카테고리에는 중심으로 삼을 편이 정해져 있지 않아, 피인용이 가장 많은 편을 허브로 고르는
> `pickCategoryHubId` 를 신설하고 기존 `buildLocalGraph` 를 그대로 재사용했다. 콘텐츠는 한
> 글자도 바뀌지 않았고, `tests/blog` 가 **14파일 194케이스**로 · 뮤턴트가 **96개**로 늘었다.
> 🔴 **테스트도 빌드도 산출물 문자열도 통과한 뒤에 브라우저가 결함 하나를 더 냈다** —
> 캡션이 `h-8`(2줄)에 고정돼 있어 이웃 수가 통째로 잘려 있었다.
> ⚠️ **브랜치 이름은 `docs/pr16-deploy-record` 이지만 내용은 코드다.** 직전 세션이 문서
> 커밋 넷을 올려 둔 브랜치에 이번 실질 작업을 이어 실었다 — 문서 갱신만을 위한 PR 을 열지
> 않기 위해서다.
>
> **직전까지의 상태 —** 직전 세션이 `diagram-design` 후보 B 로 mermaid 도식 662개의 색을
> 사이트 팔레트로 바꾸고 대비 판정을 검사기에 넣었다. 그 세션의 PR [#16](https://github.com/withwooyong/withwooyong.github.io/pull/16) 은 merge 커밋
> `16c1107` 로 닫혔고 배포 run `34011795806` 이 **success**(전체 130초)였으며, 병합이 끝난
> `docs/session-handoff-branch-cleanup` 을 지워 **원격을 `main` 하나로** 되돌렸다.
> 🔴 그 세션에서도 **팔레트 계산이 위반 0 을 낸 뒤에 브라우저 실측이 결함 하나를 더 냈다**
> (mermaid 가 테마 변수를 우회해 속성으로 박는 색 · 다크 대비 **1.18** ·
> [함정 47번](docs/TOOL-TRAPS.md)). **두 세션 연속으로 브라우저가 마지막 결함을 냈다.**

## 지금 상태

발행본 **184편**이다. 배치는 편8로 끝났고 그 뒤 열두 세션이 검사기의 사각지대를 닫아 왔다.
직전 세션에 이어 이번 세션도 **읽는 경험**을 손댔다. 직전 것이 「같은 내용이 어떻게
보이는가」였다면 이번 것은 「글과 글의 관계를 어디서 볼 수 있는가」다. 지역 그래프가
본문에만 있어서, 카테고리를 훑는 독자는 그 카테고리의 지형을 볼 방법이 없었다.
...
| 깨지는 도식 | ✅ 발행본 **0곳**(544개) · 리포 문서 **0곳**(119개) — **판정 도달 전량** |
...
| 커밋 | 이번 세션 `ebdf12a`(코드 · 검사 · 수치 문서) · `5f3bced`(이 인수인계) · PR [#17](https://github.com/withwooyong/withwooyong.github.io/pull/17) 에 함께 실었다 · **브랜치 `docs/pr16-deploy-record` 에 직전 세션의 문서 커밋 넷(`e2bdbaf`·`3bbf61b`·`7500b39`·`17a3294`)이 먼저 올라 있다** · 그 앞은 `211fd55`(PR #16 → merge `16c1107`) · `45b2c12`(PR #15 → merge `827c3e2`) |
| 푸시 · 배포 | ✅ **`docs/pr16-deploy-record` 를 푸시하고 PR [#17](https://github.com/withwooyong/withwooyong.github.io/pull/17) 을 열었다** (커밋 일곱 · `main` 기준 선형). PR 단계 CI 는 run `34017972226` **success**(전체 **2분 30초** · `build` **success** · `deploy` **skipped** · `Upload artifact` **skipped** — 설계대로다). ⏳ **merge 는 아직 하지 않았다. 사용자 승인이 필요하다** — merge 커밋과 배포 run 은 뒤이은 세션이 채운다. 아래 행이 직전 세션까지의 기록이다 |
| 직전까지의 푸시 · 배포 | ✅ **PR [#16](https://github.com/withwooyong/withwooyong.github.io/pull/16) 이 merge 커밋 `16c1107` 로 닫혔다** (부모 둘: `827c3e2` + `7631c1c`). 배포 run `34011795806` 이 **success** 였고 전체 **130초** · `build` **115초** · `deploy` **9초** 였다. PR 단계의 CI 는 run `34010490249` **success**(전체 99초 · `build` **95초**)이며, `Upload artifact` 스텝과 `deploy` job 이 설계대로 **skipped** 되었다. 그 앞은 PR #15 의 run `34007702350`(150초), PR #14 의 run `34006877177`(150초), PR #13 의 run `34004539240`(153초)였다. 머지 방식은 **merge 커밋**이다: PR #11·#12·#13·#14·#15 에 더해 #16 도 부모가 둘이며, 이 리포는 squash 를 쓴 적이 없다 |
| 프로덕션 실측 | ✅ `withwooyong.github.io` 에서 이웃 **12** · 연결선 **18** · 중심거리 표준편차 **19.15**(원형이면 0 이므로 힘 계산이 실제로 돌았다) · 이웃 간 최소 간격 **42.05** |
| 브랜치 | ⚠️ **`docs/pr16-deploy-record` 는 이름과 내용이 어긋난다.** 직전 세션이 문서 커밋 넷을 올려 두고 푸시하지 않은 브랜치에, 이번 세션의 **코드 작업**을 이어 실었다 — 문서 갱신만을 위한 PR 을 열지 않기 위해서다(`CLAUDE.md` 의 규칙). 이름만 보고 문서 PR 로 판단하지 마라. 그 밖에는 **원격이 `main` 하나뿐**이며, 병합이 끝난 브랜치 36개를 지운 상태가 그대로다 |
| 다음 단계 | 아래 **「다음 세션의 작업」** — 우선순위 표를 먼저 보라 |

### 이번 세션이 한 일 — 지역 그래프를 카테고리 목록에도 띄웠다
...
### 직전 세션이 한 일 — 도식 662개의 색을 팔레트로 바꾸고 대비를 검사기에 넣었다
...
### 그 앞 세션이 한 일 — 기록의 재귀를 한 칸 좁히고 브랜치를 정리했다
...
| **5** | 도식의 의미 검사 **663개** | 매우 높음 | 중간 | 받았다 | 🎯 **다음 세션의 작업.** 사용자가 **「먼저 실태만 조사한다」를** 골랐다. 아래 착수 절차를 그대로 따르라 |
...
### 🎯 다음 세션의 작업 — 도식 의미 검사의 **실태부터 센다**

사용자가 선택지 셋 중 **「먼저 실태만 조사한다」를** 골랐다 (2026-09-06). 검사기를 먼저 만들지
않는 이유는 하나다: **위반이 나올 것을 전제로 검사기를 만들면, 0건이 나왔을 때 그것이 참인지
검사기가 헛도는 것인지 구분할 수 없다.** 이 리포가 `check-mermaid` 에서 이미 겪은 실패다.

#### 무엇이 검사되고 무엇이 검사되지 않나

```
발행본 mermaid 도식 544개 (리포 문서 119개는 별도)

  ✅ 문법       check-mermaid  ── 파서가 판정 · 판정 도달 전량 대조
  ✅ 색 · 대비   lib/contrast.ts ── 짝 43개 계산 + 브라우저 실측 10개
  ❌ 의미       아무도 보지 않는다 ── PUBLISHING-CHECKLIST.md §5 에 사람 판단으로만 있다
```

#### 종류 분포 — **96%가 한 종류다** (2026-09-06 실측)

| 종류 | 수 | 비율 | 그래프로 파싱되나 |
| --- | ---: | ---: | :---: |
| `flowchart` | **522** | 95.9% | ✅ |
| `sequenceDiagram` | 14 | 2.6% | 참여자와 메시지 구조가 달라 별도 |
| `stateDiagram-v2` | 8 | 1.5% | 별도 |

세는 방법은 ```` ```mermaid ```` 블록 **다음 줄의 첫 낱말**이다. 합이 544 로 블록 수와 맞는지
반드시 대조하라 — 맞지 않으면 선언 줄 앞에 지시문이 붙은 도식이 있다는 뜻이다.

#### 기계가 판정할 몫과 사람이 판정할 몫

| 판정 | 기계 | 왜 위험한가 |
| --- | :---: | --- |
| **유령 노드** | ✅ | 간선이 참조하는 노드 id 에 오타가 나면 **mermaid 가 빈 상자를 자동으로 만든다.** 문법 오류가 아니므로 `check-mermaid` 를 통과하고 화면에는 라벨 없는 네모가 그려진다 |
| **고립 노드** | ✅ | 선언했는데 어느 화살표에도 닿지 않는다. 체크리스트의 「화살표가 끊겼는가」 중 기계 몫이다 |
| **도달 불가 노드** | ✅ | 진입 간선이 없어 흐름에서 떨어져 나왔다 |
| 본문에 없는 라벨 | ⚠️ | 라벨의 낱말이 그 편 본문에 한 번도 없으면 후보일 뿐이다. 확정은 사람이 한다 |
| 화살표 방향 · 라벨이 본문과 어긋나는가 | ❌ | 사람 판단이다. 체크리스트에 남긴다 |

#### 착수 절차 — 순서를 바꾸지 마라

| 단계 | 무엇 | 판단 |
| ---: | --- | --- |
| 1 | 일회성 스크립트로 `flowchart` **522개**의 유령·고립·도달 불가를 센다. 스크래치패드에 두고 리포에 남기지 마라 | **0건이면 검사기를 만들지 않는 판단도 유효하다** |
| 2 | 위반이 나오면 **몇 건을 열어 눈으로 확인한다** | 실측으로 링크·마크업에서 **31건이 전부 거짓 양성**이었던 적이 있다 |
| 3 | 참이면 `scripts/` 에 기존 열 종과 같은 규격으로 검사기를 만든다 | 자기 증명 · **판정 도달 수 대조** · 카나리 · `decideExit` 순수 함수 |
| 4 | `scripts/mutate.mjs` 의 `CHECKS` 에 넣고 뮤턴트를 더한다 | 새 id 가 기존과 겹치지 않는지 `sort \| uniq -d` 로 확인하라 |
| 5 | 훅과 CI **양쪽에** 붙인다 | 훅만 있는 검사는 `--no-verify` 로 지나간다 |
| 6 | 검사기 종 수 · 뮤턴트 수 · `tests/blog` 파일과 케이스 수를 **직접 세어** 세 문서를 같은 커밋에서 고친다 | 이어받지 마라. 세 세션 연속으로 낡았다 |
...
| 4 | **검사기가 무력해진다** | `check-mermaid` 가 발행본 **544개** · 문서 **119개**를 파서로 판정한다. HTML+SVG 로 바꾸면 그 663개가 **아무도 보지 않는 자리**가 된다. 대체 검사기부터 있어야 한다 |
...
| **도식의 의미** | 도식 **663개** (발행본 544 · 문서 119) | 문법은 지금 검사한다. 「화살표가 끊겼는가」·「라벨이 본문과 어긋나는가」는 사람의 판단이며 `docs/superpowers/PUBLISHING-CHECKLIST.md` 에 있다 |
...
| **도식의 색 — 실물 대조** | 도식 **663개** 중 브라우저로 잰 것은 **10개** | ⚠️ 팔레트 계산은 전량에 걸리지만, 「테마 변수가 닿지 않는 색」은 **렌더해 봐야** 드러난다. 이번 세션이 그렇게 하나를 찾았다. 남은 종류(`gantt`·`journey`·`erDiagram`·`quadrantChart`·`mindmap`)는 **문서에만 있어 발행되지 않으므로** 우선순위가 낮다 |
