...
> 마지막 갱신 2026-09-06 · `pm-practice` 배치 **편2/8 발행** · 발행본 **186편**.
> **이번 세션은 첫 본편을 썼고 설계서의 예산 공식을 고쳤다** —
> `product-management/product-cycle-and-metric-hierarchy` (24,596 B).
> 설계서 §3-4 ②가 지정한 **재환산 지점**이었고 §8-1 의 여덟 검사를 전부 돌렸다. 실측은 §10-2 에 있다.
>
> 🔴 **k 는 옳았고 고정 비용 상수가 틀렸다.** 초고가 목표를 **16.6%** 넘겼을 때 §8-2 규칙 2 대로
> 절 목록이 아니라 **절별 배율**을 먼저 쟀더니 **초과의 59.4%가 배율 밖(고정 비용)에서** 나왔다.
> 공식의 상수 **2,455** 는 같은 카테고리 기존 본편 셋(3,429 · 3,278 · 4,530)의 어느 값에도 닿지
> 않았다 — 이 카테고리는 **「이 편이 쓴 용어」 절을 갖는 것이 관례**이기 때문이다.
> ⇒ **목표 B = 3,746 + 배정 원본 B × 1.95 + 링크 몫** 으로 고쳤고 **편3~편8 이 일률적으로
> +1,291 B 씩 올랐다.** 근거는 편2 자신이 아니라 **밖에 있는 대조군 셋**이다.
>
> 🔴 **가설 H-1 을 기각했다.** 「인용이 두 배로 크면 k 가 산문 축(1.888)으로 끌린다」였는데,
> 분해 방식 셋(2.104 · **1.933** · 1.972) **어느 것도 산문 축 아래로 내려가지 않았다.**
> 결론이 분해에 의존하지 않는 것이 판정의 근거다. ②의 1.933 은 채택값 **1.95 와 0.9% 차**다.
>
> ⚠️ **압축을 두 번 해야 했다.** 첫 전면 재작성은 **313 B** 밖에 줄이지 못했고(「이 편이 쓴 용어」
> 절은 오히려 늘었다), 문단을 통째로 삭제한 두 번째가 **1,713 B** 를 줄였다.
> ⇒ **10% 이상 넘쳤으면 다듬지 말고 처음부터 삭제할 문단을 고르고, 매번 다시 재라.**
>
> ⚠️ **`check-links` 가 185개만 스캔하고 위반 2곳을 냈다** — 편2 가 `git add` 전이라 색인에
> 없었다. 단서는 `check-markup`(186개)과의 **스캔 수 불일치**였다. 예고된 함정에 그대로 걸렸고
> 처방(`git add` 뒤 검사 · 직전 실행과 대조)도 그대로 들었다.
> ⚠️ **CR 계수를 `grep -c $'\r'` 로 재서 없는 문제를 쫓았다.** 정본은 `tr -cd '\r' | wc -c` 이고
> 실측은 처음부터 **0** 이었다.
>
> 🔴 **실물을 열어 두 가지를 잡았고 하나는 이 배치 밖의 문제다.**
> ① 편2 의 지표 사슬 도식이 `flowchart TD` + subgraph 둘이라 **214 × 874(비 0.25)** 로 세로로
> 늘어져 있었다. `TB` + subgraph 내부 `direction LR` 로 고쳐 **770 × 308(비 2.50)** 이 됐다.
> `check-mermaid` 는 문법만 보므로 **화면의 비례를 판정하지 않는다.**
>
> 🔴 ② **리포 전체의 도식에서 한글 라벨의 마지막 글자가 잘린다 — 편2 와 무관한 기존 결함이다.**
> mermaid 가 라벨을 `foreignObject` 로 그리는데 **폭을 재는 시점의 폰트와 실제 렌더 폰트가
> 달라** 상자가 2~7px 좁게 만들어지고, SVG 사양상 `foreignObject` 는 자기 폭 밖을 클리핑한다
> (`overflow: visible` 이어도 소용없다). 실측 표본 셋이 전부 같았다.
>
> | 편 | 라벨 | 넘침 | 비율 | 눈에 보이는 것 |
> | --- | ---: | ---: | ---: | --- |
> | `pm-practice-map` (편1) | 9 | **5** | 56% | 「꺼내는 장치」 → 「꺼내는 장」 |
> | `product-cycle-…` (편2) | — | — | — | 「순이익」 → 「순이읙」 · 「Discovery」 → 「Discover」 |
> | `search-system-overview` | 35 | **17** | 49% | 「수집/크롤러」가 5px 모자란다 |
>
> **이미 배포된 편들도 같은 상태이므로 사이트 전체에 걸쳐 있다.** 이 세션은 발견만 하고
> 고치지 않았다 — 원인이 `lib/mermaid-theme.ts` 의 폰트 지정과 Next 의 폰트 최적화가
> 만나는 자리에 있어 배치 작업의 범위를 넘는다. **다음 세션의 판단 항목이다.**
>
> **직전 세션이 한 일 —** 같은 배치의 **편1 지도편**(`pm-practice-map` · 21,803 B)을 발행했고
> 문서 셋을 반증했다 — 설계서 §9-2 의 `series.ts` 누락 · 「README 세 자리」가 실제로는 **네 자리** ·
> 가설 H-4 의 이유(중복 5쌍이 전부 `07` 한 파일 안이고 원본 넷 사이는 0쌍).
>
> **그 앞 세션이 한 일 —** 사용자가 후보 넷 가운데 **`pm-po` 선별 4개**(07 플랫폼 · 10 글로벌 협업 ·
> 11 데이터 드리븐 · 12 팀빌딩)를 골랐고, 산출은 **분할 설계서까지**였다.
> 🔴 **후보를 고르기 전에 금칙어 밀도를 먼저 쟀다.** 미소비 원본 10개 가운데 셋
> (`learning` 70.3 · `pangyo-it` 59.0 · `bigdata` 47.1 건/100KB)은 실명이 본문 전체에 퍼져 있어
> **조각을 빼는 방식이 통하지 않는다.** 고른 넷은 HARD 6건이 전부 경력 메모 블록 안이었고,
> 그 블록을 뺀 사본을 만들어 **실제로 0건임을 증명했다** — 추정으로 두지 않았다.
> 🔴 **`check-markup:docs` 가 새로 쓴 설계서를 보지 않은 채 「위반 0 · 59개」를 냈다.**
> 대상 수집이 `git ls-files` 라 **untracked 파일은 애초에 목록에 없다.** 아래 「도구 함정」을 보라.
>
> **직전까지의 상태 —** 직전 세션이 도식 663개의 의미를 조사해 **검사기를 만들지 않기로**
> 판정했다 (우선순위 5번). 그 세션의 PR [#18](https://github.com/withwooyong/withwooyong.github.io/pull/18) 은 merge 커밋 `0a0583d` 로 닫혔고
> 배포 run `34021838010` 이 **success**(전체 **2분 38초** · `build` 2분 21초 · `deploy` 10초)였다.
> 그 변경은 문서뿐이라 `out/` 에 들어가지 않아 **사이트 화면에는 달라진 것이 없다.**
> 콘텐츠 · 코드 · 검사기 종 수 · 뮤턴트 수 · `tests/blog` 가 전부 그대로였다.
> 🔴 세 세션 연속으로 **마지막 결함은 계산이 아니라 실물이 냈다** — 캡션 `h-8` 잘림 ·
> 다크 대비 **1.18** ([함정 47번](docs/TOOL-TRAPS.md)). 이번 세션은 콘텐츠를 만들지 않았으므로
> 그 확인이 **다음 세션으로 넘어간다.**
...
| 검사기 | ✅ `scripts/` **열 종** + `tests/blog/` **14파일 194케이스** · 뮤턴트 **96/96 잡힘 · 생존 0 · 치환실패 0** |
...
| ⚠️ CI 스텝 수를 세는 법 | `gh run view --json jobs` 는 `build` 를 **27스텝**으로 보고한다. 러너가 `Set up job`·`Post Setup Node`·`Post Checkout`·`Complete job` **넷**을 붙이기 때문이다. 문서의 **23** 은 워크플로 정의의 수이며 둘 다 맞다 — **다른 수를 대조하고 「문서가 낡았다」고 판단하지 마라** |
| 강조 렌더 실패 | ✅ 발행본 **0회 · 185편** · 리포 문서 **0회 · 60개** |
| 깨진 링크 | ✅ 발행본 **0곳** · 리포 문서 **0곳** |
| 깨지는 도식 | ✅ 발행본 **0곳**(544개) · 리포 문서 **0곳**(123개) — **판정 도달 전량** |
| 도식의 의미 | ✅ 🆕 **조사를 마쳤고 검사기를 만들지 않기로 판정했다.** flowchart **607개**(발행본 522 · 문서 85)의 유령 노드 · 고립 노드 · 성분 분리를 셌다. 고립은 **0건**이고 나머지 **56건은 전부 정당한 관용구**다. 아래 판정 절을 보라 |
| 앵커 실존 | ✅ **28개 전부 헤딩에 닿는다** — 훅과 CI 양쪽에서 돈다 |
| 검색 인덱스 | ⚠️ `out/blog/search-index.json` · **184편 · 헤딩 2,671개 · 311 KB** — 편1 발행 뒤 빌드를 돌리지 않아 **이 값은 편1 을 담고 있지 않다.** 다음 빌드에서 185편이 된다 |
...
| 커밋 | 이번 세션은 설계서 하나(`docs/superpowers/plans/2026-09-06-pm-practice-split-design.md`)와 이 인수인계 · 변경 기록을 **한 커밋**에 실었다. **콘텐츠도 코드도 건드리지 않았다.** 그 앞은 `36b5909` · `8f28dab` · `5dc1aeb` 가 PR [#18](https://github.com/withwooyong/withwooyong.github.io/pull/18) → merge `0a0583d` 로 닫혔고, `ebdf12a` 외 셋이 PR [#17](https://github.com/withwooyong/withwooyong.github.io/pull/17) → merge `572a849`, `211fd55`(PR #16 → merge `16c1107`) · `45b2c12`(PR #15 → merge `827c3e2`) 가 그 앞이다 |
| 푸시 · 배포 | ⏳ **`content/pm-po-domains` 브랜치를 `main`(= `0a0583d`)에서 갈랐다.** 푸시와 PR 은 사용자 승인 뒤에 한다 — merge 커밋과 배포 run 은 뒤이은 세션이 채운다. 아래 행이 직전 세션까지의 기록이다 |
...
| 브랜치 | `content/pm-po-domains` 다 (`main` = `0a0583d` 기점). ✅ 병합이 끝난 `docs/pr16-deploy-record` 와 `research/diagram-semantics` 를 **로컬과 원격 양쪽에서 지웠다** — 둘 다 `main` 대비 앞선 커밋이 0개임을 확인한 뒤였다. 지금 **원격에는 `main` 과 이번 작업 브랜치뿐**이며, 병합이 끝난 브랜치 **38개**를 지운 상태다. ⚠️ 로컬에는 다른 워크트리가 잡고 있는 `research/presentation-format` 이 있으나 원격에 없고 이 세션의 대상이 아니다 |
...
### 이번 세션이 한 일 — 새 배치의 원본을 **밀도로 골랐고**, 8편 설계서를 확정했다
...
### 직전 세션이 한 일 — 도식 663개의 의미를 세고, **검사기를 만들지 않기로 판정했다**
...
| ~~5~~ | ~~도식의 의미 검사 **663개**~~ | 매우 높음 | 중간 | 받았다 | ✅ **이번 세션이 조사를 마쳤다 — 검사기를 만들지 않는다.** 아래 판정 절을 읽고, 다시 꺼내려거든 거기 적힌 셋을 먼저 반박하라 |
...
| **6-b** | 위 배치의 **집필 8편** | 매우 높음 | — | **설계서 승인** | 🎯 **편1 발행 완료 (1/8).** 다음은 **편2**(`product-cycle-and-metric-hierarchy` · 07 §1~§5 · 목표 **22,764 B** · 밴드 21,310~24,218). 🔴 **편2 초고 `compose` 가 k 를 확정한다** — 표 축 1.957 과 산문 축 1.888 이 갈려 있고, 확정하면 편3~편8 목표를 전부 다시 계산한다 (설계서 §3-4 ②) |
...
| ~~8~~ | ~~지역 그래프를 **카테고리 목록에도** 띄운다~~ | 낮음~중간 | 낮음 | 받았다 | ✅ **이번 세션이 A 안으로 닫았다.** 위 「이번 세션이 한 일」과, 원인을 남긴 아래 조사 절을 보라 |
...
| 4 | **검사기가 무력해진다** | `check-mermaid` 가 발행본 **544개** · 문서 **123개**를 파서로 판정한다. HTML+SVG 로 바꾸면 그 667개가 **아무도 보지 않는 자리**가 된다. 대체 검사기부터 있어야 한다 |
...
| `tests/blog/mermaid-theme.test.ts` | 🔴 도식 팔레트의 대비 짝 · 획 교정 | 25 |
...
| **도식의 색 — 실물 대조** | 도식 **667개** 중 브라우저로 잰 것은 **10개** | ⚠️ 팔레트 계산은 전량에 걸리지만, 「테마 변수가 닿지 않는 색」은 **렌더해 봐야** 드러난다. 이번 세션이 그렇게 하나를 찾았다. 남은 종류(`gantt`·`journey`·`erDiagram`·`quadrantChart`·`mindmap`)는 **문서에만 있어 발행되지 않으므로** 우선순위가 낮다 |

⚠️ **기계만으로 닫히는 자리는 남지 않았다.** 새 배치는 설계까지 왔으나 집필이 사람의 판단을
계속 요구하고(익명화 · 출처 · 도식 근거), 도식의 색은 브라우저로 재야 드러난다.
...
## 도구 함정 — 전체 48건은 [`docs/TOOL-TRAPS.md`](docs/TOOL-TRAPS.md) 에 있다

🆕 **48번은 이번 세션이 겪었다** — `git ls-files` 로 대상을 모으는 `--docs` 세 검사가
