...
> 마지막 갱신 2026-09-06 · `pm-practice` 배치 **편1/8 발행** · 발행본 **185편**.
> **이번 세션은 새 배치의 첫 편을 썼다** — `product-management/pm-practice-map` (21,803 B · 목표 대비 **+3.8%**).
> 설계서 [`2026-09-06-pm-practice-split-design.md`](docs/superpowers/plans/2026-09-06-pm-practice-split-design.md) §8-1 의
> 여덟 검사를 전부 돌렸고 실측은 그 문서 §10-1 에 있다.
> 🔴 **문서 셋이 한꺼번에 반증됐고 셋 다 이번 커밋에서 고쳤다** — ① 설계서 §9-2 가
> `content/blog/series.ts` 등록을 빠뜨렸다 ② `CLAUDE.md` 와 §9-1 의 「README 세 자리」가 실제로는
> **네 자리**였다(넷째는 CHANGELOG 의 「총 N편」이며, 검사기는 통과 시 **「검사한 자리 4곳」** 을 출력한다)
> ③ 가설 H-4 의 이유가 틀렸다 — 용어 중복 5쌍이 **전부 `07` 한 파일 안**(§0 ↔ §6)이고
> **원본 넷 사이는 0쌍**이다.
> 🔴 **`source-overlap` 이 `dup-scan` 이 못 보는 것을 여덟 행에서 잡았다.** 용어 뜻풀이가
> 원본과 거의 같았던 자리들이며, 발행본끼리만 보는 `dup-scan` 은 구조적으로 볼 수 없다.
>
> **직전 세션이 한 일 —** 사용자가 후보 넷 가운데 **`pm-po` 선별 4개**(07 플랫폼 · 10 글로벌 협업 ·
...
발행본 **185편**이다. 앞 배치가 편8로 끝난 뒤 열네 세션이 검사기의 사각지대를 닫았고, 직전
세션이 새 배치의 설계서를 냈으며, **이번 세션이 그 배치의 첫 편을 발행했다.** 남은 것은
편2~편8 일곱 편이다.

| 항목 | 값 |
| --- | --- |
| 발행 완료 | **A 편1~9 + B 편1~편8 + `pm-practice` 편1 (185편)** · 새 시리즈 **1/8** |
