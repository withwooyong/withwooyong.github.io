---
title: "발행 순서와 읽는 순서는 다르다 — 31편 지도와 15주제 교차 참조"
description: "이 카테고리의 31편을 두 번 갠다 — 발행 구조 그대로인 8개 시리즈와 독립편 3으로 한 번, 깊이 순서의 15개 주제로 다시 한 번. 목적별 읽는 순서 여덟 갈래를 함께 놓고, 두 지도 어느 쪽에서도 빠지는 편이 없는지 자리를 세어 확인한다."
...
`agentic-coding` 카테고리에는 이 글을 뺀 **31편**이 있다. 한 편씩은 각자 완결돼 있다. 문제는 31편을 앞에서부터 통째로 읽으라고 하면 대부분 중간에서 멈춘다는 것이다 — 지금 필요한 것이 어디 있는지 모르는 채로 읽기 때문이다.

이 글은 그 31편의 지도다. 같은 31편을 **두 번 갠다.**

- **시리즈 축** — 발행 구조 그대로다. 8개 시리즈와 어디에도 속하지 않는 3편. 순서대로 읽을 때 쓴다.
- **주제 축** — 15개 주제 각각을 깊이 순서로 늘어놓는다. 열다섯 주제 모두 가장 깊은 자리가 차 있다. 아는 주제를 찾아갈 때 쓴다.

두 축이 어긋난다는 것이 지도가 필요한 이유다. 시리즈의 첫 편이 어떤 주제에서는 3차로 내려가는 자리가 **다섯**이다 — `claude-md-context`의 첫 편은 주제 7(컨텍스트·토큰 예산)에서 3차다. 그리고 어느 시리즈에도 속하지 않는 독립편 **둘**이 각각 주제 10(사람 개입과 승인 게이트)과 주제 15(조직 도입과 측정)의 1차다.

```mermaid
flowchart LR
    A["31편"] --> B["시리즈 축<br/>8 시리즈 + 독립 3"]
...
```mermaid
flowchart TD
    A["agentic-coding 31편"] --> B["기반 — 도구와 규약"]
    A --> C["설계 — 정의서와 팀"]
    A --> D["운영 — 통제·배포·확장"]
    A --> E["답부터 — 질문에 결론부터"]
    A --> F["독립편 3"]
...
| — | 독립편 | 3 | 어느 시리즈에도 들어가지 않는다 |
| | **계** | **31** | |

여덟 시리즈가 28편이고 독립편이 3편이다. 묶음 이름 넷(기반·설계·운영·답부터)은 이 글이 붙인 것이지 각 시리즈가 스스로 선언한 상위 분류가 아니다.
...
| [승인 지점 하나와 모델·effort 배정](/blog/agentic-coding/human-gate-and-model-routing/) | 사람이 멈춰 세우는 자리를 하나로 모으고, 각 에이전트와 작업에 모델과 사고 깊이를 배정한다 |

여기까지가 31편 전부다 — 4 + 3 + 3 + 3 + 3 + 4 + 4 + 4 + 3으로 합이 31이다. 각 칸의 한 줄 설명은 그 편이 무엇을 다루는지 가리키기만 한다. 표와 수치는 전부 해당 편 안에 있다.

## 목적별 읽는 순서

시간이 없으면 전부 읽지 않는다. 목적을 하나 고르고 네다섯 편에서 멈추는 편이 낫다.

> **여덟 경로의 순서는 이 글이 짠 것이다.** 각 편이 스스로 선언한 선행 관계가 아니라, 31편을 놓고 「무엇을 먼저 알아야 다음이 읽히는가」로 배열한 결과다.
...
| **여럿을 함께 굴린다** | [협업 패턴과 격리 3계층](/blog/agentic-coding/collaboration-patterns-isolation/) → [팀 운용 규칙](/blog/agentic-coding/agent-team-operations/) → [승인 지점 하나와 모델·effort 배정](/blog/agentic-coding/human-gate-and-model-routing/) → [라우팅 붕괴와 구간별 운영](/blog/agentic-coding/scaling-routing-collapse/) → [에이전트 설계 Q&A](/blog/agentic-coding/agentic-coding-qna-agent-design/) | 묶는 구조 → 첫날 멈추는 자리 → 사람이 서는 자리와 각 자리의 단가 → 개수가 늘었을 때 |
...
여덟 경로의 자리를 다 세면 **35개**다. 그중 네 편이 두 경로에 겹쳐 나오므로 중복을 걷어내면 **31편 전부**가 된다. 두 번 나오는 넷은 내장 도구 편·`settings.json` 편·3계층 통제 편·enforcement 편이고, 앞의 셋은 「거버넌스와 보안을 본다」 경로에서 한 번 더 불린다.

## 주제에서 글로 — 15주제 교차 참조

같은 주제가 여러 편에 흩어져 있다. 주제를 알고 들어오는 경우에는 이쪽이 빠르다.

> **주제 열다섯 중 열하나와 1차·2차·3차라는 깊이 3단은 원 자료의 교차 참조표에서 받았다. 나머지 주제 넷을 세우고 서른한 편을 각 자리에 배정한 것이 이 글의 판정이다.** 각 편이 그 주제를 **글의 주어로 삼는가**(1차) · **한 절로 다루는가**(2차) · **곁가지로 언급하는가**(3차)로 갈랐다. 분량은 보조 근거로만 썼다. 다른 기준으로 재면 순서가 달라질 수 있다.
>
...
| 10 | 사람 개입과 승인 게이트 | [승인 지점 하나와 모델·effort 배정](/blog/agentic-coding/human-gate-and-model-routing/) | [30개 에이전트 카탈로그](/blog/agentic-coding/thirty-agent-catalog/) | [에이전트 설계 Q&A](/blog/agentic-coding/agentic-coding-qna-agent-design/) | [정의서 공통 스키마](/blog/agentic-coding/definition-writing-quality/) · [운영과 거버넌스 Q&A](/blog/agentic-coding/agentic-coding-qna-governance/) |
...
| 12 | 비용 통제 | [라우팅 붕괴와 구간별 운영](/blog/agentic-coding/scaling-routing-collapse/) | [MCP 도입과 예산](/blog/agentic-coding/mcp-selection-practice/) | [자율성 3세대](/blog/agentic-coding/claude-code-autonomy-tiers/) | [운영과 거버넌스 Q&A](/blog/agentic-coding/agentic-coding-qna-governance/) · [승인 지점 하나와 모델·effort 배정](/blog/agentic-coding/human-gate-and-model-routing/) |
...
| 「함께 볼 것」 자리 | 11 |
| **배정한 자리 계** | **56** |
| 깊이 순서(1·2·3차)만 세었을 때 편 수 | 28 |
| 중복을 걷어낸 편 수 | **31** |

쉰여섯 자리에 서른한 편이 들어간다. **어느 편도 이 표에서 빠지지 않는다** — 단 깊이 순서만 세면 스물여덟 편이고, 나머지 세 편은 깊이 순서 밖인 「함께 볼 것」 열에만 있다.

1차 자리 열다섯이 모두 찼다. 그것을 채우는 편은 **열넷**이다 — 라우팅 붕괴 편이 비용 통제와 확장·스케일링 두 주제의 1차를 겸한다. 나머지 **열일곱 편은 1차를 한 번도 받지 못했다.** 그중 열넷은 2차나 3차로 들어가고, 어떤 편은 여러 주제에서 여러 번 불린다. 남는 세 편 — [확장 메커니즘 Q&A](/blog/agentic-coding/agentic-coding-qna-extensions/) · [운영과 거버넌스 Q&A](/blog/agentic-coding/agentic-coding-qna-governance/) · [JD 5필드와 트리거 셋](/blog/agentic-coding/agent-jd-triggers/) — 은 1차도 2차도 3차도 아니고 「함께 볼 것」으로만 불린다.

### 주제 10의 1차가 채워진 것에 대해

**이 지도가 처음 쓰였을 때 주제 10의 1차는 비어 있었다.** 사람이 승인하는 지점을 글의 주어로 삼는 편이 이 카테고리에 없었고, 열다섯 주제 중 1차가 비어 있는 유일한 자리였다.

그때의 판단은 **빈칸을 그대로 두는 것**이었다. 2차를 1차로 올려 적으면 없는 깊이를 있다고 말하는 것이 되고, 그 편을 찾아간 독자가 기대한 것을 찾지 못하기 때문이다. 빈칸이 보이는 편이 낫다고 적었다.

[승인 지점 하나와 모델·effort 배정](/blog/agentic-coding/human-gate-and-model-routing/) 편이 그 자리를 채웠다. 사람이 멈춰 세우는 자리를 하나로 모으는 것을 글의 주어로 삼고, 되먹임 경로가 각각 어디로 가는지와 단계마다 무엇을 넘기는지를 계약으로 고정한다. **빈칸을 보이게 둔 것이 그 자리를 채울 편을 부른 셈이다.**

나머지 네 편은 여전히 같은 주제를 다른 깊이에서 받친다. [30개 에이전트 카탈로그](/blog/agentic-coding/thirty-agent-catalog/) 편은 사람 검수 필요도로 서른 종을 세어 본 절을 따로 두고, [에이전트 설계 Q&A](/blog/agentic-coding/agentic-coding-qna-agent-design/) 편은 사람 손이 마지막으로 닿는 자리를 어디에 둘 것인가를 문항 제목 둘로 받는다. [정의서 공통 스키마](/blog/agentic-coding/definition-writing-quality/) 편은 서른 개의 정의서 중 승인 게이트를 갖춘 것이 몇 개인지를 다른 항목들과 나란히 세고, [운영과 거버넌스 Q&A](/blog/agentic-coding/agentic-coding-qna-governance/) 편은 그 게이트를 사전·사후 두 시점으로 가른다.

**그래서 이 자리는 두 번 판정됐다.** 처음에는 비어 있다고 적었고, 지금은 채워졌다고 적는다. 지도의 값어치는 자리를 다 채우는 데 있지 않고 **어느 자리가 비어 있는지 보이게 하는 데** 있다. 빈칸을 2차로 메워 두었다면 이 자리는 지금도 비어 있었을 것이다.
...
앞쪽 용도라면 **목적별 경로에서 하나를 골라 네다섯 편을 읽고 멈추는 것**이 31편을 순서대로 훑는 것보다 낫다. 뒤쪽 용도라면 **주제 교차 참조 표에서 주제를 찾아 1차부터 내려가면** 된다. 두 축이 어긋나는 자리 — 시리즈 첫 편이 어떤 주제에서는 3차인 자리 — 가 있어서 한쪽만으로는 찾아지지 않는 편이 생긴다.
