...
![한 사람이 통제하는 멀티 에이전트 시스템 — 사람이 목표와 최종 승인을 쥐고, 기획·코드·리뷰 에이전트가 파이프라인을 이루며, 테스트·배포 에이전트가 그 파이프라인을 되먹인다](/images/blog/multi-agent-system.svg "구조도")
...
![승인 지점을 하나로 모은 상태 전이 — 계획됨 · 구현중 · 검증중 · 리뷰중은 에이전트 사이에서 전이하고, 사람이 개입하는 상태는 사람대기 하나뿐이다](/images/blog/human-gate-state.svg "상태 다이어그램")
