...
| 한영검색 | 자판 오타(한/영 전환 실수)를 교정해 검색. 예: "tjf" → "설", "샬" → "shal" |
| 무결과(Zero-result) | 검색 결과가 0건인 상황과 그 후처리(추천·오타교정·동의어 확장 등) |
| IDOL | Autonomy(→HP) 계열 상용 검색·분석 엔진. SKB에서 3SoftPlus를 통해 도입해 사용 |
| 코난 | 코난테크놀러지의 상용 한국어 검색엔진. TVING(CJ헬로비전) 시절 사용 |

## 한눈에 보는 검색 이력 타임라인

검색 시스템을 **서로 다른 회사·규모·스택으로 두 번** 구축했다는 점이 핵심 차별점이다.

| 시기 | 회사 | 서비스 | 검색엔진 | 역할·핵심 산출물 |
|---|---|---|---|---|
| 2012.06~2017.04 | CJ헬로비전 | TVING(OTT) | **코난테크놀러지** | CMS·검색·랭킹추천 구축, N-Screen 통합 API(MongoDB), 색인 파이프라인·검색 API 서빙 |
| 2017.04~2021.06 | SK브로드밴드 | BTV N-Screen | **IDOL → Elasticsearch** | 상용엔진(IDOL)에서 ELK로 전환 주도, 한영·자동완성·초성·무결과 처리 직접 구현 |

```mermaid
flowchart LR
    A["코난 검색엔진<br/>(상용·규격서 기반)<br/>TVING / CMS·랭킹추천"] --> B["IDOL 검색엔진<br/>(상용·3SoftPlus 도입)<br/>SKB / BTV N-Screen"]
    B --> C["Elasticsearch / ELK<br/>(오픈소스·직접 구현)<br/>SKB / 한영·자동완성·초성"]
```

`상용 규격서에 의존하던 검색`에서 `오픈소스를 직접 뜯어 규격을 스스로 정의하는 검색`으로 넘어온 것이 내 검색 커리어의 성장 축이다.

## TVING 시절 — 코난테크놀러지 기반 검색
...
- 자동완성·한글처리·랭킹 같은 기능이 **엔진 기능으로 제공**되어, 내 일은 "규격에 맞춰 설정하고 API화"하는 쪽에 가까웠다.
- 덕분에 검색 기능의 **요구사항과 사용자 기대치**를 먼저 체득했다. 이 감각이 다음 회사에서 직접 구현할 때 결정적으로 쓰였다.

## SKB 시절 — IDOL에서 Elasticsearch로

### 전환의 배경

- BTV N-Screen 검색은 처음에 **3SoftPlus를 통해 도입한 IDOL**(상용 엔진)을 사용했다.
...
- 이 경험이 "검색을 두 번, 서로 다른 스택(코난·IDOL 상용 ↔ ES 오픈소스)으로 구축했다"는 서사의 뼈대가 됐다.
