...
| 원칙 | 의미 | 실무 함의 |
|---|---|---|
| **① 사람은 게이트, AI는 드래프트** | 생성·탐색·1차 검증은 AI, *승인·책임·방향 결정*은 사람 | 모든 단계에 "사람이 승인하는 체크포인트"를 명시 — [요구사항에서 운영까지 여섯 게이트와 개발·QA·배포 각론](/blog/ai-transformation/sdlc-human-gates/) |
...
| **③ 측정 가능한 것만 확산** | "빨라진 것 같다"가 아니라 리드타임·결함률·재작업률로 검증한 뒤 확산 | 성숙도 모델·KPI — [팀 공유 인프라 넷과 성숙도](/blog/ai-transformation/enablement-infra-and-maturity/) |
...
| 측정 가능한 것만 확산 | "빨라진 것 같다"는 확산 근거가 못 된다 | 30일: 리드타임·결함률·재작업률을 확산 게이트로 문서화 |
