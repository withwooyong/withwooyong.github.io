---
title: "같은 이름이 편에 따라 다른 층위를 가리킨다 — 열 편 지도와 열네 주제 교차 참조"
description: "이 카테고리의 열 편을 세 번 갠다 — 목적별 읽는 순서 일곱 갈래, 다루는 범위로 다섯 묶음, 깊이 순서의 열네 주제로. 「하네스」·「오케스트레이터」·「사람 게이트」·「총계 30」처럼 편에 따라 가리키는 층위가 갈리는 이름 넷이 어디서 정의되는지 함께 놓고, 세 지도 어느 쪽에서도 빠지는 편이 없는지 자리를 세어 확인한다. 열네 주제 중 하나는 가장 깊은 자리가 비어 있고, 그 빈칸을 채우지 않고 그대로 둔다."
category: "ai-transformation"
tags: ["org-design", "knowledge-management", "ai-automation", "human-in-the-loop"]
date: "2026-08-17"
updated: "2026-08-17"
featured: false
draft: false
...
```mermaid
flowchart LR
    A["ai-transformation 열 편"] --> C["목적 축<br/>일곱 경로"]
    A --> B["묶음 축<br/>다섯 묶음"]
    A --> D["주제 축<br/>열네 주제 · 깊이 2단"]
    C --> C1["지금 할 일이<br/>정해져 있을 때"]
    B --> B1["어디까지 다루는 편인지<br/>범위로 찾을 때"]
    D --> D1["아는 소재를<br/>찾아갈 때"]
```
...
```mermaid
flowchart TD
    A["ai-transformation 열 편"] --> B["조직을 무엇으로 보나 — 1편"]
    A --> C["일이 지나가는 단계 — 2편"]
    A --> D["팀이 딛는 바닥 — 2편"]
    A --> E["부서를 에이전트로 — 4편"]
    A --> F["답부터 — 1편"]
    E --> E1["공통 골격 + 부서 각론 둘<br/>시리즈로 선언됨"]
    E --> E2["부서별 정의서<br/>시리즈 밖 · 편성이 다른 세트"]
```
