...
**구조적으로 말하면** 순수 RAG는 **DAG(비순환 그래프)**이고, Agentic RAG는 **사이클이 있는 그래프**다. LCEL의 `|`는 왼쪽 출력을 오른쪽 입력으로 넘기는 연산이라 사이클을 표현할 수 없다. 조건 분기와 반복을 표현하려면 상태(State)를 들고 노드 사이를 오갈 수 있는 실행 모델이 필요하고, 그것이 **LangGraph**다. LCEL 체인과 LangGraph가 각각 어떤 작업에 맞는지는 LangGraph의 실행 모델을 다루는 편에서 축별로 정리한다.
