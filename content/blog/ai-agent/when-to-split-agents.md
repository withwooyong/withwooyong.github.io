...
>
> 실제로 아래 두 사례는 도구가 이질적인데도 쪼개지 않았다. 기준은 도구의 종류가 아니라 **책임의 분리 필요성**이다. 검색·이미지 생성·코드 실행·문서 검색을 한 에이전트가 전부 쓰면서도 쪼개지 않은 사례는 [ChatGPT를 라우팅 구조로 재현한 편](/blog/ai-agent/chatgpt-as-tool-router/)에 있는데, 거기서도 판정 기준은 프롬프트 충돌 여부 하나였다.
