...
다음 단계는 이 부품들로 실제 제품을 해체해 보는 것이다. Perplexity의 검색 흐름, ChatGPT의 라우팅 구조, 리포트 자동화, 코딩 에이전트 — 전부 지금까지 본 조각의 조합이다. [이어지는 시리즈](/blog/ai-agent/reverse-engineering-agent-graph/)에서 화면만 보고 그래프를 되짚어 그리는 절차부터 세우고, 제품을 하나씩 뜯는다.

언제 쪼개고 언제 쪼개지 않는지, Supervisor가 왜 병목인지, 계층에서 상태가 섞이는 문제를 어떻게 푸는지는 [멀티에이전트 Q&A](/blog/ai-agent/ai-agent-qna-multi-agent/)에 문답으로 정리돼 있다.
