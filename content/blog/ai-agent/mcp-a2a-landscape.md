...
**같은 지표에 두 값이 있는 것은 오기가 아니다.** 두 화면의 캡처 시점이 달라서 생긴 차이이고, 이 수치를 인용할 때는 하나를 고르는 것이 아니라 **"2025년 4월 기준 4천 건대"**라고 범위로 말하는 편이 정확하다. 단일 숫자를 단언하는 순간 어느 캡처를 봤느냐에 따라 반박당한다.

그리고 이 지표는 빠르게 움직인다. 같은 레지스트리의 3개월 뒤 화면은 **7,538개**를 표시했다 — 3개월에 65% 증가다. 그 시점의 기록과 그것이 현장에서 만든 부작용은 [MCP와 RAG 패러다임의 재편 편](/blog/ai-agent/mcp-rag-as-service/)에 있다. **두 수치를 나란히 인용할 때는 시점을 반드시 붙여야 하고, 어느 쪽도 현재형으로 말하면 안 된다.**
...
>
> 앞의 것은 설문 응답자 중 "현재 사용 중"이라고 답한 비율이고, 뒤의 것은 발표자가 현장에서 관찰한 **프로덕션 적용률**에 대한 서술이다. 둘을 섞어 "설문에서도 10%, 현장에서도 10%니까 신뢰할 만하다"고 말하면 같은 근거를 두 번 센 것이 된다. 이 표를 인용할 때는 어느 쪽 근거인지 반드시 명시해야 한다.
...
>
> AIME 2025에서 o4-mini는 도구 없이 92.7%, 파이썬을 쓰면 99.5%다. 오답이 7.3%에서 0.5%로 줄었다 — 모델 세대를 하나 올려 얻기 어려운 폭이다. 이것이 에이전트 아키텍처의 경제적 근거이고, 그래서 조건을 지우고 "99%"만 말하면 근거 자체가 사라진다. 남는 것은 모델 자랑뿐이다.
