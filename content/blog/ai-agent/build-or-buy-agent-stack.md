...
> 이 글은 **2025년 5월부터 10월까지의 자료를 정리한 것**이다. 서베이는 2024년 조사분이고, 도입 기업 목록과 제품 상태는 2025년 시점이다. 등장하는 회사 사례와 수치는 모두 원 자료의 것이다.
...
표에 붙은 기호가 근거의 성격을 가른다. **◎는 원 자료에 명시된 내용이고, ○는 그 근거로 구성한 판단이다.** 두 근거는 성격이 달라 함께 묶이지 않는다.
...
아홉 축 중 ◎가 붙은 것은 여섯 자리뿐이고 전부 ②와 ③에 몰려 있다. **①에 ◎가 하나(모델 교체 비용)뿐이라는 사실 자체가 이 자료의 입장을 드러낸다** — 직접 구현은 비교 대상으로 놓였지 권장 대상으로 놓이지 않았다. 이 편향이 표 전체에 깔려 있다.
...
>
> 이 지표가 확정하는 것은 "2025년 4월경 LangChain 월간 다운로드가 OpenAI SDK를 넘어섰다고 발표됐다"까지다. 그리고 지표 자체의 성질도 함께 봐야 한다 — 다운로드 수는 채택의 대리 지표일 뿐이고, CI 파이프라인의 재설치가 상당 부분을 차지한다. 추세의 방향은 읽히지만 절대값은 근거가 되지 못한다.
...
여기까지의 서사에는 공통 전제가 하나 깔려 있다 — **모델이 좋아지면 제품도 좋아진다**는 전제다. 그런데 2026년의 논의는 정확히 그 전제를 의심하는 데서 시작한다. 모델은 상향 평준화됐는데 제품 성능이 비례하지 않는다면 원인은 어디에 있는가. 답이 모델이 아니라 모델을 감싼 실행 구조에 있다면, 개선 예산은 모델 교체가 아니라 다른 곳으로 가야 한다. 그 진단과 그것이 개발 조직에 미친 영향이 이 카테고리의 다음 시리즈이며, [Agent Harness — 컨텍스트를 파일로 밀어내고 비우는 구조](/blog/ai-agent/agent-harness-architecture/)에서 시작한다.

무엇을 재야 개선을 말할 수 있는지, 도입할 때 무엇부터 정하는지 같은 운영 층의 판단은 [운영 Q&A](/blog/ai-agent/ai-agent-qna-operations/)에 문답으로 정리돼 있다.
