...
> **L4는 판정 단위를 답변에서 문장으로 내리면 완화된다.** 답변을 문장으로 쪼개 각각을 문서와 대조하면 어느 문장이 근거 없는지 특정되고, 재생성이 그 문장만 겨냥할 수 있다. 전체를 다시 만들 이유가 사라진다.
>
> 그런데 이것도 비용 쪽으로 되돌아온다. 문장이 다섯이면 환각 판정 1회가 5회가 된다. **L2의 완화책이 비용과 충돌한 것과 같은 구조다** — 판정을 정밀하게 만드는 수단은 예외 없이 호출 횟수를 늘린다.
