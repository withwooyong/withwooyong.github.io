...
| 교두보 | 비치헤드 | 처음 잡을 하나의 좁은 시장. 한 번에 하나만 잡는다 | 편5 |
| 해자 | Moat | 왜 모방당하지 않는가에 대한 답 | 편5 |
...
| 용어 | 원어 | 뜻 | 다루는 편 |
| --- | --- | --- | --- |
| 라이프사이클 | 4D | 탐색·기획·제작·성장 네 단계를 도는 바퀴 | 편2 |
...
| --- | --- |
| 기획·디자인·마케팅 영역이 어떤 도구를 쓰는가 — 도구의 목록과 선택 기준 | [병목은 만드는 자리에 없다](/blog/ai-transformation/discovery-to-growth-tooling/) |
| 기획 이후 단계, 즉 개발·검증·배포에 사람 게이트를 어디에 박는가 | [요구사항에서 운영까지 여섯 게이트](/blog/ai-transformation/sdlc-human-gates/) |

**두 글과 이 시리즈는 같은 문제의 다른 구간을 맡는다.** 앞의 글이 도구의 지형을 그리고 뒤의 글이 만드는 단계의 게이트를 다룬다면, 이 시리즈는 그보다 앞, 만들지 말지를 정하는 자리에 게이트를 세운다.

## 아홉 편이 각각 내세우는 것

여덟 편은 각자 하나씩 주장을 세운다. 지도를 먼저 훑을 때는 이 목록만 봐도 된다.
...
**여덟 줄 가운데 여섯 줄이 「무엇을 하지 않는가」를 담고 있다.** 순서를 바꾸지 않고, 통과로 세지 않고, 게이트를 대신하지 않고, 선언하지 않고, 재고 넘어가지 않고, 혼자 갖고 있지 않는다. 이 방법론의 형태가 그렇다 — 더 잘하는 법이 아니라 **덜 건너뛰는 법**이다.
...
| 편 | 다루는 것 | 이 편이 답하는 질문 |
| --- | --- | --- |
| 편2 — 좋은 제품이 지나는 네 단계 | 탐색·기획·제작·성장의 순환 구조와 단계를 건너뛸 때의 비용 | 순서를 바꾸면 무엇이 망가지는가 |
| 편3 — 만들지 말지를 먼저 묻는 세 질문 | 증거·중복·수익성 세 관문과 발견 단계의 도구 | 조사를 어디서부터 시작하는가 |
