...
description: "믿음과 가설을 가르는 것은 검증 가능성이고, 기능을 답으로 정해 두면 기회영역이 닫힌다. 비교 실험이 답하는 것과 답하지 않는 것, 요구사항이 접수되어 종료되기까지의 아홉 단계, 품질을 개발이 아니라 기획 단계에서 시작하는 방법, 그리고 화면이 아니라 흐름을 먼저 설계해야 하는 도메인까지 따라간다."
...
| 가설 | 결제 기능만 붙이면 매출이 따라올 것이다 | 정보를 얻기 어렵다는 문제를 짚고, 조회율 30%가 매출로 이어지는지 본다 |
...
| 완료 판정 | 없다. 만들었으면 끝이다 | 목록과 검색을 주고, 자릿수를 제한하고, 입력이 끝나면 버튼이 살아난다 |
...
쓸 때 지킬 것이 넷이다. 될 수 있는 대로 작게 쪼갠다 — 「뱅킹 시스템을 쓸 수 있다」는 판정이 불가능하지만 「첫 화면에서 송금 메뉴를 고를 수 있다」는 가능하다. 화면을 보며 확인할 수 있는 단위로 끊는다. 인수조건은 상세히 적는다 — 화면을 그리고 코드를 쓰는 사람이 꺼내 쓰는 정보다. 그리고 두 주가량인 한 주기 안에 만들 수 있는 크기로 묶는다.
...
크기가 정해진 이야기는 작성에서 시작해 과업 분해, 인수, 검증 항목 작성과 수행을 거쳐 배포로 끝난다.
...
| 3 | 1차 분석 | 이 요청이 실제로 푸는 문제가 무엇인가 |
| 4 | 2차 분석 | 이 방법 말고 다른 방법은 없는가 |
| 5 | 요구사항 문서 작성 | 만들 것과 완료 기준은 무엇인가 |
| 6 | 선별 위원회 승인 | 지금 자원을 여기에 쓸 것인가 |
| 7 | 업무 지정 | 누가 맡는가 |
| 8 | 착수와 관리 | 정해진 대로 가고 있는가 |
| 9 | 종료 | 완료 기준을 채웠는가 |

3단계와 4단계를 나눠 둔 것이 이 절차의 핵심이다. **한 번만 분석하면 요청을 그대로 명세로 옮기게 되고, 두 번 분석하면 요청과 해결책 사이에 다른 후보가 낄 자리가 생긴다.** 앞 절에서 기회영역을 열어 두라고 한 것이 절차로 굳은 자리다.
...
마지막 줄이 앞 절의 기술부채 이야기와 같은 자리다. **공통 기반 과제는 별도 안건으로 세워야 살아남는다** — 기능 과제와 한 줄에 놓으면 언제나 다음 분기로 밀린다.
...
호출 순서를 시간축으로 그리면 기업별·도메인별로 업무 범위가 어디서 갈리는지가 한 장에 드러난다.
...
| 이야기와 인수조건 | Story | 이용자 가치로 쓴 요구사항과, 완료를 판정하는 기준 |
