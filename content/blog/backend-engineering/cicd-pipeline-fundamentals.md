...
flowchart LR
    ...
    ART --> REG["레지스트리"]
    REG --> GATE{"승인 게이트<br/>사람이 누른다"}
    GATE --> CD["CD<br/>Rolling · Blue-Green"]
    CD --> HC{"Health Check"}
...
### 이 블로그 안에서 이미 다른 뜻으로 자리잡은 넷

시리즈 밖으로 한 발만 나가도 같은 낱말이 다른 것을 가리킨다.

| 낱말 | 이 시리즈에서 | 이 블로그의 다른 자리에서 |
| --- | --- | --- |
| **파이프라인** | 커밋에서 배포까지의 단계 사슬 | Redis의 원자 연산 묶음 · 데이터/ML 변환 사슬 · 배치의 청크 처리 루프 |
| **워크플로** | `.github/workflows/*.yml` 파일 하나 | [미리 정의된 코드 경로로 오케스트레이션되는 시스템](/blog/ai-agent/agent-vs-workflow/) · Airflow의 잡 오케스트레이션 |
| **블루그린** | **서버 세트** 두 벌을 두고 전환 | **인덱스·노드 세트** 두 벌을 두고 전환 — [Elasticsearch 운영](/blog/search-engineering/elasticsearch-operations/) |
| **게이트** | Quality Gate — 기계가 수치로 판정하는 관문 | [사람 게이트](/blog/ai-transformation/sdlc-human-gates/) — 배포 승인자와 감사 기록이 놓이는 자리 |

첫 행이 가장 심하다. 이 카테고리 안에서만 「파이프라인」이 이미 네 가지를 가리키고 있고 이 시리즈가 더해지면 다섯이 된다. 그래서 뒤의 편들은 이 낱말을 홀로 쓰지 않고 **「CI/CD 파이프라인」**·**「Jenkins Pipeline」** 처럼 한정어를 붙인다.
...
| Artifact | — | 빌드 결과물 파일(jar 등). CI 서버에 업로드·다운로드해 단계 간에 전달한다 |
...
| 2 | 형상관리와 Git의 세 영역 | 「되돌린다」는 무엇을 어디로 되돌리는 것인가 |
| 3 | 브랜치 전략과 컨테이너 | 무엇을 단위로 합치고 무엇을 단위로 나르는가 |
| 4 | GitHub Actions | 저장소 이벤트에서 시작하는 파이프라인은 어떻게 조립되는가 |
