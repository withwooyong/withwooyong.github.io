...
ERD는 네 가지로 이뤄진다. 이보다 적으면 모델이 성립하지 않고, 더 많은 표기는 이 넷의 변형이다.
...
세 행 중 실무에서 사고가 나는 곳은 마지막 행이다. M:M은 표기법에는 있지만 저장 구조에는 없어서, **모델을 그린 사람과 테이블을 만든 사람이 다르면 조용히 1:M으로 눌러 담긴다.**
...
화살표 라벨을 이어 읽으면 정규화가 무엇을 하는 절차인지 한 줄로 나온다 — **종속성을 한 종류씩 제거하는 것**이다. 단계가 올라갈수록 제거 대상이 좁아지고, 그만큼 실무에서 마주치는 빈도도 떨어진다.
...
여섯 단계 중 개발자가 흔히 떠올리는 것은 1·4번뿐이고, 실제 일정을 잡아먹는 것은 3·5번이다. 조회 SQL이 어디에 몇 개나 있는지는 **모델을 바꾸기로 결정한 뒤에야** 세어진다.
...
다섯 행의 오른쪽 열은 전부 같은 형태다 — **읽기를 싸게 만든 대가를 쓰기 쪽이 지불한다.** 그래서 반정규화의 성패는 「얼마나 빨라졌나」가 아니라 「그 대가를 누가 어떻게 감당하기로 했나」에서 갈린다. 아래 3원칙을 순서대로 적용한다.
...
세 번째가 가장 자주 빠진다. 중복 컬럼을 만든 사람은 그 순간 동기화 코드를 함께 쓰지만, **여섯 달 뒤에 원본 컬럼을 수정하는 사람은 사본이 있다는 사실을 모른다.** 값 불일치는 그렇게 생긴다.
...
네 행 모두 「이름이 다르다」에서 출발해 「비용이 든다」로 끝난다. 특히 세 번째 행의 70%는 분석 인력이 모델링 부재의 비용을 뒤늦게 대신 치르고 있다는 뜻이다. 표준화는 네 단계로 진행한다.
...
모델링의 각 단계는 「무엇을 얻고 무엇을 지불하는가」로 요약된다.
...
다섯 행을 관통하는 질문은 하나다 — **되돌리는 비용이 얼마인가.** 정규화는 되돌릴 수 있고(반정규화가 그것이다), 반정규화는 정규화된 모델이 남아 있을 때만 되돌릴 수 있으며, 운영 중인 데이터 구조 변경은 앞에서 본 6단계를 전부 치른다. 그래서 모델링에 쓰는 시간은 설계 품질의 문제이기 전에 **나중에 지불할 금액을 지금 깎는 일**이다.
