...
운영 측면에서는 단일 스레드 모델 하나가 대부분의 제약을 설명한다. 원자성을 공짜로 얻는 대신 느린 명령 하나가 전체를 세우고, 그래서 `KEYS`가 금지되며, 그래서 대형 컬렉션 연산을 피한다. 여기까지의 선택지를 한 표로 모으고 실패 모드 여덟 가지와 함께 정리한 내용은 다음 편에서 다룬다.
