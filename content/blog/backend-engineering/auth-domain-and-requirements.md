...
[다음 편](/blog/backend-engineering/requirements-spec-to-erd/)은 이 문서들이 실제로 다음 단계의 입력이 되는 과정을 따라간다. 요구 문장 하나마다 API 한 줄이 붙고, 그렇게 쌓인 표에서 되풀이되는 명사가 엔티티 후보로 올라오며, 후보끼리 몇 개씩 이어지는지를 정하는 동안 저장 구조가 확정되는 길이다. 여기서 8번 이벤트가 요구했던 「사람과 역할을 잇는 표」가 실제 이름을 갖게 된다.
