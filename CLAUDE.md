...
- `npm run e2e` — Playwright over `e2e/`. It serves the **built** `out/`, not a dev server, so run `npm run build` first; `e2e/global-setup.ts` refuses to start (exit 1) when `out/` is older than the sources rather than testing yesterday's artifact. **That refusal exits 1 without running a single test, which looks exactly like a real failure** — read the first line, not `$?`. Note `scripts/` counts as a source, so adding any script invalidates `out/` — and so does a **comment-only** edit under `components/`, since the guard reads mtimes, not content. Right now it is **expected to fail with 4 out of 48** (18 passed · 26 skipped, measured 2026-08-27) — the shell is attached to no page, so the `/atlas/` sentinel is red on purpose in **both** `shell.spec.ts` and `search.spec.ts`, × desktop·mobile. That duplication is deliberate: a sentinel answers "is this file's silence justified?" for *its own* file, and a suite without one goes quietly green. Keep this count current — a stale number here is indistinguishable from a regression to the next reader. It joins CI in the prior plan's T14, not before.
...
| **A `-g` filter silently removes the sentinel that justifies a suite's skips** | This repo pairs every gated suite with a sentinel in a *separate* `describe`, so any filter narrow enough to select the gated tests drops the sentinel with it. Measured 2026-08-27: `npx playwright test e2e/search.spec.ts -g "검색 팔레트" --project=desktop` → `4 skipped`, **exit 0**. A perfect silent green — the run reports success precisely because everything meaningful was skipped and the one test that would have said so was filtered out | Never treat a filtered run's green as passing. Judge gate state from a **full** run, and read the skip count, not `$?`. The sentinel and its gate are one unit; filters cut through it |
...
| **Pagefind fragments are gzip, so grepping `out/` is structurally blind** | `npx pagefind --site out` writes 242 `.pf_fragment` blobs that begin `\037\213\b`. `grep -a` over them returns 0 for a string that is provably inside — the control (`url`) also returns 0, so the zero means "cannot read", not "not present". `check-forbidden:built` scans `out/blog` and therefore never sees these files at all: the shipped bytes now include a body-text copy that no checker reads. Verified once by hand (all 242 inflated to 3,992,070 bytes, then scanned — **no leak today**) | Inflate before scanning: `for f in out/pagefind/fragment/*.pf_fragment; do gzip -dc "$f"; done > /tmp/frag.txt`. Always grep a control string you know is there first — a zero on compressed input is not evidence. Note the inflated bytes are **not** pure JSON either: each fragment begins with a `pagefind_dcd` prefix, so `JSON.parse` on them dies with `Unexpected token 'p'` — slice from the first `{` |
| **A `declare module` for a `/`-rooted specifier can never apply** | `import("/pagefind/pagefind.js")` throws TS2307. Adding `types/pagefind.d.ts` with `declare module "/pagefind/pagefind.js";` changes nothing — and `tsc --noEmit --listFiles \| grep types/pagefind` **confirms the d.ts is in the program**, which reads as "the declaration loaded, so the problem must be elsewhere". It isn't: TypeScript resolves a `/`-prefixed specifier as a rooted path rather than a module name, so no ambient declaration can ever match it. The confirming `--listFiles` output is what makes this expensive — it sends you hunting through `include` patterns. Hit for real in T5 | Stop chasing `tsconfig`. Use a form neither bundler nor checker can parse: `new Function('return import("/path.js")')()`. Verify it in a real browser rather than assuming — measured here as 45 results with zero console errors. It needs `unsafe-eval`, which GitHub Pages does not restrict, but a stricter host would |
| **`locator('[role="dialog"]')` counts hidden dialogs; `getByRole('dialog')` does not** | Measured on one page: the CSS-attribute locator returns **2** (this repo's mobile drawer stays in the DOM behind `hidden`), the ARIA locator returns **1**. A test asserting "exactly one dialog is open" passes or fails purely on which locator it uses — and the attribute selector looks more literal, so it is the tempting one to reach for | Assert dialog counts with `getByRole`. Same family as the `exact: true` row above: the locator, not the markup, decides what you actually measured |
