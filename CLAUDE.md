...
| `npm run check-mermaid:verify` → `npm run check-mermaid` · `npm run check-mermaid:docs` | 🔴 **그려지지 않는 도식.** 리포에 Mermaid 블록이 **654개**(발행본 544 · 문서 110) 있는데 문법이 어긋나면 화면에는 흐름도 대신 빨간 오류 상자가 뜬다. 판정은 정규식이 아니라 **화면을 그리는 mermaid 파서**가 한다. 🔴 위험은 위반을 놓치는 것이 아니라 **거짓 0** 이다 — Node 에는 DOM 이 없어 파싱에 닿기도 전에 죽는데 그것을 「위반 없음」으로 세면 초록이 나온다. **실측 649개 중 621개가 그렇게 빠졌다.** 그래서 검사기는 판정에 **도달한** 수를 함께 세고, 카나리 도식 하나를 먼저 파싱해 파서가 살아 있는지 본다. 못 미치면 종료 코드 2 |
