...
- `npm run e2e` — Playwright over `e2e/`. It serves the **built** `out/`, not a dev server, so run `npm run build` first; `e2e/global-setup.ts` refuses to start (exit 1) when `out/` is older than the sources rather than testing yesterday's artifact. As of T8 it is **expected to fail** — the shell is not attached to any page yet, so the sentinels plus `/work/`·`/about/` are red on purpose. It joins CI in T14, not before (see the plan's T14 Step 5).
