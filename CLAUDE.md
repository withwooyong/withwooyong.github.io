...
- **실측 수치를 적지 마라.** 테스트 개수·발행본 수·E2E 통과 수는 **명령을 적고 결과는 적지 않는다.** 과거에 이 파일의
  수치는 절반 이상이 틀린 채 커밋돼 있었고, 낡은 숫자는 다음 사람에게 회귀와 구분되지 않는다. 게이트로 필요하면
  문서가 아니라 검사기로 만든다(`check-counts`). **측정값은 썩고 결정값은 썩지 않는다** — 상한·금지·순서는 적어도 된다.
- **상한 200줄** — Claude Code 공식 지침값이다(그 이상은 컨텍스트를 더 먹고 준수율이 떨어진다).
  `wc -l CLAUDE.md` 로 확인한다. 여유가 있어도 **한 줄을 더하면 한 줄을 뺀다** — 이 파일은 21개 커밋 동안
  한 번도 줄어든 적이 없다. 늘어날 것 같으면 `.claude/rules/` 의 `paths:` 로 경로 스코프를 걸어
  해당 파일을 만질 때만 로드시키거나, `/doctor` 로 트림 제안을 받는다.
