 ...
 * `<SiteShell>` 로 감싸는 순간(아틀라스·검색 계획서 T11), **아무도 플래그를 켜지 않아도**
 * 저절로 돈다. 게이트가 사람이 뒤집는 스위치가 아니라 **셸의 실재** 그 자체이기 때문이다.
 *
 * ⚠️ skip 은 이 리포가 반복해서 데인 「거짓 0」과 같은 얼굴을 하고 있다.
 *    그래서 이 게이트는 **`shell.spec.ts` 맨 위의 센티넬**(「셸이 `SHELL_HOME` 에 붙어 있다」)과
...
export async function gotoWithShell(page: Page, path: string): Promise<void> {
  ...
  test.skip(
    ...
    `셸 미부착: ${path} — 아틀라스·검색 계획서 T11 에서 켜진다 (shell.spec.ts 의 센티넬을 보라)`,
  );
}
