...
## 2026-09-02 — `product-management` **프로덕트 매니지먼트 편3**(총 179편) · 반려선으로 지정한 편이 반대 방향으로 어긋났고, 배정 밖 블록에서 온 문장은 축자 대조에 걸리지 않는다

> 원본 `pm-po/01` §5·§6 5,590 B → `tactics-and-goal-setting` **14,372 B**.
> 설계값 13,969 대비 **+2.9%** · 밴드 13,187 ~ 14,752 **안**(상단까지 380 B).
